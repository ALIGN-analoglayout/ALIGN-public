...
if __name__ == "__main__":
  ...
  if True:
    ...
    w.create_lib( "pcell", 0.00025, 2.5e-10)
    ...
    w.gds_write_xy( rect_to_boundary( [-36,-36,36,36]))
    w.gds_write_endel()

    w.gds_write_boundary()
    w.gds_write_layer( 20)
    w.gds_write_datatype( 0)
    w.gds_write_xy( rect_to_boundary( [-56,-36,56,36]))
    w.gds_write_endel()

    w.gds_write_boundary()
    w.gds_write_layer( 30)
    w.gds_write_datatype( 0)
    w.gds_write_xy( rect_to_boundary( [-36,-56,36,56]))
    w.gds_write_endel()
