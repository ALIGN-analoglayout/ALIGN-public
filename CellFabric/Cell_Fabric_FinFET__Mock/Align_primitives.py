...
if __name__ == "__main__":
    ...
    if args.primitive in ["Switch_NMOS", "Switch_PMOS"]:
        ...
        cell_pin = ["S", "DA", "DB"]
        for (x,y) in ( (x,y) for x in range(x_cells) for y in range(y_cells)):
           Routing = [('S', S, 1, CcM3), ('DA', DA+G, 2, CcM3-1), ('DB', DB, 3, CcM3+1)]
           uc.unit( x, y, x_cells, y_cells, fin_u, fin, finDummy, gate, gateDummy, SDG, Routing)

    elif args.primitive in ["CMC_NMOS_S", "CMC_PMOS_S"]:
        cell_pin = ["S", "DA", "DB", "G"]
        for (x,y) in ( (x,y) for x in range(x_cells) for y in range(y_cells)):
            Routing = [('S', S, 1, CcM3), ('DA', DA if y%2==0 else DB, 2, CcM3-1), ('DB', DB if y%2==0 else DA, 3, CcM3+1), ('G', G, 4, CcM3-2)]
            uc.unit( x, y, x_cells, y_cells, fin_u, fin, finDummy, gate, gateDummy, SDG, Routing)
