...
class RingGenerator(DefaultCanvas):
    ...
    def addRing( self, x_cells, y_cells):
        ...
        self.addWire( self.m1_ring, 'Body', 'Body', 0, (self.ring_enclosureY, -1), (gridY+self.ring_enclosureY, -1))
        for j in range(self.ring_enclosureY+1, gridY+self.ring_enclosureY): 
           self.addVia( self.va, 'Body', None, 0, j)
        #self.addWire( self.m2, None, None, 1, (1, -1), (5, -1))
        #
        self.addRegion( self.active_ring, None, None, 0, self.ring_enclosureY, 1, (gridY+self.ring_enclosureY))

        self.addRegion( self.pselect_ring, None, None, -(gridX//2+self.ring_enclosureX), 0, (gridX//2+self.ring_enclosureX), (gridY+2*self.ring_enclosureY))
