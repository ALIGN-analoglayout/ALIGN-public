...
def run_sh(cmd, tag=None):
    ...
    with subprocess.Popen([cmd], shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                          encoding='utf-8', universal_newlines=True, bufsize=1) as p:
...
def run_router_in_container(args):
    ...
    run_sh(
        f'(cd {args.techdir} & & tar cvf - .) |'
        f'docker run - -rm {M_DR_COLLATERAL} - i ubuntu bash - c "cd /Cktgen/DR_COLLATERAL && tar xvf -"',
    ...
    run_sh(
        f'docker run --name sam {M_out} {M_INPUT} {M_DR_COLLATERAL} {ROUTER_IMAGE} '
        'bash -c "cd /Cktgen && amsr.exe -file INPUT/ctrl.txt"',
        'run detailed_router')
...
def cmdline(argv):
    ...
    cmd = f'-n mydesign {route}{showglobalroutes}{showmetaltemplates}{source}{placer_json}' \
          f'{gr_json}{small}{nets_to_route}{nets_not_to_route}{topmetal}{no_interface}'
