...
def json_lef(input_json, out_lef, cell_pin, bodyswitch, blockM, p):
    ...
    assert (j['bbox'][3]-j['bbox'][1]) % m2pitch == 0, f"Cell height not a multiple of the grid {j['bbox']}"
    assert (j['bbox'][2]-j['bbox'][0]) % m1pitch == 0, f"Cell width not a multiple of the grid {j['bbox']}"
