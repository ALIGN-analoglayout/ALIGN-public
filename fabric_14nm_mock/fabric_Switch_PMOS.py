...
class UnitCell:
    ...
    def unit( self, x, y):
        ...
        if x == x_cells -1 and y == y_cells -1:
            uc.pselectSegment( 'pselect', 'NA', -self.m2Pitch//2, ((y+1)*y_length + self.m2Pitch//2), (-self.plPitch//2), ((1+x)*(x_length) - self.plPitch//2)) ####Nselect Region
...
############### M3 routing ###########################
        for i in range(3):
            ...
            if x == 0 and y_cells > 1 and i == 2:
                if y == 0:
                    uc.m3c1Segment( 'm3', 'NA', (i+self.gateDummy +(x*self.gate) -1), ((self.m2Pitch*(2 + (self.finDummy-1)/2)) + self.m2Offset - self.m2Width/2 - self.v_enclosure), ((y_cells-1)*self.m2Pitch*(fin//2) + (self.m2Pitch*(2 + (self.finDummy-1)/2)) + self.m2Offset + self.m2Width/2 + self.v_enclosure))  ### Drain Connection
...
############### M2 routing ###########################
        ...
        for i in range((m2_tracks+1)):
            ...
            if x_cells > 0 and x == 0 and i == (y*(m2_tracks //y_cells) + self.finDummy//2 + 0):
                if i%2 == 0:
            ...
            if x_cells > 0 and x == 0 and i == (y*(m2_tracks //y_cells) + self.finDummy//2 + 1):
                if i%2 == 0:
            ...
            if x_cells > 0 and x == 0 and i == (y*(m2_tracks //y_cells) + self.finDummy//2 + 2):
                if i%2 == 0:
                    uc.m2c1Segment( 'm2', 'D', (self.m1Offset + (self.gateDummy-1)*self.plPitch - self.m1Width/2 - self.v_enclosure), (self.m1Offset + (x_cells*self.gate - 2*self.gateDummy + 2)*self.plPitch + self.m1Width/2 + self.v_enclosure), i)
                else:
                    uc.m2c2Segment( 'm2', 'D', (self.m1Offset + (self.gateDummy-1)*self.plPitch - self.m1Width/2 - self.v_enclosure), (self.m1Offset + (x_cells*self.gate - 2*self.gateDummy + 2)*self.plPitch + self.m1Width/2 + self.v_enclosure), i)
...
######## Vias placement ########
        if (x_cells - 1 - x) == 0:
            ...
            for i in D:                    
                uc.v1Segment( 'v1', 'NA', (i*self.m1Pitch + self.m1Offset - self.v1Width/2), (i*self.m1Pitch + self.m1Offset + self.v1Width -   self.v1Width/2), (y*(m2_tracks //y_cells) + self.finDummy//2 + 2))
