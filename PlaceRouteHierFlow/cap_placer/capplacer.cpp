...
json ToJsonAry(const PnRDB::bbox& box) {
  json xy = json::array();
  xy.push_back(box.LL.x);
  xy.push_back(box.LL.y);
  xy.push_back(box.LL.x);
  xy.push_back(box.UR.y);
  xy.push_back(box.UR.x);
  xy.push_back(box.UR.y);
  xy.push_back(box.UR.x);
  xy.push_back(box.LL.y);
  xy.push_back(box.LL.x);
...
json ToJsonAry(const PnRDB::point& p0, const PnRDB::point& p1) {
  json xy = json::array();
  xy.push_back(p0.x);
  xy.push_back(p0.y);
  xy.push_back(p1.x);
...
Placer_Router_Cap::Placer_Router_Cap(const string& opath, const string& fpath, PnRDB::hierNode& current_node, PnRDB::Drc_info& drc_info,
...
void Placer_Router_Cap::Placer_Router_Cap_function(vector<int>& ki, vector<pair<string, string> >& cap_pin, const string& fpath, const string& unit_capacitor,
  ...
  logger->debug("pin_minx {0} pin_miny {1} shifting_x {2} shifting_y {3} H_metal {4} V_metal {5} HV_via_metal {6}", pin_min.x, pin_min.y, shifting.x,
                shifting.y, H_metal, V_metal, HV_via_metal);
  ...
  min_dis = PnRDB::point((drc_info.Metal_info.at(V_metal_index).width + drc_info.Metal_info.at(V_metal_index).dist_ss),
                         (drc_info.Metal_info.at(H_metal_index).width + drc_info.Metal_info.at(H_metal_index).dist_ss)) *
            2;
  ...
  if (index.size() == 1) {
    ...
    for (unsigned int i = start_index; i < dis.size(); i++) {
      for (unsigned int j = i + 1; j < dis.size(); j++) {
        ...
        if (Caps[index[i]].index_x + Caps[index[j]].index_x == 2 * Cx && Caps[index[i]].index_y + Caps[index[j]].index_y == 2 * Cy) {
  ...
  if (dummy_flag) {
    auto not_on_border = [&](const auto& c) { return c.index_x != 0 && c.index_x != r - 1 && c.index_y != 0 && c.index_y != s - 1; };
...
static int getLayerMask(const std::string& layer, const PnRDB::Drc_info& drc_info) {
  int index = drc_info.Metalmap.at(layer);
  int mask = stoi(drc_info.MaskID_Metal.at(index));
  return mask;
}

static int getLayerViaMask(const std::string& layer, const PnRDB::Drc_info& drc_info) {
  int index = drc_info.Metalmap.at(layer);
  int mask = stoi(drc_info.MaskID_Via.at(index));
  return mask;
}

void fillContact(PnRDB::contact& con, const PnRDB::bbox& box) {
...
void Placer_Router_Cap::ExtractData(const string& fpath, const string& unit_capacitor, const string& final_gds, const PnRDB::lefMacro& uc,
                                    const PnRDB::Drc_info& drc_info, int H_metal, int V_metal, int HV_via_index, const string& opath) {
  ...
  auto extract_data_1_2 = [&](auto& n_array) {
    for (unsigned int i = 0; i < n_array.size(); i++) {  // for each net
      ...
      for (unsigned int j = 0; j < n_array[i].start_connection_pos.size(); j++) {  // for segment
        ...
        PnRDB::bbox box = fillPathBBox(n_array[i].start_connection_pos[j], n_array[i].end_connection_pos[j], width);
        minmax.update(box);
  ...
  for (unsigned int i = 0; i < uc.interMetals.size(); i++) internal_metal_layer.insert(uc.interMetals[i].metal);

  for (auto it = internal_metal_layer.begin(); it != internal_metal_layer.end(); it++) internal_metal.push_back(*it);
...
void Placer_Router_Cap::cal_offset(const PnRDB::Drc_info& drc_info, int H_metal, int V_metal, int HV_via_index) {
  ...
  for (unsigned int i = 0; i < Nets_pos.size(); i++) {                            // for each net
    for (unsigned int j = 0; j < Nets_pos[i].start_connection_pos.size(); j++) {  // for segment
      ...
      minmax.update(fillPathBBox(Nets_pos[i].start_connection_pos[j], Nets_pos[i].end_connection_pos[j], width));
    }
  }
  ...
  for (unsigned int i = 0; i < Nets_neg.size(); i++) {                            // for each net
    for (unsigned int j = 0; j < Nets_neg[i].start_connection_pos.size(); j++) {  // for segment
      ...
      minmax.update(fillPathBBox(Nets_neg[i].start_connection_pos[j], Nets_neg[i].end_connection_pos[j], width));
    }
  }
...
void Placer_Router_Cap::Router_Cap(vector<int>& ki, vector<pair<string, string> >& cap_pin, bool dummy_flag, bool cap_ratio, int cap_r, int cap_s) {
  ...
  for (int i = 0; i <= r; i++) {
    num_router_net_v.push_back(0);
  }
  for (int i = 0; i <= s; i++) {
    num_router_net_h.push_back(0);
  }
    ...
    for (unsigned int i = 0; i < n_array.size(); i++) {
      ...
      if (sum > 0) {
        ...
        for (unsigned int l = 0; l < hrlv.size(); l++) {
          ...
          int found = std::inner_product(
              ci.begin(), ci.end(), lv.begin(), false, [](bool a, bool b) { return a || b; }, [](int a, int b) { return a == 1 && b == 1; });
          if (found == 0) {
            int router_num = n_array.size();
            int choosen_route = -1;
...
void Placer_Router_Cap::addVia(net& temp_net, PnRDB::point& coord_pos, const PnRDB::Drc_info& drc_info, const string& HV_via_metal, int HV_via_metal_index,
                               int isPin) {
  ...
  auto apply_viax = [&](const auto& ra, int idx) { apply_aux_pt(PnRDB::point(ra[0].x, 0), PnRDB::point(ra[1].x, 0), idx); };

  auto apply_viay = [&](const auto& ra, int idx) { apply_aux_pt(PnRDB::point(0, ra[0].y), PnRDB::point(0, ra[1].y), idx); };
...
class MinMaxBox {
  ...
  int get_left_right() const { return left_right; }
};

void Placer_Router_Cap::GetPhysicalInfo_merged_net(vector<net>& n_array, vector<int>& trails, const PnRDB::Drc_info& drc_info, const string& H_metal,
                                                   const string& V_metal, const string& HV_via_metal, int HV_via_metal_index, int grid_offset, int sign) {
...
void Placer_Router_Cap::GetPhysicalInfo_common_net(vector<net>& n_array, vector<int>& trails, const PnRDB::Drc_info& drc_info, const string& H_metal,
                                                   const string& V_metal, const string& HV_via_metal, int HV_via_metal_index, int grid_offset, int sign) {
  //  pair<double,double> coord;
...
void Placer_Router_Cap::GetPhysicalInfo_router(const string& H_metal, int H_metal_index, const string& V_metal, int V_metal_index,
  ...
  GetPhysicalInfo_merged_net(Nets_pos, trails, drc_info, H_metal, V_metal, HV_via_metal, HV_via_metal_index, grid_offset, 1);

  GetPhysicalInfo_common_net(Nets_pos, trails, drc_info, H_metal, V_metal, HV_via_metal, HV_via_metal_index, grid_offset, 1);

  GetPhysicalInfo_merged_net(Nets_neg, trails, drc_info, H_metal, V_metal, HV_via_metal, HV_via_metal_index, grid_offset, -1);

  GetPhysicalInfo_common_net(Nets_neg, trails, drc_info, H_metal, V_metal, HV_via_metal, HV_via_metal_index, grid_offset, -1);
}

extern void JSONReaderWrite_subcells(string GDSData, long int& rndnum, vector<string>& strBlocks, vector<int>& llx, vector<int>& lly, vector<int>& urx,
                                     vector<int>& ury, json& mjsonStrAry);

extern void JSONExtractUit(string GDSData, double& unit);

extern void addOABoundaries(json& jsonElements, int width, int height);

PnRDB::bbox Placer_Router_Cap::fillPathBBox(const PnRDB::point& start, const PnRDB::point& end, int half_width) {
  PnRDB::bbox box;
...
void Placer_Router_Cap::WriteGDSJSON(const string& fpath, const string& unit_capacitor, const string& final_gds, const PnRDB::Drc_info& drc_info,
                                     const string& opath) {
  ...
  auto doit0 = [&](const auto& n_array) {
    for (unsigned int i = 0; i < n_array.size(); i++) {  // for each net
      ...
      for (unsigned int j = 0; j < n.start_connection_pos.size(); j++) {  // for segment
        ...
        auto box = fillPathBBox(n.start_connection_pos[j], n.end_connection_pos[j], width) / unitScale;
        //		box = box * unitScale;
        json bound;
        bound["type"] = "boundary";
        bound["datatype"] = 0;
...
void Placer_Router_Cap::WriteViewerJSON(const string& fpath, const string& unit_capacitor, const string& top_name, const PnRDB::Drc_info& drc_info,
                                        const string& opath) {
...
void Placer_Router_Cap::Common_centroid_capacitor_aspect_ratio(const string& opath, const string& fpath, PnRDB::hierNode& current_node,
                                                               PnRDB::Drc_info& drc_info, const map<string, PnRDB::lefMacro>& lefData, bool aspect_ratio,
                                                               int num_aspect) {  // if aspect_ratio 1, then do CC with different aspect_ratio; Else not.
        ...
        if (current_node.CC_Caps[j].CCCap_name == b.name) {
          ...
          */
          ...
          for (unsigned int q = 0; q < cap_r.size(); q++) {
            ...
            Placer_Router_Cap_function(ki, pin_names, fpath, unit_capacitor, cc_gds_file, cap_ratio || aspect_ratio, cap_r[q], cap_s[q], drc_info, lefData,
                                       dummy_flag, opath);
...
void Placer_Router_Cap::PrintPlacer_Router_Cap(string outfile) {
  ...
  for (unsigned int i = 0; i < Caps.size(); i++) {
    std::vector<PnRDB::point> signs = {PnRDB::point(-1, -1), PnRDB::point(-1, +1), PnRDB::point(+1, +1), PnRDB::point(+1, -1), PnRDB::point(-1, -1)};
...
void Placer_Router_Cap::WriteLef(const PnRDB::block& temp_block, const string& file, const string& opath) {
  ...
  auto s = [](int v) { return (double)v / 2000.0; };
