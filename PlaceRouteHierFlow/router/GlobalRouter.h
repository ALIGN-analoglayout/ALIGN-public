...
class GlobalRouter : public RawRouter {
  ...
  void AddConnectedContactFunc(PnRDB::hierNode& HierNode, RouterDB::Net& net, int net_index, RouterDB::NType stype, int siter, int siter2, int smetal, int sx,
                               int sy, int mIdx);

  public:
  GlobalRouter();
  GlobalRouter(PnRDB::hierNode& node, PnRDB::Drc_info& drcData, int Lmetal, int Hmetal,
               const std::string& bianryDIR);  // initial Nets & Blocks with node data, also LL, UR
  ...
  void MetalSpacingCheckFunc(std::set<std::pair<int, int>, IntPairComp>& checked, int val, int Midx, int ALLx, int ALLy, int AURx, int AURy, int h, int i,
                             int j);
