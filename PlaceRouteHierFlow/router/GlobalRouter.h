...
class GlobalRouter : public RawRouter {
  ...
  public:
    GlobalRouter();
    GlobalRouter(PnRDB::hierNode& node, PnRDB::Drc_info& drcData, int Lmetal, int Hmetal, std::string bianryDIR); //initial Nets & Blocks with node data, also LL, UR
...
//    // added by wbxu
    ...
    void listSegments(std::string binaryDIR); // reuse original function
