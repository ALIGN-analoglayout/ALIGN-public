...
void Router::RouteWork(int mode, PnRDB::hierNode& node, PnRDB::Drc_info& drcData, int Lmetal, int Hmetal, std::string binaryDIR, double rate, std::string inputfile) {
  ...
  if(mode==0) {
    ...
    std::cout<<"RouteWork "<<mode<<std::endl;
    PowerRouter(node, drcData, Lmetal, Hmetal, 1, rate, inputfile);  
  } else if(mode==3){
    std::cout<<"RouteWork "<<mode<<std::endl;
    PowerRouter(node, drcData, Lmetal, Hmetal, 0, rate,inputfile);
  } else if(mode==4){
    std::cout<<"RouteWork "<<mode<<std::endl;
