  ...
  auto collect_vdd_gnd_via = [&](auto& i, auto& temp_index) {
    if (temp_index != -1) {
      if (grid.total2graph.find(temp_index) != grid.total2graph.end()) {
        ...
        if (grid.vertices_graph[graph_index].active == 1 && grid.vertices_graph[i].power == 1 && grid.vertices_graph[graph_index].power == 1 &&
            CheckActive(grid, graph_index) && CheckActive(grid, i)) {
...
void Graph::collect_nodes(Grid& grid, vector<int> temp_vector, vector<int>& adjacent_nodes, int power) {
  ...
  for (unsigned int j = 0; j < temp_vector.size(); ++j) {
    if (grid.total2graph.find(temp_vector[j]) != grid.total2graph.end()) {
      ...
      if (index < int(grid.vertices_graph.size()) && index >= 0 && grid.vertices_graph[index].active && grid.vertices_graph[index].power == power &&
          grid.vertices_graph[index].graph_index == -1) {
             ...
             if(grid.total2graph.find(temp_vector[j])!=grid.total2graph.end())
               {
                  ...
                  if( index<grid.vertices_graph.size() && index>=0 && grid.vertices_graph[index].active && grid.vertices_graph[index].power == power &&
  grid.vertices_graph[index].graph_index==-1)
                    {
                      adjacent_nodes.push_back(index);
                    }
...
Graph::Graph(std::vector<std::pair<int, int> >& global_path, std::vector<std::vector<int> >& conectedTile, std::vector<int>& Tile_Source,
             std::vector<int>& Tile_Dest)
    : path_number(1) {
  std::map<int, int> tile_graph_map;
  std::map<int, int> graph_tile_map;
  CreateAdjacentList_Global_Path(global_path, conectedTile, Tile_Source, Tile_Dest, tile_graph_map,
                                 graph_tile_map);  // create adjacentList base gird.LL_graph and gird.UR_graph
...
void Graph::CreateAdjacentList_Global_Path(std::vector<std::pair<int, int> >& global_path, std::vector<std::vector<int> >& conectedTile,
                                           std::vector<int>& Tile_Source, std::vector<int>& Tile_Dest, std::map<int, int>& tile_graph_map,
                                           std::map<int, int>& graph_tile_map) {
...
void Graph::CreateAdjacentList(Grid& grid) {
  for (unsigned int i = 0; i < grid.vertices_graph.size(); ++i) {
    if (grid.vertices_graph[i].active == 1) {
      ...
      auto fx = [&](RouterDB::vertex& u, RouterDB::vertex& v) { return (double)abs(v.x - u.x) * grid.drc_info.metal_weight[u.metal]; };
      auto fy = [&](RouterDB::vertex& u, RouterDB::vertex& v) { return (double)abs(v.y - u.y) * grid.drc_info.metal_weight[u.metal]; };
      auto f1 = [&](RouterDB::vertex& u, RouterDB::vertex& v) { return (double)1.0; };
      ...
      for (unsigned int j = 0; j < grid.vertices_graph[i].north.size(); ++j) process(grid.vertices_graph[i], grid.vertices_graph[i].north[j], fy);

      for (unsigned int j = 0; j < grid.vertices_graph[i].south.size(); ++j) process(grid.vertices_graph[i], grid.vertices_graph[i].south[j], fy);

      for (unsigned int j = 0; j < grid.vertices_graph[i].east.size(); ++j) process(grid.vertices_graph[i], grid.vertices_graph[i].east[j], fx);

      for (unsigned int j = 0; j < grid.vertices_graph[i].west.size(); ++j) process(grid.vertices_graph[i], grid.vertices_graph[i].west[j], fx);

      if (grid.vertices_graph[i].down != -1) process(grid.vertices_graph[i], grid.vertices_graph[i].down, f1);

      if (grid.vertices_graph[i].up != -1) process(grid.vertices_graph[i], grid.vertices_graph[i].up, f1);
...
void Graph::RemovefromMultMap(std::multimap<double, int>& mmap, double dist, int idx) {
  ...
  if (mark) {
    ...
    logger->debug("Graph-Info: cannot found element in map");
  }
...
std::vector<std::vector<int> > Graph::GetShorestPath() { return Path; };
