...
GlobalGrid::GlobalGrid(const GlobalGrid& other)
    : x_unit(other.x_unit),
      y_unit(other.y_unit),
      metal2tile(other.metal2tile),
      tile2metal(other.tile2metal),
...
GlobalGrid::GlobalGrid(PnRDB::Drc_info& drc_info, int LLx, int LLy, int URx, int URy, int Lmetal, int Hmetal, int tileLayerNo, int scale) {
  ...
  for (int i = Lmetal; i <= Hmetal; ++i) {
    ...
    if (drc_info.Metal_info.at(i).direct == 0) {  // vertical
      ...
      for (std::map<RouterDB::point, int, RouterDB::pointXYComp>::iterator mit = this->XYmap.at(layerIdx).begin(); mit != this->XYmap.at(layerIdx).end();
           ++mit) {
        std::map<RouterDB::point, int, RouterDB::pointXYComp>::iterator mit2 = mit;
        std::advance(mit2, 1);
        if (mit2 == this->XYmap.at(layerIdx).end()) {
          continue;
        }
        int pre = mit->second;
        int post = mit2->second;
        RouterDB::tileEdge tmpE;
        if ((mit->first).x != (mit2->first).x) {
          continue;
        }
        if (this->tiles_total.at(pre).north.empty()) {
          // logger->debug("unit_X 1 {0}", drc_info.Metal_info.at(i).grid_unit_x);
          tmpE.next = post;
          tmpE.capacity = this->tiles_total.at(pre).width / drc_info.Metal_info.at(i).grid_unit_x;
        ...
        if (this->tiles_total.at(post).south.empty()) {
          ...
          tmpE.next = pre;
          tmpE.capacity = this->tiles_total.at(post).width / drc_info.Metal_info.at(i).grid_unit_x;
      ...
      for (std::map<RouterDB::point, int, RouterDB::pointYXComp>::iterator mit = this->YXmap.at(layerIdx).begin(); mit != this->YXmap.at(layerIdx).end();
           ++mit) {
        std::map<RouterDB::point, int, RouterDB::pointYXComp>::iterator mit2 = mit;
        std::advance(mit2, 1);
        if (mit2 == this->YXmap.at(layerIdx).end()) {
          continue;
        }
        int pre = mit->second;
        int post = mit2->second;
        RouterDB::tileEdge tmpE;
        if ((mit->first).y != (mit2->first).y) {
          continue;
        }
        if (this->tiles_total.at(pre).east.empty()) {
          // logger->debug("unit_y 1 {0}", drc_info.Metal_info.at(i).grid_unit_y);
          tmpE.next = post;
          tmpE.capacity = this->tiles_total.at(pre).height / drc_info.Metal_info.at(i).grid_unit_y;
        ...
        if (this->tiles_total.at(post).west.empty()) {
          ...
          tmpE.next = pre;
          tmpE.capacity = this->tiles_total.at(post).height / drc_info.Metal_info.at(i).grid_unit_y;
  ...
  for (std::vector<RouterDB::Net>::iterator nit = Nets.begin(); nit != Nets.end(); ++nit) {
    ...
    for (std::vector<RouterDB::connectNode>::iterator cit = nit->connected.begin(); cit != nit->connected.end(); ++cit) {
      if (cit->type == RouterDB::BLOCK) {
        int iter = cit->iter;
        int iter2 = cit->iter2;
        for (std::vector<RouterDB::contact>::iterator pit = Blocks.at(iter2).pins.at(iter).pinContacts.begin();
             pit != Blocks.at(iter2).pins.at(iter).pinContacts.end(); ++pit) {
          ConvertMetal2Points(pit->metal, pit->placedLL.x, pit->placedLL.y, pit->placedUR.x, pit->placedUR.y);
        }
      }
    }
...
void GlobalGrid::AdjustPlateEdgeCapacity() {
  ...
  // limits: capacity unbalanced between the edges from one tile, which has little intermetal, and another tile, which has a lot of intermetal. In this case,
  // the capacity should be keep along with the smaller capacity;
    ...
    for (int i = this->Start_index.at(k); i <= this->End_index.at(k); ++i) {
      ...
      for (unsigned int j = 0; j < this->tiles_total.at(i).metal.size(); ++j) {
        ...
        if (this->drc_info.Metal_info.at(mIdx).direct == 0) {  // vertical
          ...
          capR = 0;
          for (std::set<RouterDB::point, RouterDB::pointYXComp>::iterator ii = itlow; ii != itup; ++ii) {
            ++capR;
          }
          if (!this->tiles_total.at(i).south.empty()) {
            ...
            if (this->tiles_total.at(i).south[0].capacity < 0) {
              this->tiles_total.at(i).south[0].capacity = 0;
            }
          }
          ...
          if (!this->tiles_total.at(i).north.empty()) {
            ...
            if (this->tiles_total.at(i).north[0].capacity < 0) {
              this->tiles_total.at(i).north[0].capacity = 0;
            }
          }
        } else {  // horizontal
          // logger->debug("horizontal");
          ...
          capR = 0;
          for (std::set<RouterDB::point, RouterDB::pointXYComp>::iterator ii = itlow; ii != itup; ++ii) {
            ++capR;
          }
          if (!this->tiles_total.at(i).west.empty()) {
            // logger->debug("west cap {0}", capR);
            this->tiles_total.at(i).west[0].capacity -= capR * scale_number;
            if (this->tiles_total.at(i).west[0].capacity < 0) {
              this->tiles_total.at(i).west[0].capacity = 0;
            }
          }
          ...
          if (!this->tiles_total.at(i).east.empty()) {
            ...
            if (this->tiles_total.at(i).east[0].capacity < 0) {
              this->tiles_total.at(i).east[0].capacity = 0;
            }
          }
        }
      }
        ...
        if (viaNo == this->drc_info.Via_model.at(pit->model_index).ViaIdx) {
          ...
          if (mit != this->XYmap.at(k).end()) {
            if (!this->tiles_total.at(mit->second).up.empty()) {
              ...
              if (this->tiles_total.at(mit->second).up[0].capacity < 0) {
                this->tiles_total.at(mit->second).up[0].capacity = 0;
              }
            }
          } else {
          ...
          if (mit != this->XYmap.at(k + 1).end()) {
            if (!this->tiles_total.at(mit->second).down.empty()) {
              ...
              if (this->tiles_total.at(mit->second).down[0].capacity < 0) {
                this->tiles_total.at(mit->second).down[0].capacity = 0;
              }
            }
          } else {
            // logger->debug("GlobalGrid-Warning: cannot find tiles to adjust vertical edge cap");
    ...
    for (std::vector<RouterDB::Net>::iterator nit = Nets.begin(); nit != Nets.end(); ++nit) {
      ...
      for (std::vector<RouterDB::connectNode>::iterator cit = nit->connected.begin(); cit != nit->connected.end(); ++cit) {
        if (cit->type == RouterDB::BLOCK) {
          int iter = cit->iter;
          int iter2 = cit->iter2;
          for (std::vector<RouterDB::Via>::iterator pit = Blocks.at(iter2).pins.at(iter).pinVias.begin(); pit != Blocks.at(iter2).pins.at(iter).pinVias.end();
               ++pit) {
            if (viaNo == this->drc_info.Via_model.at(pit->model_index).ViaIdx) {
              int LLx = floor(double(pit->position.x) / this->x_unit);
              int URx = ceil(double(pit->position.x) / this->x_unit);
...
void GlobalGrid::ConverNetTerminal(std::set<int>& sSet, std::vector<int>& sVec, int metalIdx, int x, int y) {
  ...
  if (x == this->UR.x and x % this->x_unit == 0) {
    tmpp.x = x - this->x_unit / 2;
  } else if (x_cc + this->x_unit > this->UR.x) {
    tmpp.x = x_cc + (this->UR.x - x_cc) / 2;
  } else {
    tmpp.x = x_cc + this->x_unit / 2;
  }

  if (y == this->UR.y and y % this->y_unit == 0) {
    tmpp.y = y - this->y_unit / 2;
  } else if (y_cc + this->y_unit > this->UR.y) {
    tmpp.y = y_cc + (this->UR.y - y_cc) / 2;
  } else {
    tmpp.y = y_cc + this->y_unit / 2;
  }
  ...
  for (std::vector<RouterDB::Net>::iterator nit = Nets.begin(); nit != Nets.end(); ++nit) {
    ...
    for (int i = 0; i < cNO; ++i) {
      ...
      if (nit->connected.at(i).type == RouterDB::BLOCK) {  // block pin
        for (std::vector<RouterDB::contact>::iterator cit = Blocks.at(iter2).pins.at(iter).pinContacts.begin();
             cit != Blocks.at(iter2).pins.at(iter).pinContacts.end(); ++cit) {
