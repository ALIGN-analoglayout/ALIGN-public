...
int A_star::Manhattan_distan_via(int sindex, Grid &grid) {
  int max_dis = 0;
  for (int i = 0; i < (int)dest.size(); i++) {
  ...
  while (search_flag) {
    if (culmulated_length >= half_minL) {
      ...
      if (next_node < 0 || next_node >= int(grid.vertices_total.size())) {
        ...
        search_flag = false;
        feasible = false;
      } else if ((grid.vertices_total[next_node].x != grid.vertices_total[first_node_same_layer].x &&
                  grid.vertices_total[next_node].y != grid.vertices_total[first_node_same_layer].y) ||
                 grid.vertices_total[next_node].metal != grid.vertices_total[first_node_same_layer].metal) {
        search_flag = false;
        feasible = false;
      } else {
        culmulated_length = abs(grid.vertices_total[next_node].x - grid.vertices_total[first_node_same_layer].x) +
                            abs(grid.vertices_total[next_node].y - grid.vertices_total[first_node_same_layer].y);
  ...
  while (search_flag) {
    if (culmulated_length >= half_minL) {
      ...
      if (next_node < 0 || next_node >= int(grid.vertices_total.size())) {
        ...
        search_flag = false;
        feasible = false;
      } else if ((grid.vertices_total[next_node].x != grid.vertices_total[current_node].x &&
                  grid.vertices_total[next_node].y != grid.vertices_total[current_node].y) ||
                 grid.vertices_total[next_node].metal != grid.vertices_total[current_node].metal) {
        search_flag = false;
        feasible = false;
      } else {
        culmulated_length = abs(grid.vertices_total[next_node].x - grid.vertices_total[current_node].x) +
                            abs(grid.vertices_total[next_node].y - grid.vertices_total[current_node].y);
        dummy_node = next_node;
      }
    }
  }
...
bool A_star::find_succsive_parallel_node(Grid &grid, int current_node, int left, int right, int mode, std::vector<int> &nodes, std::set<int> &src_index,
  ...
  if (drc_info.Metal_info[grid.vertices_total[current_node].metal].direct == 0) {  // v
    ...
    if (mode == 0) {
      ...
      if (exist) exist = Check_Src_Dest(temp_nodes, src_index);
    } else {
...
bool A_star::Check_activa_via_active(Grid& grid, std::vector<int> &nodes){
  ...
  for(int i=0;i<nodes.size();i++){
     ...
     if(parent_metal == current_metal && !grid.vertices_total[nodes[i]].active){
       return false;
     }else if(parent_metal > current_metal && (!grid.vertices_total[nodes[i]].active || !grid.vertices_total[nodes[i]].via_active_up ||
!grid.vertices_total[parent].active || !grid.vertices_total[parent].via_active_down)){ return false; }else if(parent_metal < current_metal &&
(!grid.vertices_total[nodes[i]].active || !grid.vertices_total[nodes[i]].via_active_down || !grid.vertices_total[parent].active ||
!grid.vertices_total[parent].via_active_up)){ return false;
     }
  ...
  for (unsigned int i = 1; i < nodes.size(); i++) {
    ...
    if (parent_metal == current_metal && !grid.vertices_total[nodes[i]].active) {
      return false;
                                                ...
                                                !grid.vertices_total[parent].active || !grid.vertices_total[parent].via_active_down)) {
      return false;
    } else if (parent_metal < current_metal && (!grid.vertices_total[nodes[i]].active || !grid.vertices_total[nodes[i]].via_active_down ||
                                                !grid.vertices_total[parent].active || !grid.vertices_total[parent].via_active_up)) {
      return false;
    }
  }
...
bool A_star::Extention_check(Grid &grid, int current_node, std::set<int> &source_index) {
  ...
  if (parent >= 0 && parent < int(grid.vertices_total.size())) {
    if (grid.vertices_total[current_node].metal == grid.vertices_total[parent].metal) {
      ...
      int length = abs(grid.vertices_total[parent].x - grid.vertices_total[node_same_layer].x) +
                   abs(grid.vertices_total[parent].y - grid.vertices_total[node_same_layer].y);
      int minL = drc_info.Metal_info[metal].minL;
      // int minL = 0;
      int delta_length = length - minL;
...
std::vector<int> A_star::extend_manner_direction_check(std::vector<int> temp_path, Grid &grid) {
  ...
  for (int i = 0; i < path_pairs.size(); ++i) {
    ...
    if (i == 0 || i == path_pairs.size() - 1) {
      ...
      int length = abs(grid.vertices_total[path_pairs[i].first].x - grid.vertices_total[path_pairs[i].second].x) +
                   abs(grid.vertices_total[path_pairs[i].first].y - grid.vertices_total[path_pairs[i].second].y);
...
std::vector<std::vector<int>> A_star::A_star_algorithm_Sym(Grid &grid, int left_up, int right_down, vector<RouterDB::Metal> &sym_path) {
  ...
  while (!L_list.empty() && !found) {
    ...
    for (int i = 0; i < (int)candidate_node.size(); i++) {
      ...
      int M_dis_via = Manhattan_distan_via(candidate_node[i], grid);
      int tmp_metal = std::min(grid.vertices_total[candidate_node[i]].metal, grid.vertices_total[current_node].metal);
      double temp_cost = grid.vertices_total[current_node].Cost +
                      ...
                      abs(grid.vertices_total[current_node].y - grid.vertices_total[candidate_node[i]].y) *
                          drc_info.Metal_info[grid.vertices_total[current_node].metal].unit_R +
                      temp_candidate_cost[i];// in unit of 0.1 ohm
      ...
      if (temp_cost < grid.vertices_total[candidate_node[i]].Cost) {
        ...
        temp_pair.first = grid.vertices_total[candidate_node[i]].Cost + M_dis * drc_info.Metal_info[grid.vertices_total[current_node].metal].unit_R +
                          drc_info.Via_info[drc_info.Metal_info[tmp_metal].upper_via_index].R * M_dis_via + sym_factor * sym_cost;
        ...
        int dis = grid.vertices_total[candidate_node[i]].Cost + M_dis * drc_info.Metal_info[grid.vertices_total[current_node].metal].unit_R +
                  drc_info.Via_info[drc_info.Metal_info[tmp_metal].upper_via_index].R * M_dis_via + sym_factor * sym_cost;
...
std::vector<std::vector<int>> A_star::Trace_Back_Paths(Grid &grid, int current_node, int left, int right, std::set<int> &src_index, std::set<int> &dest_index) {
  ...
  for (unsigned int i = 0; i < nodes.size(); i++) {
    ...
    // logger->debug("trace back node nodes {0} {1} {2} metal {3} i {4} ", nodes[i], grid.vertices_total[nodes[i]].x, grid.vertices_total[nodes[i]].y,
                  // grid.vertices_total[nodes[i]].metal, i);
