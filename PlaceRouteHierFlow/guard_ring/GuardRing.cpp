...
void GuardRing::Pcell_info(const map<string, PnRDB::lefMacro> &lefData) {
  ...
  if (lefData.find("guard_ring") == lefData.end()) {
    ...
    pcell_metal_size.height = uc.macroPins[0].pinContacts[0].originBox.UR.y -
                              uc.macroPins[0].pinContacts[0].originBox.LL.y;  // calculate metal layer height of guard ring primitive cell
    offset.width = uc.macroPins[0]
                       .pinContacts[0]
                       .originBox.LL.x;  // calculate offset(lower left coordinate) between Metal layer and FEOL layer of guard ring primitive cell
    offset.height = uc.macroPins[0].pinContacts[0].originBox.LL.y;
    minimal_PC.width =
        uc.width - uc.macroPins[0]
                       .pinContacts[0]
                       .originBox.UR.x;  // calculate offset(upper right coordinate) between Metal layer and FEOL layer of guard ring primitive cell
...
GuardRing::GuardRing(PnRDB::hierNode &node, const map<string, PnRDB::lefMacro> &lefData, const PnRDB::Drc_info &drc_info, const string &fpath) {
  ...
  x_number = ceil(double((wcell_size.width + 2 * minimal.width + drc_info.Metal_info[0].grid_unit_x)) / double(pcell_metal_size.width)) +
             2;  // number of guard ring cells at the bottom or top, including corner
  ...
  southwest.x = wcell_ll.x - minimal.width - pcell_metal_size.width -
                ((((x_number - 2) * pcell_metal_size.width) - (wcell_size.width + 2 * minimal.width)) / 2);  // make sure left and right extra space the same
  southwest.y = wcell_ll.y - minimal.height - pcell_metal_size.height -
                (((y_number * pcell_metal_size.height) - (wcell_size.height + 2 * minimal.height)) / 2);  // make sure top and down extra space the same
  ...
  for (unsigned int i_print = 0; i_print < stored_point_ll.size(); i_print++) {
    logger->debug("lower left: {0}, {1} upper right: {2}, {3}", stored_point_ll[i_print].x, stored_point_ll[i_print].y, stored_point_ur[i_print].x,
                  stored_point_ur[i_print].y);
  }
...
void GuardRing::moveterminal(PnRDB::terminal &terminal, GuardRingDB::point offset) { moveveccontact(terminal.termContacts, offset); }
...
void GuardRing::gnuplot() {
  ...
  for (unsigned int i = 0; i < stored_point_ll.size(); i++) {
    fout << "\nset label \"" << i << "\" at " << (stored_point_ll[i].x + stored_point_ur[i].x) / 2 << "," << (stored_point_ll[i].y + stored_point_ur[i].y) / 2
         << " center " << std::endl;
  }
  // set label for Wrapped cell
  fout << "\nset label \""
       << "Wrapped Cell"
       << "\" at " << (wcell_ll.x + wcell_ur.x) / 2 << "," << (wcell_ll.y + wcell_ur.y) / 2 << " center " << std::endl;

  // plot blocks
  for (unsigned int i = 0; i < stored_point_ll.size(); i++) {
    fout << "\nset object \"" << i + 1 << "\" rectangle from " << stored_point_ll[i].x << "," << stored_point_ll[i].y << " to " << stored_point_ur[i].x << ","
         << stored_point_ur[i].y << " back" << std::endl;
  }
  fout << "\nset object \"" << stored_point_ll.size() + 1 << "\" rectangle from " << wcell_ll.x << "," << wcell_ll.y << " to " << wcell_ur.x << ","
       << wcell_ur.y << " back" << std::endl;
  fout << "plot "
       << "[" << wcell_ll.x - 4 * pcell_size.width << ":" << wcell_ur.x + 4 * pcell_size.width << "] "
       << "[" << wcell_ll.y - 4 * pcell_size.height << ":" << wcell_ur.y + 4 * pcell_size.height << "] " << wcell_ur.y + 4 * pcell_size.height + 1 << " title "
       << "\"#GuardRing Pcells= " << stored_point_ll.size() << "\"";
