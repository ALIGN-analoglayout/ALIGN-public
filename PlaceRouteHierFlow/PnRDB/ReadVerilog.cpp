...
bool PnRdatabase::ReadVerilog(const string& fpath, const string& vname, const string& topcell) {
  ...
  try {
      ...
      logger->error("Failed to close Verilog file {0}",verilogfile);
      return false;
  }
...
void PnRdatabase::semantic( const string& fpath, const string& topcell, PnRDB::hierNode& Supply_node)
{
  ...
  for(unsigned int i=0;i<hierTree.size();i++){
    ...
    if(DRC_info.Metal_info.size() < 2) {
      logger->error("Too few metal layers");
    }
    ...
    if (ReadConstraint_Json(curr_node, fpath, "const.json")) {
      ...
      logger->warn("Failed to read constraint file of module {0}", curr_node.name);
    }
	...
	if(hierTree[i].Nets[k].name == temp_LinearConst.net_name){
	  ...
	  for(int h=0;h<hierTree[i].Nets[k].connected.size();h++){
	    ...
	    for(int l=0;l<temp_LinearConst.pins.size();l++){
	      ...
	      if(hierTree[i].Nets[k].connected[h].type == PnRDB::Block and hierTree[i].Nets[k].connected[h].iter2 == temp_LinearConst.pins[l].first and hierTree[i].Nets[k].connected[h].iter == temp_LinearConst.pins[l].second){
		logger->debug("LinearConst alpha {0}",temp_LinearConst.alpha[l]);
		hierTree[i].Nets[k].connected[h].alpha = temp_LinearConst.alpha[l];
	      }else if(hierTree[i].Nets[k].connected[h].type == PnRDB::Terminal and temp_LinearConst.pins[l].first==-1 and hierTree[i].Nets[k].connected[h].iter == temp_LinearConst.pins[l].second){
		hierTree[i].Nets[k].connected[h].alpha = temp_LinearConst.alpha[l];
		logger->debug("LinearConst alpha {0}",temp_LinearConst.alpha[l]);
	      }
	    }
