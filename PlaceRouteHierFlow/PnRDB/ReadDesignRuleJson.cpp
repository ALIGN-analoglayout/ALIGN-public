	...
	json jedb = json::parse (jsonFile);
        ...
        for(json::iterator lit = layerAry.begin(); lit != layerAry.end(); ++lit) {
          ...
          if(lname.front()=='M') {
            ...
            logger->info("Reading Json PDK on GdsLayerNo");
            ...
            int lpitch=-1;
            logger->info("Reading Json PDK on Pitch");
            ...
            int lwidth=-1;
            logger->info("Reading Json PDK on Width");
            ...
            logger->info("Reading Json PDK on MinL");
            int lminL=layer["MinL"];
            //int lmaxL=layer["MaxL"];
            logger->info("Reading Json PDK on EndToEnd");
            int le2e=layer["EndToEnd"];

            double unit_C = 0;
            double unit_CC = 0;
            double unit_R = 0;
            logger->info("Reading Json PDK on Units, C, CC, R");
            #ifdef FinFET_MOCK_PDK
