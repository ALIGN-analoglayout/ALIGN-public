...
bool ConstGraph::RemoveEdgeforVertex(int current, int next, vector<Vertex>& graph, bool isBackward) {
  ...
  for (std::vector<Edge>::iterator it = graph.at(current).Edges.begin(); it != graph.at(current).Edges.end(); ++it, ++iN) {
    if (it->isBackward == isBackward && it->next == next) {
      mark = true;
      break;
    }
  }
  if (mark) {
    graph.at(current).Edges.erase(graph.at(current).Edges.begin() + iN);
  }
  return mark;
}
...
void ConstGraph::DeleteSetNode(design& caseNL, Aplace& caseAP, std::set<dataNode, dataNodeComp>& setD, dataNode& Dnode, std::vector<int>& cand, int mode,
                               int bias_mode, int bias_graph) {
  ...
  for (lit = setIt;; --lit) {
    ...
    if (mode == 1) {  // sort in y order, check overlap along x direction
      ...
      if (j != this->sourceNode) {
        ...
        if (std::min(xi + caseNL.GetBlockWidth(i, caseAP.GetBlockOrient(i), caseAP.GetSelectedInstance(i)) - xj,
                     xj + caseNL.GetBlockWidth(j, caseAP.GetBlockOrient(j), caseAP.GetSelectedInstance(j)) - xi) > 0) {
          mark = true;
          break;
        }
      } else {
      ...
      if (j != this->sourceNode) {
        ...
        if (std::min(yi + caseNL.GetBlockHeight(i, caseAP.GetBlockOrient(i), caseAP.GetSelectedInstance(i)) - yj,
                     yj + caseNL.GetBlockHeight(j, caseAP.GetBlockOrient(j), caseAP.GetSelectedInstance(j)) - yi) > 0) {
          mark = true;
          break;
        }
      } else {
...
void ConstGraph::DeleteSetNodeConstraint(design& caseNL, Aplace& caseAP, std::set<dataNode, dataNodeComp>& setD, dataNode& Dnode, std::vector<int>& cand,
                                         int mode, int bias_mode, int bias_graph) {
    ...
    if (i < caseNL.GetSizeofBlocks()) {  // current node is block
      if (mode == 1) {
        ...
        if (j != this->sourceNode) {
          ...
          if (j < caseNL.GetSizeofBlocks()) {
            ...
            if (xj >= xi && xj <= xi + caseNL.GetBlockWidth(i, caseAP.GetBlockOrient(i), caseAP.GetSelectedInstance(i))) {
              mark = true;
              break;
            }
          }
        ...
        if (j != this->sourceNode) {
          ...
          if (j < caseNL.GetSizeofBlocks()) {
            ...
            if (yj >= yi && yj <= yi + caseNL.GetBlockHeight(i, caseAP.GetBlockOrient(i), caseAP.GetSelectedInstance(i))) {
              mark = true;
              break;
            }
          }
      ...
      if (mode == 1) {
        ...
        if (j != this->sourceNode) {
          ...
          if (j < caseNL.GetSizeofBlocks()) {
            xj = caseAP.GetBlockCenter(j).x - caseNL.GetBlockWidth(j, caseAP.GetBlockOrient(j), caseAP.GetSelectedInstance(j)) / 2;
            if (xi >= xj && xi <= xj + caseNL.GetBlockWidth(j, caseAP.GetBlockOrient(j), caseAP.GetSelectedInstance(j))) {
              mark = true;
              break;
            }
          } else {
        ...
        if (j != this->sourceNode) {
          ...
          if (j < caseNL.GetSizeofBlocks()) {
            yj = caseAP.GetBlockCenter(j).y - caseNL.GetBlockHeight(j, caseAP.GetBlockOrient(j), caseAP.GetSelectedInstance(j)) / 2;
            if (yi >= yj && yi <= yj + caseNL.GetBlockHeight(j, caseAP.GetBlockOrient(j), caseAP.GetSelectedInstance(j))) {
              mark = true;
              break;
            }
          } else {
...
void ConstGraph::InsetSetNodeConstraint(design& caseNL, Aplace& caseAP, std::set<dataNode, dataNodeComp>& setD, dataNode& Dnode, std::vector<int>& cand,
                                        int mode) {
...
ConstGraph::ConstGraph(design& caseNL, SeqPair& caseSP, int mode) {
  ...
  */
  ...
  for (int i = 0; i < (int)HGraph.size(); i++) {
    ...
    candidate.clear();
    candidate = caseSP.GetRightBlock(i);
    ...
    candidate.clear();
    candidate = caseSP.GetLeftBlock(i);
    if (candidate.empty()) {
      // tmpE.weight=0;
      // tmpE.next=i;
  ...
  for (int i = 0; i < (int)VGraph.size(); i++) {
    ...
    candidate.clear();
    candidate = caseSP.GetAboveBlock(i);
    for (vector<int>::iterator it = candidate.begin(); it != candidate.end(); ++it) {
    ...
    candidate.clear();
    candidate = caseSP.GetBelowBlock(i);
    if (candidate.empty()) {
      // tmpE.weight=0;
      // tmpE.next=i;
...
void ConstGraph::PrintConstGraph() {
  ...
  for (int i = 0; i < (int)HGraph.size(); i++) {
    ...
    logger->debug("isVirtual- {0} position {1} << {2} backpost {3} << {4}", HGraph.at(i).isVirtual, HGraph.at(i).position, HGraph.at(i).precedent,
                  HGraph.at(i).backpost, HGraph.at(i).backprec);
    for (int j = 0; j < (int)HGraph.at(i).Edges.size(); j++) {
      logger->debug("Edge to {0} weight {1} isbackward {2}", HGraph.at(i).Edges.at(j).next, HGraph.at(i).Edges.at(j).weight,
                    HGraph.at(i).Edges.at(j).isBackward);
    }
  }
  ...
  for (int i = 0; i < (int)VGraph.size(); i++) {
    ...
    logger->debug("isVirtual- {0} position {1} << {2} backpost {3} << {4}", VGraph.at(i).isVirtual, VGraph.at(i).position, VGraph.at(i).precedent,
                  VGraph.at(i).backpost, VGraph.at(i).backprec);
    for (int j = 0; j < (int)VGraph.at(i).Edges.size(); j++) {
      logger->debug("Edge to {0} weight {1} isbackward {2}", VGraph.at(i).Edges.at(j).next, VGraph.at(i).Edges.at(j).weight,
                    VGraph.at(i).Edges.at(j).isBackward);
    }
  }
}
...
bool ConstGraph::AddEdgeforVertex(int current, int next, int weight, vector<Vertex>& graph) {
  ...
  tmpE.next = next;
  tmpE.weight = weight;
  if (CheckForwardPath(next, current, graph)) {
    tmpE.isBackward = true;
    // if(CheckRelatedForwardEdge(current, next, graph)) {tmpE.isBackward=true;
  } else {
    tmpE.isBackward = false;
  }
  bool mark = false;
  if (current >= 0 && current < (int)graph.size() && next >= 0 && next < (int)graph.size()) {
...
void ConstGraph::topologicalSortUtil(int v, bool visited[], stack<int>& Stack, vector<Vertex>& graph, bool backward) {
  ...
  for (vector<Edge>::iterator it = graph.at(v).Edges.begin(); it != graph.at(v).Edges.end(); ++it) {
    if (!backward) {  // forward sorting
      if (!it->isBackward && !visited[it->next]) {
        topologicalSortUtil(it->next, visited, Stack, graph, backward);
      }
...
void ConstGraph::CalculateLongestPath(int s, vector<Vertex>& graph, bool backward) {
  ...
  for (unsigned int i = 0; i < graph.size(); ++i)
    if (!visited[i]) topologicalSortUtil(i, visited, Stack, graph, backward);
...
int ConstGraph::GetX(int i) { return this->HGraph.at(i).position; }

int ConstGraph::GetY(int i) { return this->VGraph.at(i).position; }
...
double ConstGraph::CalculateWireLengthAP(design& caseNL, Aplace& caseAP) {
  ...
  for (int i = 0; i < (int)caseNL.GetSizeofTerminals(); ++i) {
    ...
    // pos.clear();
    if ((caseNL.Nets.at(netIdx).priority).compare("min") == 0) {
      alpha = 4;
    } else if ((caseNL.Nets.at(netIdx).priority).compare("mid") == 0) {
      alpha = 2;
    } else {
      alpha = 1;
    }
    ...
    if (sbIdx != -1) {                       // in symmetry group
      ...
      if (cp == i) {  // self-symmetric
        ...
        if (solved_terminals.find(cp) != solved_terminals.end()) {
          logger->debug("Placer-Error: terminal {0} and {1} are not solved simultaneously!", i, cp);
          continue;
        }
        ...
        int alpha2;
        if ((caseNL.Nets.at(netIdx2).priority).compare("min") == 0) {
          alpha2 = 4;
        } else if ((caseNL.Nets.at(netIdx2).priority).compare("mid") == 0) {
          alpha2 = 2;
        } else {
          alpha2 = 1;
        }
      ...
      for (unsigned int j = 0; j < caseNL.Port_Location.size(); ++j) {
        if (caseNL.Port_Location.at(j).tid == i) {
          tar = j;
          break;
        }
      }
  ...
  for (unsigned int i = 0; i < caseNL.ML_Constraints.size(); i++) {
    ...
    for (unsigned int j = 0; j < caseNL.ML_Constraints[i].Multi_linearConst.size(); j++) {
      for (unsigned int k = 0; k < caseNL.ML_Constraints[i].Multi_linearConst[j].pins.size(); k++) {
        int index_i = 0;
        int index_j = 0;
        // std::cout<<"ML Linear "<<caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].first<<"
        // "<<caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].second<<std::endl;
        for (unsigned int m = 0; m < caseNL.Nets.size(); m++) {
          for (unsigned int n = 0; n < caseNL.Nets[m].connected.size(); n++) {
            // std::cout<<"searching" <<m<<" "<<n<<" "<<caseNL.Nets[m].connected[n].iter << " "<< caseNL.Nets[m].connected[n].iter2 << " " <<
            // caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].first << " " << caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].second<<std::endl;

            if (caseNL.Nets[m].connected[n].iter2 == caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].first &&
                caseNL.Nets[m].connected[n].iter == caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].second) {
              // std::cout<<" found " <<caseNL.Nets[m].connected[n].iter2 << " "<< caseNL.Nets[m].connected[n].iter << " " <<
              // caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].first << " " << caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].second<<std::endl;
        ...
        // std::cout<< caseNL.Nets[index_i].connected[index_j].iter2 << " "<< caseNL.Nets[index_i].connected[index_j].iter << " " <<
        // caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].first << " " << caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].second<<std::endl;
        // std::cout<<"MLLinearConst Cost: alpha "<<caseNL.ML_Constraints[i].Multi_linearConst[j].alpha[k]<<" upperbound
        // "<<caseNL.ML_Constraints[i].upperBound<<" block idex "<<caseNL.Nets[index_i].connected[index_j].iter2<<" pin idx
        // "<<caseNL.Nets[index_i].connected[index_j].iter<<" index_i "<<index_i<<" index_j "<<index_j<<" dist "<<feature_value[index_i][index_j]<<" alpha*dist
        // "<<caseNL.ML_Constraints[i].Multi_linearConst[j].alpha[k]*feature_value[index_i][index_j]<<" pin_name "<<feature_name[index_i][index_j]<<std::endl;
...
void ConstGraph::ExtractLength(design& caseNL, SeqPair& caseSP, std::vector<std::vector<double> >& feature_value,
                               std::vector<std::vector<std::string> >& feature_name) {
  vector<placerDB::point> pos;
  placerDB::point p, bp;
  vector<placerDB::point> pos_pin;
...
double ConstGraph::CalculateCost(design& caseNL, SeqPair& caseSP) {
  ...
  // cout<<"Penalt: "<<  (CalculatePenalty(this->HGraph)+CalculatePenalty(this->VGraph))*GAMAR<<" vs "<<
  // (CalculatePenalty(this->HGraph)+CalculatePenalty(this->VGraph)) << endl; cout<<"WL: "<<CalculateWireLength(caseNL, caseSP)*LAMBDA<<" vs
  // "<<CalculateWireLength(caseNL, caseSP)<<endl; cout<<"Area: "<<CalculateArea()<<endl; cout<<"MAtch: "<<CalculateMatchCost(caseNL, caseSP)*BETA<<" vs "<<
  // CalculateMatchCost(caseNL, caseSP) <<endl; cout<<"Ratio: "<<CalculateRatio()*SIGMA<<" vs "<<CalculateRatio() <<endl; cout<<"DeadArea:
  // "<<CalculateDeadArea(caseNL, caseSP)*PHI<<" vs "<<CalculateDeadArea(caseNL, caseSP)<<endl;
  return cost;
}
...
bool ConstGraph::ConstraintVerticalOrder(int below, int above) { return AddEdgeforVertex(below, above, VGraph.at(below).weight, VGraph); }

bool ConstGraph::ConstraintHorizontalOrder(int left, int right) { return AddEdgeforVertex(left, right, HGraph.at(left).weight, HGraph); }
...
vector<int> ConstGraph::GenerateSlack(vector<int>& x) {
  ...
  if (!x.empty()) {
    ...
    for (vector<int>::iterator it = x.begin(); it != x.end(); ++it) newx.push_back(MAX - (*it));
  }
  return newx;
}
  ...
  if (axis == placerDB::V) {
    ...
    for (int j = 0; j < (int)sympair.size(); j++) {
      ...
      if (HGraph.at(sympair.at(j).second).position != NINF) {
        ...
        } else if (sympair.at(j).second == sinkNode) {
          magrin = HGraph.at(sympair.at(j).second).position / 2;
        }
        if (magrin > tmpx) {
          tmpx = magrin;
        }
      ...
      if (HGraph.at(sympair.at(j).second).backpost != NINF) {
        ...
        if (sympair.at(j).second < (int)caseNL.GetSizeofBlocks()) {
          ...
          magrin = HGraph.at(sympair.at(j).second).backpost / 2;
        }
        if (magrin > tmpx) {
          tmpx = magrin;
        }
      ...
      for (vector<pair<int, placerDB::Smark> >::iterator zit = caseNL.SBlocks.at(i).selfsym.begin(); zit != caseNL.SBlocks.at(i).selfsym.end(); ++zit) {
        ...
        if (HGraph.at(zit->first).position != NINF) {
          ...
          if (zit->first < (int)caseNL.GetSizeofBlocks()) {
            magrin += HGraph.at(zit->first).weight / 2;
          }
          if (magrin > tmpx) {
            tmpx = magrin;
          }
        ...
        if (HGraph.at(sympair.at(j).second).position != NINF) {
          ...
          if (sympair.at(j).second < (int)caseNL.GetSizeofBlocks()) {
            magrin += HGraph.at(sympair.at(j).second).weight;
          }
          if (magrin > tmpx) {
            tmpx = magrin;
          }
    ...
    for (int j = 0; j < (int)sympair.size(); j++) {
      ...
      if (VGraph.at(sympair.at(j).second).position != NINF) {
        ...
        if (sympair.at(j).second < (int)caseNL.GetSizeofBlocks()) {
          ...
          magrin = VGraph.at(sympair.at(j).second).position / 2;
        }
        if (magrin > tmpx) {
          tmpx = magrin;
        }
      ...
      if (VGraph.at(sympair.at(j).second).backpost != NINF) {
        ...
        if (sympair.at(j).second < (int)caseNL.GetSizeofBlocks()) {
          ...
          magrin = VGraph.at(sympair.at(j).second).backpost / 2;
        }
        if (magrin > tmpx) {
          tmpx = magrin;
        }
      ...
      for (vector<pair<int, placerDB::Smark> >::iterator zit = caseNL.SBlocks.at(i).selfsym.begin(); zit != caseNL.SBlocks.at(i).selfsym.end(); ++zit) {
        ...
        if (VGraph.at(zit->first).position != NINF) {
          ...
          if (zit->first < (int)caseNL.GetSizeofBlocks()) {
            magrin += VGraph.at(zit->first).weight / 2;
          }
          if (magrin > tmpx) {
            tmpx = magrin;
          }
        ...
        if (VGraph.at(sympair.at(j).second).position != NINF) {
          ...
          if (sympair.at(j).second < (int)caseNL.GetSizeofBlocks()) {
            magrin += VGraph.at(sympair.at(j).second).weight;
          }
          if (magrin > tmpx) {
            tmpx = magrin;
          }
...
bool ConstGraph::SymmetryConstraintCoreAxisCenter(design& caseNL, placerDB::Smark axis, int i) {
  ...
  for (int j = 0; j < (int)sympair.size(); j++) Lslack.at(j) = -1 * NINF;
  // 5. Update Lslack(x) according to U1,U2
  for (int j = 0; j < (int)sympair.size(); j++) UpdateLslackElement(caseNL, sympair, caseNL.SBlocks.at(i).selfsym, Lslack, xL, j, axis);
  if (CheckIfLslackViolation(Lslack)) {
    return false;
  ...
  while ((vio = CheckIfL2Violation(caseNL, sympair, xL, axis)).first != -1) {
    ...
    if (xL.at(vio.first) + Lslack.at(vio.first) < newxL) {
      newxL = xL.at(vio.first) + Lslack.at(vio.first);
    }
    ...
    if (CheckIfSingleL2Violation(caseNL, sympair, xL, vio.first, vio.second, axis)) {
      ...
      if (xL.at(vio.second) + Lslack.at(vio.second) < newxL) {
        newxL = xL.at(vio.second) + Lslack.at(vio.second);
      }
...
void ConstGraph::ReverseEdge(int current, int next, vector<Vertex>& graph) {
  for (int i = 0; i < (int)graph.at(current).Edges.size(); i++) {
    if (graph.at(current).Edges.at(i).next == next) {
      ...
      if (graph.at(current).Edges.at(i).isBackward) {
        graph.at(current).Edges.at(i).isBackward = false;
      } else {
        graph.at(current).Edges.at(i).isBackward = true;
      }
      break;
    }
  }
...
void ConstGraph::OtherGeometricConstraintCore(design& caseNL) {
  ...
  //                 AddEdgeforVertex(caseNL.Abument_blocks[i].blockid1, caseNL.Abument_blocks[i].blockid2,
  //                 caseNL.Blocks[caseNL.Abument_blocks[i].blockid1].width, HGraph);
  //                     }
  // else
  //{
  ...
  //                 AddEdgeforVertex(caseNL.Abument_blocks[i].blockid1, caseNL.Abument_blocks[i].blockid2,
  //                 caseNL.Blocks[caseNL.Abument_blocks[i].blockid1].height, VGraph);
  //}
  //          }
  //
  //  }
...
bool ConstGraph::ConstraintGraphAP(design& caseNL, Aplace& caseAP) {
  ...
  for (int i = 0; i < (int)caseNL.SBlocks.size(); i++) {
    ...
    if (!SymmetryConstraintCoreAxisCenter(caseNL, axis, i)) {
      return false;
    }
  }
...
bool ConstGraph::ConstraintGraph(design& caseNL, SeqPair& caseSP) {
  ...
  for (int i = 0; i < (int)caseNL.SBlocks.size(); i++) {
    axis = caseSP.GetSymmBlockAxis(i);
    if (!SymmetryConstraintCore(caseNL, axis, i)) {
      return false;
    }
  }
    ...
    for (int k = 0; k < (int)sympair.size(); k++) {
      ...
      if (axis == placerDB::V) {
        ...
        if (HGraph.at(sympair.at(j).second).position != NINF) {
          ...
          // if( sympair.at(j).second<(int)caseNL.GetSizeofBlocks() ) {magrin+=caseNL.GetBlockWidth(sympair.at(j).second,
          // caseSP.GetBlockOrient(sympair.at(j).second)); }
          if (sympair.at(k).second < (int)caseNL.GetSizeofBlocks()) {
            magrin -= HGraph.at(sympair.at(k).second).weight;
          }
          // if( sympair.at(k).second<(int)caseNL.GetSizeofBlocks() ) {magrin-=caseNL.GetBlockWidth(sympair.at(k).second,
          // caseSP.GetBlockOrient(sympair.at(k).second)); }
          if (magrin > xL.at(j)) {
            xL.at(j) = magrin;
          }
        }
        ...
        if (VGraph.at(sympair.at(j).second).position != NINF) {
          ...
          // if( sympair.at(j).second<(int)caseNL.GetSizeofBlocks() ) {magrin+=caseNL.GetBlockHeight(sympair.at(j).second,
          // caseSP.GetBlockOrient(sympair.at(j).second)); }
          if (sympair.at(k).second < (int)caseNL.GetSizeofBlocks()) {
            magrin -= VGraph.at(sympair.at(k).second).weight;
          }
          // if( sympair.at(k).second<(int)caseNL.GetSizeofBlocks() ) {magrin-=caseNL.GetBlockHeight(sympair.at(k).second,
          // caseSP.GetBlockOrient(sympair.at(k).second)); }
          if (magrin > xL.at(j)) {
            xL.at(j) = magrin;
          }
...
int ConstGraph::CalculateBminusXminusY(design& caseNL, vector<pair<int, int> >& sympair, vector<int>& xL, int j, int k, placerDB::Smark axis) {
  ...
  if (axis == placerDB::V) {  // vertical symmetry
    ...
    if (HGraph.at(sympair.at(k).second).position != NINF) {
      ...
      // if(sympair.at(k).second<(int)caseNL.GetSizeofBlocks()) {magrin+=caseNL.GetBlockWidth(sympair.at(k).second,
      // caseSP.GetBlockOrient(sympair.at(k).second));}
      if (xL.at(j) + xL.at(k) < magrin) {
        M = magrin - xL.at(k) - xL.at(j);
      }
    }
    // backward graph constraint
    CalculateLongestPath(sympair.at(j).first, this->HGraph, true);
...
pair<int, int> ConstGraph::CheckIfL2Violation(design& caseNL, vector<pair<int, int> >& sympair, vector<int>& xL, placerDB::Smark axis) {
  for (int j = 0; j < (int)sympair.size(); j++) {
    int vio;
    if ((vio = CheckIfL2ViolationUnit(caseNL, sympair, xL, j, axis)) != -1) {
      return make_pair(j, vio);
    }
  }
...
int ConstGraph::CheckIfL2ViolationUnit(design& caseNL, vector<pair<int, int> >& sympair, vector<int>& xL, int j, placerDB::Smark axis) {
  ...
  for (int k = 0; k < (int)sympair.size(); k++) {
    ...
    if (CheckIfSingleL2Violation(caseNL, sympair, xL, j, k, axis)) {
      return k;
    }
  }
  return -1;
  ...
  //      int magrin=HGraph.at(sympair.at(k).second).position;
  //      if(sympair.at(k).second<(int)caseNL.GetSizeofBlocks()) {magrin+=caseNL.GetBlockWidth(sympair.at(k).second,
  //      caseSP.GetBlockOrient(sympair.at(k).second));} if(xL.at(j)+xL.at(k)<magrin) {return k;}
  //    }
  //    // backward graph constraint
  //    CalculateLongestPath(sympair.at(j).first, this->HGraph, true);
  //    if(HGraph.at(sympair.at(k).second).backpost!=NINF) {
...
void ConstGraph::UpdateLslackElement(design& caseNL, vector<pair<int, int> >& sympair, vector<pair<int, placerDB::Smark> >& selfsym, vector<int>& Lslack,
                                     vector<int>& xL, int j, placerDB::Smark axis) {
  if (axis == placerDB::V) {
    // vertical symmetry axis
  ...
  for (int j = 0; j < caseNL.GetSizeofSBlocks(); j++) {
    ...
    if (i == -1) {  // no corresponding symmgroup in reduced design
      ...
      for (std::vector<pair<int, placerDB::Smark> >::iterator it = caseNL.SBlocks.at(j).selfsym.begin(); it != caseNL.SBlocks.at(j).selfsym.end(); ++it) {
        if (it->first >= caseNL.GetSizeofBlocks()) {
          continue;
        }
      ...
      for (std::vector<pair<int, int> >::iterator it = caseNL.SBlocks.at(j).sympair.begin(); it != caseNL.SBlocks.at(j).sympair.end(); ++it) {
        if (it->first >= caseNL.GetSizeofBlocks() || it->second >= caseNL.GetSizeofBlocks()) {
          continue;
        }
...
void ConstGraph::UpdateTerminalinHierNode(design& caseNL, PnRDB::hierNode& node, PnRDB::Drc_info& drcInfo) {
  ...
  if (1) {
    for (int i = 0; i < (int)caseNL.GetSizeofTerminals(); i++) {
      ...
      for (unsigned int j = 0; j < temp_pin.pinContacts.size(); j++) temp_pin.pinContacts[j].metal = drcInfo.Metal_info[0].name;
...
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).placedpos=caseNL.GetPlacedBlockInterMetalAbsPoint(i, ort,
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).originpos, LL);
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).UpperMetalRect.placedBox=caseNL.GetPlacedBlockInterMetalAbsBox(i, ort,
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).UpperMetalRect.originBox, LL);
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).LowerMetalRect.placedBox=caseNL.GetPlacedBlockInterMetalAbsBox(i, ort,
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).LowerMetalRect.originBox, LL);
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).ViaRect.placedBox=caseNL.GetPlacedBlockInterMetalAbsBox(i, ort,
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).ViaRect.originBox, LL);
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).UpperMetalRect.placedCenter=caseNL.GetPlacedBlockInterMetalAbsPoint(i, ort,
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).UpperMetalRect.originCenter, LL);
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).LowerMetalRect.placedCenter=caseNL.GetPlacedBlockInterMetalAbsPoint(i, ort,
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).LowerMetalRect.originCenter, LL);
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).ViaRect.placedCenter=caseNL.GetPlacedBlockInterMetalAbsPoint(i, ort,
//        node.Blocks.at(i).instance.blockPins.at(j).pinVias.at(k).ViaRect.originCenter, LL);
//      }
//    }
//  // [wbxu] Complete programing: to update internal metals
//    // update internal metals
//    for(int j=0;j<(int)node.Blocks.at(i).instance.interMetals.size();j++) {
//      node.Blocks.at(i).instance.interMetals.at(j).placedBox=caseNL.GetPlacedBlockInterMetalAbsBox(i, ort,
//      node.Blocks.at(i).instance.interMetals.at(j).originBox, LL);
//      node.Blocks.at(i).instance.interMetals.at(j).placedCenter=caseNL.GetPlacedBlockInterMetalAbsPoint(i, ort,
//      node.Blocks.at(i).instance.interMetals.at(j).originCenter, LL);
//    }
//    // update internal vias
//    for(int j=0;j<(int)node.Blocks.at(i).instance.interVias.size();j++) {
//      node.Blocks.at(i).instance.interVias.at(j).placedpos=caseNL.GetPlacedBlockInterMetalAbsPoint(i, ort,
//      node.Blocks.at(i).instance.interVias.at(j).originpos,LL);
//      node.Blocks.at(i).instance.interVias.at(j).UpperMetalRect.placedBox=caseNL.GetPlacedBlockInterMetalAbsBox(i, ort,
//      node.Blocks.at(i).instance.interVias.at(j).UpperMetalRect.originBox,LL);
//      node.Blocks.at(i).instance.interVias.at(j).LowerMetalRect.placedBox=caseNL.GetPlacedBlockInterMetalAbsBox(i, ort,
//      node.Blocks.at(i).instance.interVias.at(j).LowerMetalRect.originBox,LL);
//      node.Blocks.at(i).instance.interVias.at(j).ViaRect.placedBox=caseNL.GetPlacedBlockInterMetalAbsBox(i, ort,
//      node.Blocks.at(i).instance.interVias.at(j).ViaRect.originBox,LL);
//      node.Blocks.at(i).instance.interVias.at(j).UpperMetalRect.placedCenter=caseNL.GetPlacedBlockInterMetalAbsPoint(i, ort,
//      node.Blocks.at(i).instance.interVias.at(j).UpperMetalRect.originCenter,LL);
//      node.Blocks.at(i).instance.interVias.at(j).LowerMetalRect.placedCenter=caseNL.GetPlacedBlockInterMetalAbsPoint(i, ort,
//      node.Blocks.at(i).instance.interVias.at(j).LowerMetalRect.originCenter,LL);
//      node.Blocks.at(i).instance.interVias.at(j).ViaRect.placedCenter=caseNL.GetPlacedBlockInterMetalAbsPoint(i, ort,
//      node.Blocks.at(i).instance.interVias.at(j).ViaRect.originCenter,LL);
//    }
//  }
//  // [wbxu] Complete programing: to update terminal for top-level
//  if(node.isTop) {
...
void ConstGraph::updateTerminalCenter(design& caseNL, SeqPair& caseSP) {
  ...
  for (int i = 0; i < caseNL.GetSizeofTerminals(); ++i) {
    ...
    if (sbIdx != -1) {                       // in symmetry group
      ...
      int tar = -1;
      for (unsigned int j = 0; j < caseNL.Port_Location.size(); ++j) {
        if (caseNL.Port_Location.at(j).tid == (int)i) {
          tar = j;
          break;
        }
      }
...
void ConstGraph::WritePlacementAP(design& caseNL, Aplace& caseAP, string outfile) {
  ...
  fout << "DIE {" << HGraph.at(sourceNode).position << ", " << VGraph.at(sourceNode).position << "} {" << HGraph.at(sinkNode).position << ", "
       << VGraph.at(sinkNode).position << "}" << endl
       << endl;
  for (int i = 0; i < (int)caseNL.GetSizeofBlocks(); ++i) {
    int x, y;
    string ort;
    switch (caseAP.GetBlockOrient(i)) {
      case placerDB::N:
        x = HGraph.at(i).position;
        y = VGraph.at(i).position;
        ort = "N";
        break;
      case placerDB::S:
        x = HGraph.at(i).position + caseNL.GetBlockWidth(i, caseAP.GetBlockOrient(i), caseAP.GetSelectedInstance(i));
  ...
  for (vector<placerDB::net>::iterator ni = caseNL.Nets.begin(); ni != caseNL.Nets.end(); ++ni) {
    ...
    if (hasTerminal) {
      fout << caseNL.Terminals.at(tno).name << "\t" << caseNL.Terminals.at(tno).center.x << "\t" << caseNL.Terminals.at(tno).center.y << endl;
      // update the terminal information to the placerDB
      // caseNL.Terminals.at(tno).center.x = tp.x;
      // caseNL.Terminals.at(tno).center.y = tp.y;
    }
  ...
  for (int i = 0; i < (int)caseNL.GetSizeofBlocks(); ++i) {
    ...
    switch (caseSP.GetBlockOrient(i)) {
      case placerDB::N:
        ...
        break;
      case placerDB::S:
        x = HGraph.at(i).position + caseNL.GetBlockWidth(i, caseSP.GetBlockOrient(i), caseSP.GetBlockSelected(i));
...
void ConstGraph::PlotPlacement(design& caseNL, SeqPair& caseSP, string outfile, bool plot_pin, bool plot_terminal, bool plot_net) {
  ...
  */
  ...
  fout << "\nset title\" " << caseNL.name << " #Blocks= " << caseNL.GetSizeofBlocks() << ", #Terminals= " << caseNL.GetSizeofTerminals()
       << ", #Nets= " << caseNL.GetSizeofNets() << ", Area=" << CalculateArea() << ", HPWL= " << CalculateWireLength(caseNL, caseSP) << " \"" << endl;
  fout << "\nset nokey" << endl;
  ...
  if (plot_net) {
    for (vector<placerDB::net>::iterator ni = caseNL.Nets.begin(); ni != caseNL.Nets.end(); ++ni) {
      ...
      if (hasTerminal) {
        pins.push_back(caseNL.Terminals.at(tno).center);
      }
      fout << "\n#Net: " << ni->name << endl;
      if (pins.size() >= 2) {
...
void ConstGraph::PlotPlacementAP(design& caseNL, Aplace& caseAP, string outfile) {
  ...
  fout << "\nset title\" #Blocks= " << caseNL.GetSizeofBlocks() << ", #Terminals= " << caseNL.GetSizeofTerminals() << ", #Nets= " << caseNL.GetSizeofNets()
       << ", Area=" << CalculateArea() << " , HPWL=" << CalculateWireLengthAP(caseNL, caseAP) << " \"" << endl;
  fout << "\nset nokey" << endl;
  fout << "#   Uncomment these two lines starting with \"set\"" << endl;
  fout << "#   to save an EPS file for inclusion into a latex document" << endl;
  fout << "# set terminal postscript eps color solid 20" << endl;
