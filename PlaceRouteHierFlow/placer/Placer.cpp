...
std::map<double, std::pair<SeqPair, ILP_solver>> Placer::PlacementCoreAspectRatio_ILP(design& designData, SeqPair& curr_sp, ILP_solver& curr_sol, int mode,
  ...
  while ((curr_cost = curr_sol.GenerateValidSolution_select(designData, curr_sp, drcInfo)) < 0) {
  ...
  while (T > hyper.T_MIN) {
    ...
    while (i <= MAX_Iter) {
      ...
      double trial_cost = trial_sol.GenerateValidSolution_select(designData, trial_sp, drcInfo);
      if (trial_cost >= 0) {
