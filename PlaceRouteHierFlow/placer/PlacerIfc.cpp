...
PlacerIfc::PlacerIfc(PnRDB::hierNode& currentNode, int numLayout, string opath, int effort, PnRDB::Drc_info& drcInfo, const PlacerHyperparameters& hyper, bool select_in_ILP = false, bool bottom_up = true, int ilp_solver = 0) : _nodeVec( numLayout, currentNode) {
  ...
  if (hyper.use_analytical_placer) {
    ...
    Placer curr_plc(_nodeVec, opath, effort, drcInfo, hyper, select_in_ILP, bottom_up, ilp_solver);
