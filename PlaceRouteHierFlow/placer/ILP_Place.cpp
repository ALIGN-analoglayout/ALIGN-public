  ...
  for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) {
    ...
    for (unsigned int j = i + 1; j < mydesign.Blocks.size(); j++) {
      ...
      if (!alignhij && !alignvij) {
        ...
        sens.push_back('G');
        rhs.push_back(std::max(bias_Hgraph, mydesign.getSpread(i, j, true)));
        rowtype.push_back('o');
        ...
        sens.push_back('L');
        rhs.push_back(maxxdim - std::max(bias_Hgraph, mydesign.getSpread(i, j, true)));
        rowtype.push_back('o');
        ...
        sens.push_back('G');
        rhs.push_back(-maxdim + std::max(bias_Vgraph, mydesign.getSpread(i, j, false)));
        rowtype.push_back('o');
        ...
        sens.push_back('L');
        rhs.push_back(maxydim + maxdim - std::max(bias_Vgraph, mydesign.getSpread(i, j, false)));
        rowtype.push_back('o');
      } else if (alignhij) {
        buf_indx_map[std::make_pair(i, j)] = N_var++;
        if (collb.size() < N_var) {
        ...
        constrvalues[j * 6 + 5].push_back(-1);
        sens.push_back('G');
        rhs.push_back(std::max(bias_Vgraph, mydesign.getSpread(i, j, false)));
        rowtype.push_back('o');
        ...
        constrvalues[i * 6 + 5].push_back(1);
        sens.push_back('L');
        rhs.push_back(maxydim - std::max(bias_Vgraph, mydesign.getSpread(i, j, false)));
        rowtype.push_back('o');
      }
    }
  }
  ...
  for (const auto& clknetit : mydesign.clockNets) {
    ...
    for (auto yitr : {0, 1}) {
      ...
      { // driver
        ...
        int urx = std::get<0>(piit->second) + 2, ury = std::get<0>(piit->second) + 3;
        if (yitr == 0) {
          rowindofcol[block_id * 6].push_back(rhs.size());
          rowindofcol[llx].push_back(rhs.size());
          rowindofcol[urx].push_back(rhs.size());
          constrvalues[block_id * 6].push_back(-1);
          constrvalues[llx].push_back(-0.5);
          constrvalues[urx].push_back(-0.5);
          ...
          rowindofcol[ury].push_back(rhs.size());
          constrvalues[block_id * 6 + 1].push_back(-1);
          constrvalues[lly].push_back(-0.5);
          constrvalues[ury].push_back(-0.5);
          if (std::get<1>(piit->second) != INT_MIN) {
            rowindofcol[block_id * 6 + 3].push_back(rhs.size());
      ...
      for (auto& rcvr : clknetit.second) {
        ...
        if (yitr == 0) {
          ...
          constrvalues[llx].push_back(0.5/clknetit.second.size());
          constrvalues[urx].push_back(0.5/clknetit.second.size());
          if (std::get<1>(piit->second) != INT_MIN) {
            rowindofcol[block_id * 6 + 2].push_back(rhs.size());
          ...
          constrvalues[lly].push_back(0.5/clknetit.second.size());
          constrvalues[ury].push_back(0.5/clknetit.second.size());
          if (std::get<1>(piit->second) != INT_MIN) {
            rowindofcol[block_id * 6 + 3].push_back(rhs.size());
