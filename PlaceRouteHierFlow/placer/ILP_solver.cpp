...
ILP_solver::ILP_solver(design& mydesign, PnRDB::hierNode& node) {
  ...
  for (unsigned int i = 0; i < node.Blocks.size() - 1; i++) {
    for (unsigned int j = i + 1; j < node.Blocks.size(); j++) {
      if ((block_order[i][j] & 0x000e) && (block_order[i][j] & 0x0e00))
        logger->error("wrong constraint between block {0} and {1}", mydesign.Blocks[i][0].name, mydesign.Blocks[j][0].name);
  ...
  int a = 1;
  block_x_order_set.resize(node.Blocks.size());
  block_y_order_set.resize(node.Blocks.size());
  for (auto i : node.Ordering_Constraints) {
    if (i.second == placerDB::H) {
      for (unsigned int j = 0; j < i.first.size(); j++) {
        for (unsigned int k = 0; k < j; k++) {
          block_x_order_set[i.first[j]].first.insert(i.first[k]);
        }
        for (unsigned int k = j + 1; k < i.first.size(); k++) {
          block_x_order_set[i.first[j]].second.insert(i.first[k]);
        }
      }
    } else {
      for (unsigned int j = 0; j < i.first.size(); j++) {
        for (unsigned int k = 0; k < j; k++) {
          block_y_order_set[i.first[j]].second.insert(i.first[k]);
        }
        for (unsigned int k = j + 1; k < i.first.size(); k++) {
          block_y_order_set[i.first[j]].first.insert(i.first[k]);
        }
      }
    }
  }
...
double ILP_solver::GenerateValidSolution(design& mydesign, PnRDB::Drc_info& drcInfo, PnRDB::hierNode& node) {
  auto logger = spdlog::default_logger()->clone("placer.ILP_solver.GenerateValidSolution");
  vector<vector<int>> block_order_temp(block_order);
  vector<int> block_x_order, block_y_order;
  for (unsigned int i = 0; i < node.Blocks.size(); i++) {
    int min_id = -1, max_id = block_x_order.size();
    for (auto j : block_x_order_set[i].first) {
      auto id = find(block_x_order.begin(), block_x_order.end(), j);
      if (id != block_x_order.end()) min_id = max(min_id, int(id - block_x_order.begin()));
    }
    for (auto j : block_x_order_set[i].second) {
      auto id = find(block_x_order.begin(), block_x_order.end(), j);
      if (id != block_x_order.end()) max_id = min(max_id, int(id - block_x_order.begin()));
    }
    int size = max_id - min_id;
    int insert_pos = rand() % size + min_id + 1;
    block_x_order.insert(block_x_order.begin() + insert_pos, i);
  }
  for (unsigned int i = 0; i < node.Blocks.size(); i++) {
    int min_id = -1, max_id = block_y_order.size();
    for (auto j : block_y_order_set[i].first) {
      auto id = find(block_y_order.begin(), block_y_order.end(), j);
      if (id != block_y_order.end()) min_id = max(min_id, int(id - block_y_order.begin()));
    }
    for (auto j : block_y_order_set[i].second) {
      auto id = find(block_y_order.begin(), block_y_order.end(), j);
      if (id != block_y_order.end()) max_id = min(max_id, int(id - block_y_order.begin()));
    }
    int size = max_id - min_id;
    int insert_pos = rand() % size + min_id + 1;
    block_y_order.insert(block_y_order.begin() + insert_pos, i);
  }
  vector<pair<int, float>> block_x(node.Blocks.size()), block_y(node.Blocks.size());
  for (unsigned int i = 0; i < node.Blocks.size(); i++) {
    block_x[i].second = (1.0 * rand() / RAND_MAX);
    block_y[i].second = (1.0 * rand() / RAND_MAX);
  }
  sort(block_x.begin(), block_x.end(), [](pair<int, float> a, pair<int, float> b) { return a.second < b.second; });
  sort(block_y.begin(), block_y.end(), [](pair<int, float> a, pair<int, float> b) { return a.second < b.second; });
  for (unsigned int i = 0; i < node.Blocks.size(); i++) {
    block_x[i].first = block_x_order[i];
    block_y[i].first = block_y_order[i];
  }
  sort(block_x.begin(), block_x.end(), [](pair<int, float> a, pair<int, float> b) { return a.first < b.first; });
  sort(block_y.begin(), block_y.end(), [](pair<int, float> a, pair<int, float> b) { return a.first < b.first; });

  for (unsigned int i = 0; i < node.Blocks.size() - 1; i++) {
    for (unsigned int j = i + 1; j < node.Blocks.size(); j++) {
      if ((block_order_temp[i][j] & 0x000e) && (block_order_temp[i][j] & 0x0e00))
        logger->error("wrong constraint between block {0} and {1}", mydesign.Blocks[i][0].name, mydesign.Blocks[j][0].name);
      if ((block_order_temp[i][j] & 0x1111) == 0) {  // neither left right below above
        if (block_order_temp[i][j] & 0x00ff) {
          // align to left, x center, or right
          block_order_temp[i][j] &= 0x00ff;
          if (block_y[i].second < block_y[j].second)
            block_order_temp[i][j] |= 0x0100;
          else
            block_order_temp[i][j] |= 0x1000;
        } else if (block_order_temp[i][j] & 0xff00) {
          block_order_temp[i][j] &= 0xff00;
          if (block_x[i].second < block_x[j].second)
            block_order_temp[i][j] |= 0x0001;
          else
            block_order_temp[i][j] |= 0x0010;
        } else {
          int choice = rand() % 2;
          if (abs(block_x[i].second - block_x[j].second) < abs(block_y[i].second - block_y[j].second)) {
            block_order_temp[i][j] &= 0x00ff;
            if (block_y[i].second < block_y[j].second)
              block_order_temp[i][j] |= 0x0100;
            else
  ...
  for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) {
    for (unsigned int j = i + 1; j < mydesign.Blocks.size(); j++) {
      if (block_order_temp[i][j] & 0x0001) {
        // i is at the left of j
      ...
      } else if (block_order_temp[i][j] & 0x0008) {
      ...
      } else if (block_order_temp[i][j] & 0x0010) {
      ...
      if (block_order_temp[i][j] & 0x0100) {
        // i is at below j
        double sparserow[2] = {-1, 1};
      ...
      } else if (block_order_temp[i][j] & 0x0200) {
        // i and j align to LLy
        double sparserow[2] = {1, -1};
        int colno[2] = {i * 4 + 2, j * 4 + 2};
        if (!add_constraintex(lp, 2, sparserow, colno, EQ, 0)) logger->error("error");
      } else if (block_order_temp[i][j] & 0x0400) {
      ...
      } else if (block_order_temp[i][j] & 0x0800) {
      ...
      } else if (block_order_temp[i][j] & 0x1000) {
        // i is above j
        double sparserow[2] = {1, -1};
        int colno[2] = {i * 4 + 2, j * 4 + 2};
...
void ILP_solver::updateTerminalCenter(design& mydesign) {
...
void ILP_solver::UpdateHierNode(design& mydesign, PnRDB::hierNode& node, PnRDB::Drc_info& drcInfo) {
  node.width = UR.x;
  node.height = UR.y;
  for (unsigned int i = 0; i < mydesign.Blocks.size(); ++i) {
  ...
  for (unsigned int i = 0; i < mydesign.SNets.size(); ++i) {
    ...
    UpdateSymmetryNetInfo(mydesign, node, i, SBidx, axis_dir);
  }
}
...
void ILP_solver::UpdateSymmetryNetInfo(design& mydesign, PnRDB::hierNode& node, int i, int SBidx, placerDB::Smark axis_dir) {
