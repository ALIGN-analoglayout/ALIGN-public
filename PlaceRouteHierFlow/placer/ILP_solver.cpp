    ...
    for (const auto &connectedj : neti.connected) {
      if (connectedj.type == placerDB::Block) {
        ...
        if (mydesign.Blocks[connectedj.iter2][0].blockPins.size() > 0) {
          for (const auto &ci : mydesign.Blocks[connectedj.iter2][0]
...
bool ILP_solver::FrameSolveILP(const design &mydesign, const SeqPair &curr_sp,
                               const PnRDB::Drc_info &drcInfo, bool flushbl,
  ...
  for (int i = 0; i < mydesign.Nets.size(); ++i) {
    ...
    set_col_name(lp, ind++,
                 const_cast<char *>((mydesign.Nets[i].name + "_ll_x").c_str()));
    set_col_name(lp, ind++,
                 const_cast<char *>((mydesign.Nets[i].name + "_ll_y").c_str()));
    set_col_name(lp, ind++,
                 const_cast<char *>((mydesign.Nets[i].name + "_ur_x").c_str()));
    set_col_name(lp, ind,
                 const_cast<char *>((mydesign.Nets[i].name + "_ur_y").c_str()));
  }
  ...
  // set_add_rowmode(lp, FALSE);
  {
    ...
    for (unsigned int i = 0; i < mydesign.Nets.size(); i++) {
      ...
      for (unsigned int j = 0; j < mydesign.Nets[i].connected.size(); j++) {
        if (mydesign.Nets[i].connected[j].type == placerDB::Block) {
          int block_id = mydesign.Nets[i].connected[j].iter2,
              pin_id = mydesign.Nets[i].connected[j].iter;
          const auto &blk =
              mydesign.Blocks[block_id][curr_sp.selected[block_id]];
          ...
          {
            double sparserow[3] = {1, 1. * (blk.height - pin_lly - pin_ury),
                                   -1};
    ...
    if (write_cnt < 10) {
      write_lp(lp, const_cast<char*>((mydesign.name + "_ilp_" +
    std::to_string(write_cnt) + ".lp").c_str()));
      ++write_cnt;
    }*/
  }
...
bool ILP_solver::MoveBlocksUsingSlack(const std::vector<Block> &blockslocal,
                                      ...
                                      const SeqPair &curr_sp,
                                      const PnRDB::Drc_info &drcInfo) {
...
double ILP_solver::GenerateValidSolution(const design &mydesign,
                                         const SeqPair &curr_sp,
                                         const PnRDB::Drc_info &drcInfo) {
  auto logger = spdlog::default_logger()->clone(
      "placer.ILP_solver.GenerateValidSolution");
  ...
  for (const auto &neti : mydesign.Nets) {
    ...
    for (const auto &connectedj : neti.connected) {
      if (connectedj.type == placerDB::Block) {
        int iter2 = connectedj.iter2, iter = connectedj.iter;
        for (const auto &centerk :
             mydesign.Blocks[iter2][curr_sp.selected[iter2]]
                 .blockPins[iter]
                 .center) {
          // calculate contact center
          int pin_x = centerk.x;
  ...
  if (cost >= 0.) {
    logger->debug("ILP__HPWL_compare : HPWL_extend={0} HPWL_ILP={1}",
                  HPWL_extend, HPWL_ILP);
  }
...
double ILP_solver::CalculateCost(const design &mydesign,
                                 const SeqPair &curr_sp) {
  auto logger =
