...
double ILP_solver::CalculateCFCost(const design& mydesign, const SeqPair& curr_sp) const {
	double cost(0.);
...
double ILP_solver::CalculateRectangularIslandCost(const design& mydesign, const SeqPair& curr_sp) const
{
  double cost{0.};
  for (int i = 0; i < curr_sp.posPair.size(); ++i) {
    if (curr_sp.posPair[i] >= mydesign.Blocks.size()) continue;
    const auto& iname   = mydesign.Blocks[curr_sp.posPair[i]][0].name;
    const auto& iwidth  = mydesign.Blocks[curr_sp.posPair[i]][curr_sp.selected[curr_sp.posPair[i]]].width;
    const auto& iheight = mydesign.Blocks[curr_sp.posPair[i]][curr_sp.selected[curr_sp.posPair[i]]].height;
    bool isipmos{iname.find("PMOS") != std::string::npos};
    bool isinmos{iname.find("NMOS") != std::string::npos};
    for (int j = 0; j < curr_sp.posPair.size(); ++j) {
      if (i == j || curr_sp.posPair[j] >= mydesign.Blocks.size()) continue;
      const auto& jname   = mydesign.Blocks[curr_sp.posPair[j]][0].name;
      const auto& jwidth  = mydesign.Blocks[curr_sp.posPair[j]][curr_sp.selected[curr_sp.posPair[j]]].width;
      const auto& jheight = mydesign.Blocks[curr_sp.posPair[j]][curr_sp.selected[curr_sp.posPair[j]]].height;
      bool isjpmos{jname.find("PMOS") != std::string::npos};
      bool isjnmos{jname.find("NMOS") != std::string::npos};
      if ((isipmos && isjpmos) || (isinmos && isjnmos)) {
        cost += ((iwidth == jwidth || iheight == jheight) ? (2. * std::fabs(i - j)) : (1. * std::fabs(i - j)));
      }
    }
  }
...
double ILP_solver::CalculateCost(const design& mydesign, const SeqPair& curr_sp) {
  ...
  rcost = CalculateRectangularIslandCost(mydesign, curr_sp);
  if (rcost > 0) {
    cost += log(rcost) * hyper.LAMBDA;
  }
  ...
  if (cost > 0 && HPWL_extend_net_priority > 0 && cfcost > 0) {
    logger->debug("ILP calculate cost hpwl : {0} cfcost : {1} rect_cost : {2} logcosts : {3} {4} {5} {6}",
        HPWL_extend_net_priority, cfcost, rcost,
        log(HPWL_extend_net_priority) * hyper.LAMBDA,
