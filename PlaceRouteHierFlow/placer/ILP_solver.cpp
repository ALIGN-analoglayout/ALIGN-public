...
void ExtremeBlocksOfNet::FindExtremes(const placerDB::net& n, const int neti, const std::vector<ILP_solver::Block>& Blocks, const design& mydesign, const SeqPair& curr_sp)
{
  _ltExtreme[neti].clear(); _rtExtreme[neti].clear();
  _botExtreme[neti].clear(); _topExtreme[neti].clear();
  std::set<int> blocks;
  ...
  for (unsigned i = 0; i < blks.size(); ++i) {
    ...
    for (unsigned j = i+1; j < blks.size(); ++j) {
      const auto& blkj = blks[j];
      if (Blocks[blki].x + mydesign.Blocks[blki][curr_sp.selected[blki]].width <= Blocks[blkj].x) {
        rtOf[i] = 0;
        ltOf[j] = 0;
      } else if (Blocks[blkj].x + mydesign.Blocks[blkj][curr_sp.selected[blkj]].width <= Blocks[blki].x) {
        rtOf[j] = 0;
        ltOf[i] = 0;
      }
      if (Blocks[blki].y + mydesign.Blocks[blki][curr_sp.selected[blki]].height <= Blocks[blkj].y) {
        beOf[i] = 0;
        abOf[j] = 0;
      } else if (Blocks[blkj].y + mydesign.Blocks[blkj][curr_sp.selected[blkj]].height <= Blocks[blki].y) {
        beOf[j] = 0;
        abOf[i] = 0;
      }
    }
...
struct UniqRow {
  int _id1, _id2;
  int _d1, _ll, _ur;
  UniqRow(int id1, int id2, int d1, int ll, int ur) :
    _id1(id1), _id2(id2), _d1(d1), _ll(ll), _ur(ur) {}
...
bool ILP_solver::FrameSolveILPOrig(const design& mydesign, const SeqPair& curr_sp, const PnRDB::Drc_info& drcInfo, bool flushbl, const vector<placerDB::point>* prev) {
  ...
  ConstGraph const_graph;
  {
    ...
    unsigned int N_var = (mydesign.Blocks.size() * 2 + mydesign.Nets.size() * 4);
    // i*2+1:x
    // i*2+2:y
    lprec* area_lp = make_lp(0, N_var);
    set_verbose(area_lp, CRITICAL);
    put_logfunc(area_lp, &ILP_solver::lpsolve_logger, NULL);

    for (int i = 0; i < mydesign.Blocks.size(); i++) {
      int ind = i * 2 + 1;
      set_col_name(area_lp, ind++, const_cast<char*>((mydesign.Blocks[i][0].name + "_x").c_str()));
      set_col_name(area_lp, ind++, const_cast<char*>((mydesign.Blocks[i][0].name + "_y").c_str()));
    }
    for (int i = 0; i < mydesign.Nets.size(); ++i) {
      int ind = i * 4 + mydesign.Blocks.size() * 2 + 1;
      set_col_name(area_lp, ind++, const_cast<char*>((mydesign.Nets[i].name + "_ll_x").c_str()));
      set_col_name(area_lp, ind++, const_cast<char*>((mydesign.Nets[i].name + "_ll_y").c_str()));
      set_col_name(area_lp, ind++, const_cast<char*>((mydesign.Nets[i].name + "_ur_x").c_str()));
      set_col_name(area_lp, ind,   const_cast<char*>((mydesign.Nets[i].name + "_ur_y").c_str()));
    }

    std::vector<double> row(N_var + 1, 0);
    for (unsigned int i = 0; i < mydesign.Nets.size(); i++) {
      if (mydesign.Nets[i].connected.size() < 2) continue;
      int ind = int(mydesign.Blocks.size() * 2 + i * 4 + 1);
    ...
    for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) {
      ...
      row.at(curr_sp.negPair[i] * 2 + 2) += ((flushbl ? estimated_width : -estimated_width) / 2);
    }
    // estimate height
    ...
    for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) {
      if (curr_sp.negPair[i] >= mydesign.Blocks.size()) continue;
      row.at(curr_sp.negPair[i] * 2 + 1) += ((flushbl ? estimated_height : -estimated_height) / 2);
    }

    set_obj_fn(area_lp, row.data());

    set_add_rowmode(area_lp, TRUE);
      ...
      for (unsigned int j = i + 1; j < mydesign.Blocks.size(); j++) {
        ...
        if (i_pos_index < j_pos_index) {
          if (i_neg_index < j_neg_index) {
            // i is left of j
            double sparserow[2] = {1, -1};
            int colno[2] = {int(i) * 2 + 1, int(j) * 2 + 1};
            if (find(mydesign.Abut_Constraints.begin(), mydesign.Abut_Constraints.end(), make_pair(make_pair(int(i), int(j)), placerDB::H)) !=
              ...
              if (!add_constraintex(area_lp, 2, sparserow, colno, EQ, -mydesign.Blocks[i][curr_sp.selected[i]].width - 0)) logger->error("error");
            } else if (!add_constraintex(area_lp, 2, sparserow, colno, LE, -mydesign.Blocks[i][curr_sp.selected[i]].width - bias_Hgraph))
              logger->error("error");
          } else {
            // i is above j
            double sparserow[2] = {1, -1};
            int colno[2] = {int(i) * 2 + 2, int(j) * 2 + 2};
            if (find(mydesign.Abut_Constraints.begin(), mydesign.Abut_Constraints.end(), make_pair(make_pair(int(i), int(j)), placerDB::V)) !=
                mydesign.Abut_Constraints.end()) {
              if (!add_constraintex(area_lp, 2, sparserow, colno, EQ, mydesign.Blocks[j][curr_sp.selected[j]].height + 0)) logger->error("error");
            } else if (!add_constraintex(area_lp, 2, sparserow, colno, GE, mydesign.Blocks[j][curr_sp.selected[j]].height + bias_Vgraph))
              logger->error("error");
          }
        } else {
          if (i_neg_index < j_neg_index) {
            ...
            if (find(mydesign.Abut_Constraints.begin(), mydesign.Abut_Constraints.end(), make_pair(make_pair(int(j), int(i)), placerDB::V)) !=
                mydesign.Abut_Constraints.end()) {
              if (!add_constraintex(area_lp, 2, sparserow, colno, EQ, -mydesign.Blocks[i][curr_sp.selected[i]].height - 0)) logger->error("error");
            } else if (!add_constraintex(area_lp, 2, sparserow, colno, LE, -mydesign.Blocks[i][curr_sp.selected[i]].height - bias_Vgraph))
              logger->error("error");
          } else {
            // i is right of j
            double sparserow[2] = {1, -1};
            ...
            if (find(mydesign.Abut_Constraints.begin(), mydesign.Abut_Constraints.end(), make_pair(make_pair(int(j), int(i)), placerDB::H)) !=
                mydesign.Abut_Constraints.end()) {
              if (!add_constraintex(area_lp, 2, sparserow, colno, EQ, mydesign.Blocks[j][curr_sp.selected[j]].width + 0)) logger->error("error");
            } else if (!add_constraintex(area_lp, 2, sparserow, colno, GE, mydesign.Blocks[j][curr_sp.selected[j]].width + bias_Hgraph))
              logger->error("error");
          }
        }
      }
    ...
    if (flushbl) {
      for (const auto& id : curr_sp.negPair) {
        if (id < int(mydesign.Blocks.size())) {
          if (prev) {
            set_bounds(area_lp, (id * 2 + 1), (*prev)[id].x, get_infinite(area_lp));
            set_bounds(area_lp, (id * 2 + 2), (*prev)[id].y, get_infinite(area_lp));
          } else {
            // x>=0
            set_bounds(area_lp, (id * 2 + 1), 0, get_infinite(area_lp));
            // y>=0
            set_bounds(area_lp, (id * 2 + 2), 0, get_infinite(area_lp));
          }
        }
      }
    } else {
      ...
      for (const auto& id : curr_sp.negPair) {
        if (id < int(mydesign.Blocks.size())) {
          set_bounds(area_lp, (id * 2 + 1), -10*minx, -mydesign.Blocks[id][curr_sp.selected[id]].width);
          set_bounds(area_lp, (id * 2 + 2), -10*miny, -mydesign.Blocks[id][curr_sp.selected[id]].height);
        }
      }
      ...
      for (unsigned i = 0; i < mydesign.Nets.size(); ++i) {
        ...
        for (int j = 0; j < 4; ++j) {
          set_bounds(area_lp, ind+j, -get_infinite(area_lp), 0);
        }
      }
    }
    ...
    for (const auto& SPBlock : mydesign.SPBlocks) {
      if (SPBlock.axis_dir == placerDB::H) {
        ...
        for (int i = 0; i < SPBlock.sympair.size(); i++) {
          ...
          {
            double sparserow[2] = {1, -1};
            int colno[2] = {first_id * 2 + 1, second_id * 2 + 1};
            int first_x_center = mydesign.Blocks[first_id][curr_sp.selected[first_id]].width / 2;
            int second_x_center = mydesign.Blocks[second_id][curr_sp.selected[second_id]].width / 2;
            add_constraintex(area_lp, 2, sparserow, colno, EQ, -first_x_center + second_x_center);
          }
        }
        ...
        for (int i = 0; i < SPBlock.sympair.size(); i++) {
          ...
          for (unsigned int j = i + 1; j < SPBlock.sympair.size(); j++) {
            ...
            double sparserow[] = {0.5, 0.5, -0.5, -0.5};
            int colno[] = {i_first_id * 2 + 2, i_second_id * 2 + 2, j_first_id * 2 + 2, j_second_id * 2 + 2};
            int bias = -i_first_y_center - i_second_y_center + j_first_y_center + j_second_y_center;
            add_constraintex(area_lp, 4, sparserow, colno, EQ, bias);
          }
        }
        ...
        for (int i = 0; i < SPBlock.sympair.size(); i++) {
          ...
          for (unsigned int j = 0; j < SPBlock.selfsym.size(); j++) {
            ...
            double sparserow[3] = {0.5, 0.5, -1};
            int colno[3] = {i_first_id * 2 + 2, i_second_id * 2 + 2, j_id * 2 + 2};
            int bias = -i_first_y_center - i_second_y_center + j_y_center;
            add_constraintex(area_lp, 3, sparserow, colno, EQ, bias);
          }
        }
        ...
        for (int i = 0; i < SPBlock.selfsym.size(); i++) {
          ...
          for (unsigned int j = i + 1; j < SPBlock.selfsym.size(); j++) {
            ...
            int j_y_center = mydesign.Blocks[j_id][curr_sp.selected[j_id]].height / 2;
            double sparserow[2] = {1, -1};
            int colno[2] = {i_id * 2 + 2, j_id * 2 + 2};
            int bias = -i_y_center + j_y_center;
            add_constraintex(area_lp, 2, sparserow, colno, EQ, bias);
          }
        }
      } else {
        // axis_dir==V
        ...
        for (int i = 0; i < SPBlock.sympair.size(); i++) {
          ...
          {
            double sparserow[2] = {1, -1};
            int colno[2] = {first_id * 2 + 2, second_id * 2 + 2};
            int first_y_center = mydesign.Blocks[first_id][curr_sp.selected[first_id]].height / 2;
            int second_y_center = mydesign.Blocks[second_id][curr_sp.selected[second_id]].height / 2;
            add_constraintex(area_lp, 2, sparserow, colno, EQ, -first_y_center + second_y_center);
          }
        }
        ...
        for (int i = 0; i < SPBlock.sympair.size(); i++) {
          ...
          for (unsigned int j = i + 1; j < SPBlock.sympair.size(); j++) {
            ...
            double sparserow[] = {0.5, 0.5, -0.5, -0.5};
            int colno[] = {i_first_id * 2 + 1, i_second_id * 2 + 1, j_first_id * 2 + 1, j_second_id * 2 + 1};
            int bias = -i_first_x_center - i_second_x_center + j_first_x_center + j_second_x_center;
            add_constraintex(area_lp, 4, sparserow, colno, EQ, bias);
          }
        }
        ...
        for (int i = 0; i < SPBlock.sympair.size(); i++) {
          ...
          for (unsigned int j = 0; j < SPBlock.selfsym.size(); j++) {
            ...
            double sparserow[] = {0.5, 0.5, -1};
            int colno[] = {i_first_id * 2 + 1, i_second_id * 2 + 1, j_id * 2 + 1};
            int bias = -i_first_x_center - i_second_x_center + j_x_center;
            add_constraintex(area_lp, 3, sparserow, colno, EQ, bias);
          }
        }
      ...
      for (unsigned int j = 0; j < alignment_unit.blocks.size() - 1; j++) {
        ...
        if (alignment_unit.horizon == 1) {
          if (alignment_unit.line == 0) {
            ...
            double sparserow[2] = {1, -1};
            int colno[2] = {first_id * 2 + 2, second_id * 2 + 2};
            add_constraintex(area_lp, 2, sparserow, colno, EQ, 0);
...

    {
        ...
        for (unsigned int j = 0; j < mydesign.Nets[i].connected.size(); j++) {
          if (mydesign.Nets[i].connected[j].type == placerDB::Block) {
            ...
            if (netExtremes.InLeftExtreme(i, block_id)) {
              double sparserow[] = {1, -1};
              int    colno[]     = {block_id * 2 + 1, ind};
              add_constraintex(area_lp, 2, sparserow, colno, GE, -pin_llx);
            }
            if (netExtremes.InBottomExtreme(i, block_id)) {
              double sparserow[] = {1, -1};
              int    colno[]     = {block_id * 2 + 2, ind + 1};
              add_constraintex(area_lp, 2, sparserow, colno, GE, -pin_lly);
            }
            if (netExtremes.InRightExtreme(i, block_id)) {
              double sparserow[] = {1, -1};
              int    colno[]     = {block_id * 2 + 1, ind + 2};
              add_constraintex(area_lp, 2, sparserow, colno, LE, -pin_urx);
            }
            if (netExtremes.InTopExtreme(i, block_id)) {
              double sparserow[] = {1, -1};
              int    colno[]     = {block_id * 2 + 2, ind + 3};
              add_constraintex(area_lp, 2, sparserow, colno, LE, -pin_ury);
            }
          }
        }
      }
      set_add_rowmode(area_lp, FALSE);

      set_minim(area_lp);
      set_timeout(area_lp, 1);
      ...
      //}
      //if (fail_cnt++ < 10) {
      //  write_lp(area_lp, const_cast<char*>((mydesign.name + "_ilp_" + std::to_string(fail_cnt) + ".area_lp").c_str()));
      //}
      int ret{0};
      {
      ...
      get_variables(area_lp, var.data());
      delete_lp(area_lp);

      int minx(INT_MAX), miny(INT_MAX);
      for (int i = 0; i < mydesign.Blocks.size(); i++) {
        Blocks[i].x = var.at(i * 2);
        Blocks[i].y = var.at(i * 2 + 1);
  ...
  for (int i = 0; i < mydesign.Nets.size(); ++i) {
    netExtremes.FindExtremes(mydesign.Nets[i], i, Blocks, mydesign, curr_sp);
  }
  {
    unsigned int N_var = (mydesign.Blocks.size() * 2 + mydesign.Nets.size() * 4);
    lprec* hpwl_lp = make_lp(0, N_var);
    set_verbose(hpwl_lp, CRITICAL);
    // set integer constraint, H_flip and V_flip can only be 0 or 1
    for (int i = 0; i < mydesign.Blocks.size(); i++) {
      int ind = i * 2 + 1;
      set_binary(hpwl_lp, ind, TRUE);
      set_col_name(hpwl_lp, ind++, const_cast<char*>((mydesign.Blocks[i][0].name + "_flx").c_str()));
      set_binary(hpwl_lp, ind, TRUE);
      set_col_name(hpwl_lp, ind,   const_cast<char*>((mydesign.Blocks[i][0].name + "_fly").c_str()));
    }

    for (int i = 0; i < mydesign.Nets.size(); ++i) {
      int ind = i * 4 + mydesign.Blocks.size() * 2 + 1;
      set_col_name(hpwl_lp, ind++, const_cast<char*>((mydesign.Nets[i].name + "_ll_x").c_str()));
      set_col_name(hpwl_lp, ind++, const_cast<char*>((mydesign.Nets[i].name + "_ll_y").c_str()));
      set_col_name(hpwl_lp, ind++, const_cast<char*>((mydesign.Nets[i].name + "_ur_x").c_str()));
      set_col_name(hpwl_lp, ind,   const_cast<char*>((mydesign.Nets[i].name + "_ur_y").c_str()));
    }
    ...
    set_obj_fn(hpwl_lp, row.data());

    set_add_rowmode(hpwl_lp, TRUE);
    for (const auto& SPBlock : mydesign.SPBlocks) {
      if (SPBlock.axis_dir == placerDB::H) {
        for (int i = 0; i < SPBlock.sympair.size(); i++) {
          int first_id = SPBlock.sympair[i].first, second_id = SPBlock.sympair[i].second;
          {
            double sparserow[2] = {1, 1};
            int colno[2] = {first_id * 2 + 2, second_id * 2 + 2};
            add_constraintex(hpwl_lp, 2, sparserow, colno, EQ, 1);
          }
          {
            double sparserow[2] = {1, -1};
            int colno[2] = {first_id * 2 + 1, second_id * 2 + 1};
            add_constraintex(hpwl_lp, 2, sparserow, colno, EQ, 0);
          }
        }
      } else {
        for (int i = 0; i < SPBlock.sympair.size(); i++) {
    ...
    for (unsigned int i = 0; i < mydesign.Nets.size(); i++) {
      ...
      for (unsigned int j = 0; j < mydesign.Nets[i].connected.size(); j++) {
        if (mydesign.Nets[i].connected[j].type == placerDB::Block) {
          ...
          if (netExtremes.InLeftExtreme(i, block_id)) {
            double sparserow[2] = {deltax, -1};
            int    colno[2]     = {block_id * 2 + 1, ind};
            add_constraintex(hpwl_lp, 2, sparserow, colno, GE, -Blocks[block_id].x -pin_llx);
          }
          if (netExtremes.InBottomExtreme(i, block_id)) {
            double sparserow[2] = {deltay, -1};
            int    colno[2]     = {block_id * 2 + 2, ind + 1};
            add_constraintex(hpwl_lp, 2, sparserow, colno, GE, -Blocks[block_id].y -pin_lly);
          }
          if (netExtremes.InRightExtreme(i, block_id)) {
            double sparserow[2] = {deltax, -1};
            int    colno[2]     = {block_id * 2 + 1, ind + 2};
            add_constraintex(hpwl_lp, 2, sparserow, colno, LE, -Blocks[block_id].x -pin_urx);
          }
          if (netExtremes.InTopExtreme(i, block_id)) {
            double sparserow[2] = {deltay, -1};
            int    colno[2]     = {block_id * 2 + 2, ind + 3};
            add_constraintex(hpwl_lp, 2, sparserow, colno, LE, -Blocks[block_id].y -pin_ury);
          }
        }
      }
    }
    set_add_rowmode(hpwl_lp, FALSE);

    set_minim(hpwl_lp);
    set_timeout(hpwl_lp, 1);
    ...
    int ret{0};
    {
      ...
      if (fail_cnt++ < 10) {
        write_lp(hpwl_lp, const_cast<char*>((mydesign.name + "_hpwl_ilp_" + std::to_string(fail_cnt) + ".lp").c_str()));
      }*/
      ret = solve(hpwl_lp);
    }
    if (ret == 0 || ret == 1) {
      std::vector<double> var(N_var);
      get_variables(hpwl_lp, var.data());
      for (int i = 0; i < mydesign.Blocks.size(); i++) {
        Blocks[i].H_flip = var.at(i * 2);
        Blocks[i].V_flip = var.at(i * 2 + 1);
      }
      HPWL_ILP = 0.;
      for (int i = 0; i < mydesign.Nets.size(); ++i) {
        int ind = (int(mydesign.Blocks.size()) + i) * 2;
...
bool ILP_solver::FrameSolveILP(const design& mydesign, const SeqPair& curr_sp, const PnRDB::Drc_info& drcInfo, bool flushbl, const vector<placerDB::point>* prev) {
...

  {
    ...
    for (unsigned int i = 0; i < mydesign.Nets.size(); i++) {
      ...
      /*for (unsigned int j = 0; j < mydesign.Nets[i].connected.size(); j++) {
        if (mydesign.Nets[i].connected[j].type == placerDB::Block) {
          ...
          uniqueXRows.emplace(block_id * 4 + 1, ind, (blk.width - pin_llx - pin_urx), -pin_llx, -pin_urx);
          uniqueYRows.emplace(block_id * 4 + 2, ind + 1, (blk.height - pin_lly - pin_ury), -pin_lly, -pin_ury);
        }
      }*/
      for (unsigned int j = 0; j < mydesign.Nets[i].connected.size(); j++) {
        if (mydesign.Nets[i].connected[j].type == placerDB::Block) {
    ...
    /*for (const auto& it : uniqueXRows) {
      double sparserow[] = {1, 1.*it._d1, -1};
      int    colno[]     = {it._id1, it._id1 + 2, it._id2};
      add_constraintex(lp, 3, sparserow, colno, GE, it._ll);
      colno[2] += 2;
      add_constraintex(lp, 3, sparserow, colno, LE, it._ur);
    }
    ...
    //}
    //if (fail_cnt++ < 10) {
    //  write_lp(lp, const_cast<char*>((mydesign.name + "_ilp_" + std::to_string(fail_cnt) + ".lp").c_str()));
    //}
    int ret{0};
    {
      TimeMeasure tmsol(const_cast<design&>(mydesign).ilp_solve_runtime);
