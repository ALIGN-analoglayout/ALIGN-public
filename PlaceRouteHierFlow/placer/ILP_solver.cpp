  ...
  for (auto symmetry : mydesign.SPBlocks) {
    if (symmetry.axis_dir == placerDB::V) {
      ...
      for (auto i_sympair : symmetry.sympair) {
        ...
        distance_set.insert(
            abs(node.Blocks[i_sympair.first].instance[0].placedCenter.y -
                node.Blocks[i_sympair.second].instance[0].placedCenter.y));
        int center_x =
            (node.Blocks[i_sympair.first].instance[0].placedCenter.x +
             node.Blocks[i_sympair.second].instance[0].placedCenter.x) /
            2;
        while (center_x_set.find(center_x) != center_x_set.end())
          center_x++;
  ...
  for (auto align : mydesign.Align_blocks) {
    if (align.horizon) {
      ...
      for (unsigned int i = 0; i < align.blocks.size(); i++) {
        LLy += node.Blocks[align.blocks[i]].instance[0].placedCenter.y -
               node.Blocks[align.blocks[i]].instance[0].height / 2;
      }
      ...
      for (unsigned int i = 0; i < align.blocks.size(); i++) {
        ...
        node.Blocks[align.blocks[i]].instance[0].placedCenter.y =
            LLy + node.Blocks[align.blocks[i]].instance[0].height / 2;
      }
    } else {
      int LLx = 0;
      set<int> center_y_set;
      for (unsigned int i = 0; i < align.blocks.size(); i++) {
        LLx += node.Blocks[align.blocks[i]].instance[0].placedCenter.x -
               node.Blocks[align.blocks[i]].instance[0].width / 2;
      }
      ...
      for (unsigned int i = 0; i < align.blocks.size(); i++) {
        ...
        center_y_set.insert(
            node.Blocks[align.blocks[i]].instance[0].placedCenter.y);
        node.Blocks[align.blocks[i]].instance[0].placedCenter.x =
            LLx + node.Blocks[align.blocks[i]].instance[0].width / 2;
      }
    }
  }
  block_order = vector<vector<int>>(mydesign.Blocks.size(),
                                    vector<int>(mydesign.Blocks.size(), 0));
  // from LSB to MSB: at the left, align to left, the same x center, align to
  // right, at the right, reserved, reserved, reserved below, align to the
  // bottom, the same y center, align to the top, above, reserved, reserved,
  // reserved block[i][j]&0x0010==0x0010 means i is to the right of j
  ...
  for (auto order : node.Ordering_Constraints) {
    ...
    for (auto i : order.first) {
      ...
      for (auto align : node.Align_blocks) {
        if (order.second == PnRDB::H && align.horizon == 0 || order.second ==
  PnRDB::V && align.horizon == 1) { if (find(align.blocks.begin(),
  align.blocks.end(), i) != align.blocks.end()) for (auto b : align.blocks) if
  (b != i) temp_seq.push_back(b);
        }
      }
      ...
      for (unsigned int i = 0; i < blocks.size(); i++) {
        for (unsigned int j = i + 1; j < blocks.size(); j++) {
          if (block_order[blocks[i]][blocks[j]] & 0xff00)
            logger->error("wrong constraint between block {0} and {1}",
                          mydesign.Blocks[blocks[i]][0].name,
                          mydesign.Blocks[blocks[j]][0].name);
          else
            block_order[blocks[i]][blocks[j]] |=
                0x0200; // i and j align to the bottom
        }
      }
    } else {
      vector<int> blocks(align.blocks);
      ...
      for (unsigned int i = 0; i < blocks.size(); i++) {
        for (unsigned int j = i + 1; j < blocks.size(); j++) {
          if (block_order[blocks[i]][blocks[j]] & 0x00ff)
            ...
            block_order[blocks[i]][blocks[j]] |=
                0x0002; // i and j align to the left
        }
      }
    }
  }
    ...
    if (symmetry.axis_dir == placerDB::V) {
      for (auto pair : symmetry.sympair) {
        ...
        if (block_order[first][second] & 0x1100)
          logger->error("wrong constraint between block {0} and {1}",
                        mydesign.Blocks[first][0].name,
                        mydesign.Blocks[second][0].name);
        else if (block_order[first][second] & 0x0a00 &&
                 mydesign.Blocks[first][0].height !=
                     mydesign.Blocks[second][0].height)
          logger->error("wrong constraint between block {0} and {1}",
                        mydesign.Blocks[first][0].name,
                        mydesign.Blocks[second][0].name);
        else {
          ...
          block_order[first][second] |=
              0x0400; // i and j have the same y center
        }
      }
      for (unsigned int i = 0; i < symmetry.selfsym.size(); i++) {
        for (unsigned int j = i + 1; j < symmetry.selfsym.size(); j++) {
          int first = symmetry.selfsym[i].first,
              second = symmetry.selfsym[j].first;
          if (first > second)
            std::swap(first, second);
          block_order[first][second] |= 0x0004;
          if (node.Blocks[first].instance[0].placedCenter.y <
              node.Blocks[second].instance[0].placedCenter.y)
      ...
      for (auto pair : symmetry.sympair) {
        ...
        if (block_order[first][second] & 0x0011)
          logger->error("wrong constraint between block {0} and {1}",
                        mydesign.Blocks[first][0].name,
                        mydesign.Blocks[second][0].name);
        else if (block_order[first][second] & 0x000a &&
                 mydesign.Blocks[first][0].width !=
                     mydesign.Blocks[second][0].width)
          logger->error("wrong constraint between block {0} and {1}",
                        mydesign.Blocks[first][0].name,
                        mydesign.Blocks[second][0].name);
        else {
          ...
          block_order[first][second] |=
              0x0004; // i and j have the same x center
        }
      }
      for (unsigned int i = 0; i < symmetry.selfsym.size(); i++) {
        for (unsigned int j = i + 1; j < symmetry.selfsym.size(); j++) {
          ...
          if (node.Blocks[first].instance[0].placedCenter.x <
              node.Blocks[second].instance[0].placedCenter.x)
  ...
  for (unsigned int i = 0; i < node.Blocks.size() - 1; i++) {
    for (unsigned int j = i + 1; j < node.Blocks.size(); j++) {
      if ((block_order[i][j] & 0x000e) && (block_order[i][j] & 0x0e00))
        logger->error("wrong constranit between block {0} and {1}",
                      mydesign.Blocks[i][0].name, mydesign.Blocks[j][0].name);
      ...
      if ((block_order[i][j] & 0x1111) == 0) { // neither left right below above
        if (block_order[i][j] & 0x00ff) {
          ...
          if (node.Blocks[i].instance[0].placedCenter.y <
              node.Blocks[j].instance[0].placedCenter.y) {
            block_order[i][j] |= 0x0100;
            int i_counterpart =
                mydesign.SPBlocks[mydesign.Blocks[i][0].SBidx].axis_dir ==
                        placerDB::V
                    ? mydesign.Blocks[i][0].counterpart
                    : -1;
            int j_counterpart =
                mydesign.SPBlocks[mydesign.Blocks[j][0].SBidx].axis_dir ==
                        placerDB::V
                    ? mydesign.Blocks[j][0].counterpart
                    : -1;
            if (i_counterpart != -1) {
              if (i_counterpart < j &&
                  ((block_order[i_counterpart][j] & 0x1111) == 0))
                block_order[i_counterpart][j] |= 0x0100;
              if (i_counterpart > j &&
                  ((block_order[j][i_counterpart] & 0x1111) == 0))
                block_order[j][i_counterpart] |= 0x1000;
            }
            if (j_counterpart != -1) {
              if (i < j_counterpart &&
                  ((block_order[i][j_counterpart] & 0x1111) == 0))
                block_order[i][j_counterpart] |= 0x0100;
              if (i > j_counterpart &&
                  ((block_order[j_counterpart][i] & 0x1111) == 0))
                block_order[j_counterpart][i] |= 0x1000;
            }
            if (i_counterpart != -1 && j_counterpart != -1) {
              if (i_counterpart < j_counterpart &&
                  ((block_order[i_counterpart][j_counterpart] & 0x1111) == 0))
                block_order[i_counterpart][j_counterpart] |= 0x0100;
              if (i_counterpart > j_counterpart &&
                  ((block_order[j_counterpart][i_counterpart] & 0x1111) == 0))
            ...
            if (i_counterpart != -1) {
              if (i_counterpart < j &&
                  ((block_order[i_counterpart][j] & 0x1111) == 0))
                block_order[i_counterpart][j] |= 0x1000;
              if (i_counterpart > j &&
                  ((block_order[j][i_counterpart] & 0x1111) == 0))
                block_order[j][i_counterpart] |= 0x0100;
            }
            if (j_counterpart != -1) {
              if (i < j_counterpart &&
                  ((block_order[i][j_counterpart] & 0x1111) == 0))
                block_order[i][j_counterpart] |= 0x1000;
              if (i > j_counterpart &&
                  ((block_order[j_counterpart][i] & 0x1111) == 0))
                block_order[j_counterpart][i] |= 0x0100;
            }
            if (i_counterpart != -1 && j_counterpart != -1) {
              if (i_counterpart < j_counterpart &&
                  ((block_order[i_counterpart][j_counterpart] & 0x1111) == 0))
          ...
          if (node.Blocks[i].instance[0].placedCenter.x <
              node.Blocks[j].instance[0].placedCenter.x)
            block_order[i][j] |= 0x0001;
          else
            block_order[i][j] |= 0x0010;
        } else {
          // if((!node.isFirstILP && ( node.placement_id & (1<<(count%30))))
          //|| (node.isFirstILP && abs(node.Blocks[i].instance[0].placedCenter.x
          //- node.Blocks[j].instance[0].placedCenter.x) <
          // abs(node.Blocks[i].instance[0].placedCenter.y -
          // node.Blocks[j].instance[0].placedCenter.y))){
          if (abs(node.Blocks[i].instance[0].placedCenter.x -
                  node.Blocks[j].instance[0].placedCenter.x) <
              abs(node.Blocks[i].instance[0].placedCenter.y -
                  node.Blocks[j].instance[0].placedCenter.y)) {
            block_order[i][j] &= 0x00ff;
...
void ILP_solver::lpsolve_logger(lprec *lp, void *userhandle, char *buf) {
  auto logger =
      spdlog::default_logger()->clone("placer.ILP_solver.lpsolve_logger");
  ...
  // Log non-empty lines
  if (*buf != '\0')
    logger->debug("Placer lpsolve: {0}", buf);
}

double ILP_solver::GenerateValidSolutionAnalytical(design &mydesign,
                                                   PnRDB::Drc_info &drcInfo,
                                                   PnRDB::hierNode &node) {
  auto logger = spdlog::default_logger()->clone(
      "placer.ILP_solver.GenerateValidSolution");
  ...
  for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) {
    for (unsigned int j = i + 1; j < mydesign.Blocks.size(); j++) {
      if (block_order[i][j] & 0x0001) {
        ...
        if (!add_constraintex(lp, 2, sparserow, colno, GE,
                              mydesign.Blocks[i][0].width +
                                  mydesign.bias_Hgraph))
          logger->error("error");
      } else if (block_order[i][j] & 0x0002) {
        // i and j align to LLx
        double sparserow[2] = {1, -1};
        int colno[2] = {int(i) * 4 + 1, int(j) * 4 + 1};
        if (!add_constraintex(lp, 2, sparserow, colno, EQ, 0))
          logger->error("error");
      } else if (block_order[i][j] & 0x0004) {
        // i and j align to x center
        double sparserow[2] = {1, -1};
        int colno[2] = {int(i) * 4 + 1, int(j) * 4 + 1};
        if (!add_constraintex(lp, 2, sparserow, colno, EQ,
                              mydesign.Blocks[j][0].width / 2 -
                                  mydesign.Blocks[i][0].width / 2))
          logger->error("error");
      ...
      if (block_order[i][j] & 0x0100) {
        ...
        if (!add_constraintex(lp, 2, sparserow, colno, GE,
                              mydesign.Blocks[i][0].height +
                                  mydesign.bias_Vgraph))
          logger->error("error");
        ...
        int colno[2] = {int(i) * 4 + 2, int(j) * 4 + 2};
        if (!add_constraintex(lp, 2, sparserow, colno, EQ,
                              mydesign.Blocks[j][0].height / 2 -
                                  mydesign.Blocks[i][0].height / 2))
          logger->error("error");
  ...
  /**
  for (int i = 0; i < mydesign.Blocks.size(); i++) {
    int i_pos_index = find(curr_sp.posPair.begin(), curr_sp.posPair.end(), i) -
  curr_sp.posPair.begin(); int i_neg_index = find(curr_sp.negPair.begin(),
  curr_sp.negPair.end(), i) - curr_sp.negPair.begin(); for (int j = i + 1; j <
  mydesign.Blocks.size(); j++) { int j_pos_index = find(curr_sp.posPair.begin(),
  curr_sp.posPair.end(), j) - curr_sp.posPair.begin(); int j_neg_index =
  find(curr_sp.negPair.begin(), curr_sp.negPair.end(), j) -
  curr_sp.negPair.begin(); if (i_pos_index < j_pos_index) { if (i_neg_index <
  j_neg_index) {
          // i is left of j
          double sparserow[2] = {1, -1};
          int colno[2] = {i * 4 + 1, j * 4 + 1};
          if (!add_constraintex(lp, 2, sparserow, colno, LE,
  -mydesign.Blocks[i][curr_sp.selected[i]].width - mydesign.bias_Hgraph))
  logger->error("error"); } else {
          // i is above j
          double sparserow[2] = {1, -1};
          int colno[2] = {i * 4 + 2, j * 4 + 2};
          if (!add_constraintex(lp, 2, sparserow, colno, GE,
  mydesign.Blocks[j][curr_sp.selected[j]].height + mydesign.bias_Vgraph))
  logger->error("error");
        }
      } else {
        if (i_neg_index < j_neg_index) {
          ...
          int colno[2] = {i * 4 + 2, j * 4 + 2};
          if (!add_constraintex(lp, 2, sparserow, colno, LE,
  -mydesign.Blocks[i][curr_sp.selected[i]].height - mydesign.bias_Vgraph))
  logger->error("error"); } else {
          // i is right of j
          double sparserow[2] = {1, -1};
          int colno[2] = {i * 4 + 1, j * 4 + 1};
          if (!add_constraintex(lp, 2, sparserow, colno, GE,
  mydesign.Blocks[j][curr_sp.selected[j]].width + mydesign.bias_Hgraph))
  logger->error("error");
        }
      }
    }
  ...
  for (unsigned int i = 0; i < node.Blocks.size(); i++) {
    {
      ...
      if (!add_constraintex(lp, 1, sparserow, colno, GE, 0))
        logger->error("error");
    }
    {
      double sparserow[1] = {1};
  ...
  for (auto SPBlock : mydesign.SPBlocks) {
    if (SPBlock.axis_dir == placerDB::H) {
      ...
      for (unsigned int i = 0; i < SPBlock.sympair.size(); i++) {
        int first_id = SPBlock.sympair[i].first,
            second_id = SPBlock.sympair[i].second;
        ...
        // int first_x_center =
        // mydesign.Blocks[first_id][curr_sp.selected[first_id]].width / 2; int
        // second_x_center =
        // mydesign.Blocks[second_id][curr_sp.selected[second_id]].width / 2;
        // add_constraintex(lp, 2, sparserow, colno, EQ, -first_x_center +
        // second_x_center);
        //}
      }
      ...
      for (unsigned int i = 0; i < SPBlock.sympair.size(); i++) {
        int i_first_id = SPBlock.sympair[i].first,
            i_second_id = SPBlock.sympair[i].second;
        ...
        for (unsigned int j = i + 1; j < SPBlock.sympair.size(); j++) {
          ...
          int j_first_id = SPBlock.sympair[j].first,
              j_second_id = SPBlock.sympair[j].second;
          ...
          int colno[4] = {i_first_id * 4 + 2, i_second_id * 4 + 2,
                          j_first_id * 4 + 2, j_second_id * 4 + 2};
          int bias = -i_first_y_center - i_second_y_center + j_first_y_center +
                     j_second_y_center;
      ...
      for (unsigned int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        for (unsigned int j = 0; j < SPBlock.selfsym.size(); j++) {
          ...
          int colno[3] = {i_first_id * 4 + 2, i_second_id * 4 + 2,
                          j_id * 4 + 2};
      ...
      for (unsigned int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        // int first_y_center =
        // mydesign.Blocks[first_id][curr_sp.selected[first_id]].height / 2; int
        // second_y_center =
        // mydesign.Blocks[second_id][curr_sp.selected[second_id]].height / 2;
        // add_constraintex(lp, 2, sparserow, colno, EQ, -first_y_center +
        // second_y_center);
        //}
      }
      ...
      for (unsigned int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        for (unsigned int j = i + 1; j < SPBlock.sympair.size(); j++) {
          ...
          double sparserow[4] = {0.5, 0.5, -0.5, -0.5};
          int colno[4] = {i_first_id * 4 + 1, i_second_id * 4 + 1,
                          j_first_id * 4 + 1, j_second_id * 4 + 1};
          int bias = -i_first_x_center - i_second_x_center + j_first_x_center +
                     j_second_x_center;
          add_constraintex(lp, 4, sparserow, colno, EQ, bias);
        }
      }
      ...
      for (unsigned int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        for (unsigned int j = 0; j < SPBlock.selfsym.size(); j++) {
          ...
          double sparserow[3] = {0.5, 0.5, -1};
          int colno[3] = {i_first_id * 4 + 1, i_second_id * 4 + 1,
                          j_id * 4 + 1};
  ...
  for (auto alignment_unit : mydesign.Align_blocks) {
    for (int j = 0; j < alignment_unit.blocks.size() - 1; j++) {
      int first_id = alignment_unit.blocks[j], second_id =
  alignment_unit.blocks[j + 1]; if (alignment_unit.horizon == 1) {
        // same y
        double sparserow[2] = {1, -1};
        int colno[2] = {first_id * 4 + 2, second_id * 4 + 2};
  ...
  // set_add_rowmode(lp, FALSE);
  {
    ...
    for (unsigned int i = 0; i < mydesign.Nets.size(); i++) {
      ...
      ///(blockids.size() == 0 || mydesign.Nets[i].connected[j].iter2 !=
      /// curr_sp.negPair[blockids.back().first]))
      /// blockids.push_back(std::make_pair(find(curr_sp.negPair.begin(),
      /// curr_sp.negPair.end(), mydesign.Nets[i].connected[j].iter2) -
      ...
      for (unsigned int j = 0; j < mydesign.Nets[i].connected.size(); j++) {
        if (mydesign.Nets[i].connected[j].type == placerDB::Block) {
          block_pos_x_set.insert(
              std::make_pair(std::make_pair(mydesign.Nets[i].connected[j].iter2,
                                            mydesign.Nets[i].connected[j].iter),
                             node.Blocks[mydesign.Nets[i].connected[j].iter2]
                                 .instance[0]
                                 .placedCenter.x));
      ...
      vector<pair<pair<int, int>, int>> block_pos_x(block_pos_x_set.begin(),
                                                    block_pos_x_set.end());
      vector<pair<pair<int, int>, int>> block_pos_y(block_pos_y_set.begin(),
                                                    block_pos_y_set.end());
      if (block_pos_x.size() < 2)
        continue;
      sort(block_pos_x.begin(), block_pos_x.end(),
           [](const pair<pair<int, int>, int> &a,
              const pair<pair<int, int>, int> &b) {
             return a.second < b.second;
           });
      sort(block_pos_y.begin(), block_pos_y.end(),
           [](const pair<pair<int, int>, int> &a,
              const pair<pair<int, int>, int> &b) {
             return a.second < b.second;
           });
      // sort(blockids.begin(), blockids.end(), [](const pair<int, int>& a,
      // const pair<int, int>& b) { return a.first <= b.first; });
      /**int LLblock_id = curr_sp.negPair[blockids.front().first], LLpin_id =
      blockids.front().second; int LLblock_width =
      mydesign.Blocks[LLblock_id][curr_sp.selected[LLblock_id]].width,
          LLblock_height =
      mydesign.Blocks[LLblock_id][curr_sp.selected[LLblock_id]].height; int
      LLpin_x =
      mydesign.Blocks[LLblock_id][curr_sp.selected[LLblock_id]].blockPins[LLpin_id].center.front().x,
          LLpin_y =
      mydesign.Blocks[LLblock_id][curr_sp.selected[LLblock_id]].blockPins[LLpin_id].center.front().y;
      int URblock_id = curr_sp.negPair[blockids.back().first], URpin_id =
      blockids.back().second; int URblock_width =
      mydesign.Blocks[URblock_id][curr_sp.selected[URblock_id]].width,
          URblock_height =
      mydesign.Blocks[URblock_id][curr_sp.selected[URblock_id]].height; int
      URpin_x =
      mydesign.Blocks[URblock_id][curr_sp.selected[URblock_id]].blockPins[URpin_id].center.front().x,
          URpin_y =
      mydesign.Blocks[URblock_id][curr_sp.selected[URblock_id]].blockPins[URpin_id].center.front().y;**/
      int Lblock_id = block_pos_x.front().first.first,
          Lpin_id = block_pos_x.front().first.second;
      int Rblock_id = block_pos_x.back().first.first,
          Rpin_id = block_pos_x.back().first.second;
      int Lblock_width = mydesign.Blocks[Lblock_id][0].width,
          Lblock_height = mydesign.Blocks[Lblock_id][0].height;
      int Rblock_width = mydesign.Blocks[Rblock_id][0].width,
          Rblock_height = mydesign.Blocks[Rblock_id][0].height;
      int Lpin_x = mydesign.Blocks[Lblock_id][0].blockPins.size() > 0
                       ? mydesign.Blocks[Lblock_id][0]
                             .blockPins[Lpin_id]
                             .center.front()
                             .x
                       : mydesign.Blocks[Lblock_id][0].width / 2,
          Lpin_y = mydesign.Blocks[Lblock_id][0].blockPins.size() > 0
                       ? mydesign.Blocks[Lblock_id][0]
                             .blockPins[Lpin_id]
                             .center.front()
                             .y
                       : mydesign.Blocks[Lblock_id][0].height / 2;
      int Rpin_x = mydesign.Blocks[Rblock_id][0].blockPins.size() > 0
                       ? mydesign.Blocks[Rblock_id][0]
                             .blockPins[Rpin_id]
                             .center.front()
                             .x
                       : mydesign.Blocks[Rblock_id][0].width / 2,
          Rpin_y = mydesign.Blocks[Rblock_id][0].blockPins.size() > 0
                       ? mydesign.Blocks[Rblock_id][0]
                             .blockPins[Rpin_id]
                             .center.front()
                             .y
      ...
      int Dblock_id = block_pos_y.front().first.first,
          Dpin_id = block_pos_y.front().first.second;
      int Ublock_id = block_pos_y.back().first.first,
          Upin_id = block_pos_y.back().first.second;
      int Dblock_width = mydesign.Blocks[Dblock_id][0].width,
          Dblock_height = mydesign.Blocks[Dblock_id][0].height;
      int Ublock_width = mydesign.Blocks[Ublock_id][0].width,
          Ublock_height = mydesign.Blocks[Ublock_id][0].height;
      int Dpin_x = mydesign.Blocks[Dblock_id][0].blockPins.size() > 0
                       ? mydesign.Blocks[Dblock_id][0]
                             .blockPins[Dpin_id]
                             .center.front()
                             .x
                       : mydesign.Blocks[Dblock_id][0].width / 2,
          Dpin_y = mydesign.Blocks[Dblock_id][0].blockPins.size() > 0
                       ? mydesign.Blocks[Dblock_id][0]
                             .blockPins[Dpin_id]
                             .center.front()
                             .y
                       : mydesign.Blocks[Dblock_id][0].height / 2;
      int Upin_x = mydesign.Blocks[Ublock_id][0].blockPins.size() > 0
                       ? mydesign.Blocks[Ublock_id][0]
                             .blockPins[Upin_id]
                             .center.front()
                             .x
                       : mydesign.Blocks[Ublock_id][0].width / 2,
          Upin_y = mydesign.Blocks[Ublock_id][0].blockPins.size() > 0
                       ? mydesign.Blocks[Ublock_id][0]
                             .blockPins[Upin_id]
                             .center.front()
                             .y
      ...
      // min
      // abs(LLx+(LLwidth-2LLpinx)*LLHflip+LLpinx-URx-(URwidth-2URpinx)*URHflip-URpinx)=HPWLx
      //->
      ...
      if (Lblock_id != Rblock_id) {
        {
          double sparserow[5] = {
              const_graph.LAMBDA,
              (Lblock_width - 2 * Lpin_x) * const_graph.LAMBDA,
              -const_graph.LAMBDA,
              -(Rblock_width - 2 * Rpin_x) * const_graph.LAMBDA, -1};
          int colno[5] = {Lblock_id * 4 + 1, Lblock_id * 4 + 3,
                          Rblock_id * 4 + 1, Rblock_id * 4 + 3,
                          int(mydesign.Blocks.size()) * 4 + int(i) * 2 + 1};
      ...
      if (Dblock_id != Ublock_id) {
        {
          double sparserow[5] = {
              const_graph.LAMBDA,
              (Dblock_height - 2 * Dpin_y) * const_graph.LAMBDA,
              -const_graph.LAMBDA,
              -(Ublock_height - 2 * Upin_y) * const_graph.LAMBDA, -1};
          int colno[5] = {Dblock_id * 4 + 2, Dblock_id * 4 + 4,
                          Ublock_id * 4 + 2, Ublock_id * 4 + 4,
                          int(mydesign.Blocks.size()) * 4 + int(i) * 2 + 2};
...
// get_presolveloops(lp));
  ...
  dead_area = area;
  for (int i = 0; i < mydesign.Blocks.size(); i++) {
    dead_area -= double(mydesign.Blocks[i][0].width) *
  double(mydesign.Blocks[i][0].height);
  }
  // calculate ratio
  // ratio = std::max(double(UR.x - LL.x) / double(UR.y - LL.y), double(UR.y -
  LL.y) / double(UR.x - LL.x));
  **/
  ratio = double(UR.x - LL.x) / double(UR.y - LL.y);
  if (ratio < Aspect_Ratio[0] || ratio > Aspect_Ratio[1]) {
    return -1;
  }
  if (placement_box[0] > 0 && (UR.x - LL.x > placement_box[0]) ||
      placement_box[1] > 0 && (UR.y - LL.y > placement_box[1])) {
    return -1;
  }
  // calculate HPWL
  HPWL = 0;
    ...
    for (auto connectedj : neti.connected) {
      if (connectedj.type == placerDB::Block) {
        ...
        if (mydesign.Blocks[iter2][0].blockPins.size() > 0) {
          for (auto centerk :
               mydesign.Blocks[iter2][0].blockPins[iter].center) {
            // calculate contact center
            int pin_x = centerk.x;
            int pin_y = centerk.y;
            if (Blocks[iter2].H_flip)
              pin_x = mydesign.Blocks[iter2][0].width - pin_x;
            if (Blocks[iter2].V_flip)
              pin_y = mydesign.Blocks[iter2][0].height - pin_y;
  ...
  for (auto neti : mydesign.Nets) {
    ...
    for (auto connectedj : neti.connected) {
      if (connectedj.type == placerDB::Block) {
        ...
        if (mydesign.Blocks[connectedj.iter2][0].blockPins.size() > 0) {
          for (auto ci : mydesign.Blocks[connectedj.iter2][0]
                             .blockPins[connectedj.iter]
                             .center) {
            placerDB::point newp;
            newp.x = ci.x;
            newp.y = ci.y;
            if (Blocks[connectedj.iter2].H_flip)
              newp.x = mydesign.Blocks[connectedj.iter2][0].width - newp.x;
            if (Blocks[connectedj.iter2].V_flip)
              newp.y = mydesign.Blocks[connectedj.iter2][0].height - newp.y;
    ...
    std::vector<double> temp_feature =
        Calculate_Center_Point_feature(center_points);
    feature_value.push_back(temp_feature);
    double temp_sum = 0;
    for (unsigned int j = 0; j < neti.connected.size(); j++)
      temp_sum += neti.connected[j].alpha * temp_feature[j];
      ...
      for (unsigned int k = 0; k < constraintj.pins.size(); k++) {
        ...
        for (unsigned int m = 0; m < mydesign.Nets.size(); m++) {
          for (unsigned int n = 0; n < mydesign.Nets[m].connected.size(); n++) {
            if (mydesign.Nets[m].connected[n].iter2 ==
                    constraintj.pins[k].first &&
                mydesign.Nets[m].connected[n].iter ==
                    constraintj.pins[k].second) {
              index_i = m;
              index_j = n;
              break;
            }
...
bool ILP_solver::FrameSolveILP(design &mydesign, SeqPair &curr_sp,
                               PnRDB::Drc_info &drcInfo, bool flushbl,
                               const vector<placerDB::point> *prev) {
  auto logger =
      spdlog::default_logger()->clone("placer.ILP_solver.FrameSolveILP");
  ...
  for (int i = 0; i < mydesign.Blocks.size(); i++) {
    ...
    set_col_name(
        lp, i * 4 + 1,
        const_cast<char *>((mydesign.Blocks[i][0].name + "_x").c_str()));
    set_int(lp, i * 4 + 2, TRUE);
    set_col_name(
        lp, i * 4 + 2,
        const_cast<char *>((mydesign.Blocks[i][0].name + "_y").c_str()));
    set_int(lp, i * 4 + 3, TRUE);
    set_int(lp, i * 4 + 4, TRUE);
    set_binary(lp, i * 4 + 3, TRUE);
    set_col_name(
        lp, i * 4 + 3,
        const_cast<char *>((mydesign.Blocks[i][0].name + "_flx").c_str()));
    set_binary(lp, i * 4 + 4, TRUE);
    set_col_name(
        lp, i * 4 + 4,
        const_cast<char *>((mydesign.Blocks[i][0].name + "_fly").c_str()));
  }

  for (int i = 0; i < mydesign.Nets.size(); ++i) {
    int ind = i * 2 + mydesign.Blocks.size() * 4 + 1;
    set_col_name(lp, ind,
                 const_cast<char *>((mydesign.Nets[i].name + "_x").c_str()));
    set_col_name(lp, ind + 1,
                 const_cast<char *>((mydesign.Nets[i].name + "_y").c_str()));
  }
  ...
  for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) {
    int i_pos_index = find(curr_sp.posPair.begin(), curr_sp.posPair.end(), i) -
                      curr_sp.posPair.begin();
    int i_neg_index = find(curr_sp.negPair.begin(), curr_sp.negPair.end(), i) -
                      curr_sp.negPair.begin();
    for (unsigned int j = i + 1; j < mydesign.Blocks.size(); j++) {
      int j_pos_index =
          find(curr_sp.posPair.begin(), curr_sp.posPair.end(), j) -
          curr_sp.posPair.begin();
      int j_neg_index =
          find(curr_sp.negPair.begin(), curr_sp.negPair.end(), j) -
          curr_sp.negPair.begin();
      if (i_pos_index < j_pos_index) {
        if (i_neg_index < j_neg_index) {
          ...
          double sparserow[2] = {1, -1};
          int colno[2] = {int(i) * 4 + 1, int(j) * 4 + 1};
          if (find(mydesign.Abut_Constraints.begin(),
                   mydesign.Abut_Constraints.end(),
                   make_pair(make_pair(int(i), int(j)), placerDB::H)) !=
              mydesign.Abut_Constraints.end()) {
            if (!add_constraintex(
                    lp, 2, sparserow, colno, EQ,
                    -mydesign.Blocks[i][curr_sp.selected[i]].width - 0))
              logger->error("error");
          } else if (!add_constraintex(
                         lp, 2, sparserow, colno, LE,
                         -mydesign.Blocks[i][curr_sp.selected[i]].width -
                             bias_Hgraph))
            logger->error("error");
        } else {
          // i is above j
          ...
          if (find(mydesign.Abut_Constraints.begin(),
                   mydesign.Abut_Constraints.end(),
                   make_pair(make_pair(int(i), int(j)), placerDB::V)) !=
              mydesign.Abut_Constraints.end()) {
            if (!add_constraintex(
                    lp, 2, sparserow, colno, EQ,
                    mydesign.Blocks[j][curr_sp.selected[j]].height + 0))
              logger->error("error");
          } else if (!add_constraintex(
                         lp, 2, sparserow, colno, GE,
                         mydesign.Blocks[j][curr_sp.selected[j]].height +
                             bias_Vgraph))
            logger->error("error");
        }
      } else {
        if (i_neg_index < j_neg_index) {
          ...
          if (find(mydesign.Abut_Constraints.begin(),
                   mydesign.Abut_Constraints.end(),
                   make_pair(make_pair(int(j), int(i)), placerDB::V)) !=
              mydesign.Abut_Constraints.end()) {
            if (!add_constraintex(
                    lp, 2, sparserow, colno, EQ,
                    -mydesign.Blocks[i][curr_sp.selected[i]].height - 0))
              logger->error("error");
          } else if (!add_constraintex(
                         lp, 2, sparserow, colno, LE,
                         -mydesign.Blocks[i][curr_sp.selected[i]].height -
                             bias_Vgraph))
            logger->error("error");
        } else {
          // i is right of j
          ...
          if (find(mydesign.Abut_Constraints.begin(),
                   mydesign.Abut_Constraints.end(),
                   make_pair(make_pair(int(j), int(i)), placerDB::H)) !=
              mydesign.Abut_Constraints.end()) {
            if (!add_constraintex(
                    lp, 2, sparserow, colno, EQ,
                    mydesign.Blocks[j][curr_sp.selected[j]].width + 0))
              logger->error("error");
          } else if (!add_constraintex(
                         lp, 2, sparserow, colno, GE,
                         mydesign.Blocks[j][curr_sp.selected[j]].width +
                             bias_Hgraph))
            logger->error("error");
        }
      }
  ...
  if (flushbl) {
    ...
    for (auto id : curr_sp.negPair) {
      if (id < int(mydesign.Blocks.size())) {
        set_bounds(lp, (id * 4 + 1), -10 * minx,
                   -mydesign.Blocks[id][curr_sp.selected[id]].width);
        set_bounds(lp, (id * 4 + 2), -10 * miny,
                   -mydesign.Blocks[id][curr_sp.selected[id]].height);
      }
    }
  }
    ...
    if (SPBlock.axis_dir == placerDB::H) {
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        // x center of blocks in each pair are the same
        {
          ...
          int colno[2] = {first_id * 4 + 1, second_id * 4 + 1};
          int first_x_center =
              mydesign.Blocks[first_id][curr_sp.selected[first_id]].width / 2;
          int second_x_center =
              mydesign.Blocks[second_id][curr_sp.selected[second_id]].width / 2;
          add_constraintex(lp, 2, sparserow, colno, EQ,
                           -first_x_center + second_x_center);
        }
      }
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        int i_first_id = SPBlock.sympair[i].first,
            i_second_id = SPBlock.sympair[i].second;
        int i_first_y_center =
            mydesign.Blocks[i_first_id][curr_sp.selected[i_first_id]].height /
            4;
        int i_second_y_center =
            mydesign.Blocks[i_second_id][curr_sp.selected[i_second_id]].height /
            4;
        for (unsigned int j = i + 1; j < SPBlock.sympair.size(); j++) {
          // the y center of the two pairs are the same
          int j_first_id = SPBlock.sympair[j].first,
              j_second_id = SPBlock.sympair[j].second;
          int j_first_y_center =
              mydesign.Blocks[j_first_id][curr_sp.selected[j_first_id]].height /
              4;
          int j_second_y_center =
              mydesign.Blocks[j_second_id][curr_sp.selected[j_second_id]]
                  .height /
              4;
          double sparserow[4] = {0.5, 0.5, -0.5, -0.5};
          int colno[4] = {i_first_id * 4 + 2, i_second_id * 4 + 2,
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        for (unsigned int j = 0; j < SPBlock.selfsym.size(); j++) {
          ...
          int j_id = SPBlock.selfsym[j].first;
          int j_y_center =
              mydesign.Blocks[j_id][curr_sp.selected[j_id]].height / 2;
          double sparserow[3] = {0.5, 0.5, -1};
          int colno[3] = {i_first_id * 4 + 2, i_second_id * 4 + 2,
                          j_id * 4 + 2};
          int bias = -i_first_y_center - i_second_y_center + j_y_center;
      ...
      for (int i = 0; i < SPBlock.selfsym.size(); i++) {
        int i_id = SPBlock.selfsym[i].first;
        int i_y_center =
            mydesign.Blocks[i_id][curr_sp.selected[i_id]].height / 2;
        for (unsigned int j = i + 1; j < SPBlock.selfsym.size(); j++) {
          // the y center of the two selfsyms are the same
          int j_id = SPBlock.selfsym[j].first;
          int j_y_center =
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        // y center of blocks in each pair are the same
        {
          ...
          int colno[2] = {first_id * 4 + 2, second_id * 4 + 2};
          int first_y_center =
              mydesign.Blocks[first_id][curr_sp.selected[first_id]].height / 2;
          int second_y_center =
              mydesign.Blocks[second_id][curr_sp.selected[second_id]].height /
              2;
          add_constraintex(lp, 2, sparserow, colno, EQ,
                           -first_y_center + second_y_center);
        }
      }
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        int i_first_id = SPBlock.sympair[i].first,
            i_second_id = SPBlock.sympair[i].second;
        int i_first_x_center =
            mydesign.Blocks[i_first_id][curr_sp.selected[i_first_id]].width / 4;
        int i_second_x_center =
            mydesign.Blocks[i_second_id][curr_sp.selected[i_second_id]].width /
            4;
        for (unsigned int j = i + 1; j < SPBlock.sympair.size(); j++) {
          // the x center of the two pairs are the same
          int j_first_id = SPBlock.sympair[j].first,
              j_second_id = SPBlock.sympair[j].second;
          int j_first_x_center =
              mydesign.Blocks[j_first_id][curr_sp.selected[j_first_id]].width /
              4;
          int j_second_x_center =
              mydesign.Blocks[j_second_id][curr_sp.selected[j_second_id]]
                  .width /
              4;
          double sparserow[4] = {0.5, 0.5, -0.5, -0.5};
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        for (unsigned int j = 0; j < SPBlock.selfsym.size(); j++) {
          ...
          int j_id = SPBlock.selfsym[j].first;
          int j_x_center =
              mydesign.Blocks[j_id][curr_sp.selected[j_id]].width / 2;
          double sparserow[3] = {0.5, 0.5, -1};
          int colno[3] = {i_first_id * 4 + 1, i_second_id * 4 + 1,
                          j_id * 4 + 1};
          int bias = -i_first_x_center - i_second_x_center + j_x_center;
      ...
      for (int i = 0; i < SPBlock.selfsym.size(); i++) {
        int i_id = SPBlock.selfsym[i].first;
        int i_x_center =
            mydesign.Blocks[i_id][curr_sp.selected[i_id]].width / 2;
        for (unsigned int j = i + 1; j < SPBlock.selfsym.size(); j++) {
          // the x center of the two selfsyms are the same
          int j_id = SPBlock.selfsym[j].first;
          int j_x_center =
  ...
  for (auto alignment_unit : mydesign.Align_blocks) {
    for (unsigned int j = 0; j < alignment_unit.blocks.size() - 1; j++) {
      int first_id = alignment_unit.blocks[j],
          second_id = alignment_unit.blocks[j + 1];
      if (alignment_unit.horizon == 1) {
        if (alignment_unit.line == 0) {
          ...
          int colno[2] = {first_id * 4 + 2, second_id * 4 + 2};
          int bias =
              -mydesign.Blocks[first_id][curr_sp.selected[first_id]].height /
                  2 +
              mydesign.Blocks[second_id][curr_sp.selected[second_id]].height /
                  2;
          add_constraintex(lp, 2, sparserow, colno, EQ, bias);
        ...
        if (alignment_unit.line == 0) {
          ...
          int colno[2] = {first_id * 4 + 1, second_id * 4 + 1};
          int bias =
              -mydesign.Blocks[first_id][curr_sp.selected[first_id]].width +
              mydesign.Blocks[second_id][curr_sp.selected[second_id]].width;
          add_constraintex(lp, 2, sparserow, colno, EQ, bias);
        }
      }
    }
  ...
  // set_add_rowmode(lp, FALSE);
  {
        ...
        if (mydesign.Nets[i].connected[j].type == placerDB::Block &&
            (blockids.size() == 0 ||
             mydesign.Nets[i].connected[j].iter2 !=
                 curr_sp.negPair[blockids.back().first]))
          blockids.push_back(std::make_pair(
              find(curr_sp.negPair.begin(), curr_sp.negPair.end(),
                   mydesign.Nets[i].connected[j].iter2) -
                  curr_sp.negPair.begin(),
              mydesign.Nets[i].connected[j].iter));
      }
    ...
    for (unsigned int i = 0; i < mydesign.Nets.size(); i++) {
      ...
      for (unsigned int j = 0; j < mydesign.Nets[i].connected.size(); j++) {
        if (mydesign.Nets[i].connected[j].type == placerDB::Block) {
          block_pos_x_set.insert(std::make_pair(
              std::make_pair(mydesign.Nets[i].connected[j].iter2,
                             mydesign.Nets[i].connected[j].iter),
              find(curr_sp.negPair.begin(), curr_sp.negPair.end(),
                   mydesign.Nets[i].connected[j].iter2) -
                  curr_sp.negPair.begin()));
          block_pos_y_set.insert(std::make_pair(
              std::make_pair(mydesign.Nets[i].connected[j].iter2,
                             mydesign.Nets[i].connected[j].iter),
              find(curr_sp.negPair.begin(), curr_sp.negPair.end(),
                   mydesign.Nets[i].connected[j].iter2) -
      ...
      int Rblock_id = block_pos_x.back().first.first,
          Rpin_id = block_pos_x.back().first.second;
      int Lblock_width =
              mydesign.Blocks[Lblock_id][curr_sp.selected[Lblock_id]].width,
          Lblock_height =
              mydesign.Blocks[Lblock_id][curr_sp.selected[Lblock_id]].height;
      int Rblock_width =
              mydesign.Blocks[Rblock_id][curr_sp.selected[Rblock_id]].width,
          Rblock_height =
              mydesign.Blocks[Rblock_id][curr_sp.selected[Rblock_id]].height;
      int Lpin_x = mydesign.Blocks[Lblock_id][curr_sp.selected[Lblock_id]]
                               .blockPins.size() > 0
                       ? mydesign.Blocks[Lblock_id][curr_sp.selected[Lblock_id]]
                             .blockPins[Lpin_id]
                             .center.front()
                             .x
                       : mydesign.Blocks[Lblock_id][curr_sp.selected[Lblock_id]]
                                 .width /
                             2,
          Lpin_y = mydesign.Blocks[Lblock_id][curr_sp.selected[Lblock_id]]
                               .blockPins.size() > 0
                       ? mydesign.Blocks[Lblock_id][curr_sp.selected[Lblock_id]]
                             .blockPins[Lpin_id]
                             .center.front()
                       ...
                       : mydesign.Blocks[Lblock_id][curr_sp.selected[Lblock_id]]
                                 .height /
                             2;
      int Rpin_x = mydesign.Blocks[Rblock_id][curr_sp.selected[Rblock_id]]
                               .blockPins.size() > 0
                       ? mydesign.Blocks[Rblock_id][curr_sp.selected[Rblock_id]]
                             .blockPins[Rpin_id]
                             .center.front()
                             .x
                       : mydesign.Blocks[Rblock_id][curr_sp.selected[Rblock_id]]
                                 .width /
                             2,
          Rpin_y = mydesign.Blocks[Rblock_id][curr_sp.selected[Rblock_id]]
                               .blockPins.size() > 0
                       ? mydesign.Blocks[Rblock_id][curr_sp.selected[Rblock_id]]
                             .blockPins[Rpin_id]
                             .center.front()
      ...
      int Ublock_id = block_pos_y.back().first.first,
          Upin_id = block_pos_y.back().first.second;
      int Dblock_width =
              mydesign.Blocks[Dblock_id][curr_sp.selected[Dblock_id]].width,
          Dblock_height =
              mydesign.Blocks[Dblock_id][curr_sp.selected[Dblock_id]].height;
      int Ublock_width =
              mydesign.Blocks[Ublock_id][curr_sp.selected[Ublock_id]].width,
          Ublock_height =
              mydesign.Blocks[Ublock_id][curr_sp.selected[Ublock_id]].height;
      int Dpin_x = mydesign.Blocks[Dblock_id][curr_sp.selected[Dblock_id]]
                               .blockPins.size() > 0
                       ? mydesign.Blocks[Dblock_id][curr_sp.selected[Dblock_id]]
                             .blockPins[Dpin_id]
                             .center.front()
                             .x
                       : mydesign.Blocks[Dblock_id][curr_sp.selected[Dblock_id]]
                                 .width /
                             2,
          Dpin_y = mydesign.Blocks[Dblock_id][curr_sp.selected[Dblock_id]]
                               .blockPins.size() > 0
                       ? mydesign.Blocks[Dblock_id][curr_sp.selected[Dblock_id]]
                             .blockPins[Dpin_id]
                             .center.front()
                             .y
                       : mydesign.Blocks[Dblock_id][curr_sp.selected[Ublock_id]]
                                 .height /
                             2;
      int Upin_x = mydesign.Blocks[Ublock_id][curr_sp.selected[Ublock_id]]
                               .blockPins.size() > 0
                       ? mydesign.Blocks[Ublock_id][curr_sp.selected[Ublock_id]]
                             .blockPins[Upin_id]
                             .center.front()
                             .x
                       : mydesign.Blocks[Ublock_id][curr_sp.selected[Ublock_id]]
                                 .width /
                             2,
          Upin_y = mydesign.Blocks[Ublock_id][curr_sp.selected[Ublock_id]]
                               .blockPins.size() > 0
                       ? mydesign.Blocks[Ublock_id][curr_sp.selected[Ublock_id]]
                             .blockPins[Upin_id]
                             .center.front()
      ...
      if (Lblock_id != Rblock_id) {
        {
          ...
          int colno[5] = {Lblock_id * 4 + 1, Lblock_id * 4 + 3,
                          Rblock_id * 4 + 1, Rblock_id * 4 + 3,
                          int(mydesign.Blocks.size() * 4 + i * 2 + 1)};
          add_constraintex(lp, 5, sparserow, colno, LE, -Lpin_x + Rpin_x);
        }
        {
          double sparserow[5] = {
      ...
      if (Dblock_id != Ublock_id) {
        {
          ...
          int colno[5] = {Dblock_id * 4 + 2, Dblock_id * 4 + 4,
                          Ublock_id * 4 + 2, Ublock_id * 4 + 4,
                          int(mydesign.Blocks.size() * 4 + i * 2 + 2)};
          add_constraintex(lp, 5, sparserow, colno, LE, -Dpin_y + Upin_y);
        }
        {
          double sparserow[5] = {
    ...
    URblock_pos_id = find(curr_sp.posPair.begin(), curr_sp.posPair.end(),
                          curr_sp.negPair[URblock_neg_id]) -
                     curr_sp.posPair.begin();
    // estimate width
    for (int i = URblock_pos_id; i >= 0; i--) {
      if (curr_sp.posPair[i] < int(mydesign.Blocks.size())) {
        estimated_width += mydesign
                               .Blocks[curr_sp.posPair[i]]
                                      [curr_sp.selected[curr_sp.posPair[i]]]
                               .width;
      }
    }
    // add estimated area
    for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) {
      if (curr_sp.negPair[i] >= mydesign.Blocks.size())
        continue;
      row.at(curr_sp.negPair[i] * 4 + 2) +=
          ((flushbl ? estimated_width : -estimated_width) / 2);
    }
    // estimate height
    for (unsigned int i = URblock_pos_id; i < curr_sp.posPair.size(); i++) {
      if (curr_sp.posPair[i] < int(mydesign.Blocks.size())) {
        estimated_height += mydesign
                                .Blocks[curr_sp.posPair[i]]
                                       [curr_sp.selected[curr_sp.posPair[i]]]
                                .height;
      }
    ...
    for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) {
      if (curr_sp.negPair[i] >= mydesign.Blocks.size())
        continue;
      row.at(curr_sp.negPair[i] * 4 + 1) +=
          ((flushbl ? estimated_height : -estimated_height) / 2);
    }
    ...
    if (ret != 0 && ret != 1) {
      ...
      if (fail_cnt < 10) {
        write_lp(lp, const_cast<char*>((mydesign.name + "_fail_ilp_" +
      std::to_string(fail_cnt) + ".lp").c_str())); curr_sp.PrintSeqPair();
        std::string tmpstrpos, tmpstrneg;
        for (auto& it : curr_sp.posPair) if (it < mydesign.Blocks.size())
      tmpstrpos += (mydesign.Blocks[it][0].name + " "); for (auto& it :
      curr_sp.negPair) if (it < mydesign.Blocks.size()) tmpstrneg +=
      (mydesign.Blocks[it][0].name + " "); logger->info("DEBUG fail ILP seq pair
      : pos=[{0}] neg=[{1}]", tmpstrpos, tmpstrneg); logger->info("ILP fail
      {0}", fail_cnt); curr_sp.PrintSeqPair(mydesign);
        ++fail_cnt;
      }*/
      delete_lp(lp);
...
bool ILP_solver::MoveBlocksUsingSlack(const std::vector<Block> &blockslocal,
                                      design &mydesign, SeqPair &curr_sp,
                                      PnRDB::Drc_info &drcInfo) {
  ...
  for (unsigned i = 0; i < Blocks.size(); ++i) {
    ...
    if (slackxy[i].x < 0 || slackxy[i].y < 0)
      return false;
  }
  for (auto SPBlock : mydesign.SPBlocks) {
  ...
  if (!FrameSolveILP(mydesign, curr_sp, drcInfo, true, &blockpts))
    return false;
  return true;
}
...
double ILP_solver::GenerateValidSolution(design &mydesign, SeqPair &curr_sp,
  ...
  if (mydesign.leftAlign()) {
    ...
    if (!FrameSolveILP(mydesign, curr_sp, drcInfo, true))
      return -1;
    ...
    if (!FrameSolveILP(mydesign, curr_sp, drcInfo, false) ||
      ...
      // if unable to solve flush top/right or if the solution changed
      // significantly, use the bottom/left flush solution
      Blocks = blockslocal;
    }
  }
  ...
  for (int i = 0; i < mydesign.Blocks.size(); i++) {
    LL.x = std::min(LL.x, Blocks[i].x);
    LL.y = std::min(LL.y, Blocks[i].y);
    UR.x = std::max(UR.x, Blocks[i].x +
                              mydesign.Blocks[i][curr_sp.selected[i]].width);
    UR.y = std::max(UR.y, Blocks[i].y +
                              mydesign.Blocks[i][curr_sp.selected[i]].height);
  }
  // calculate area
  area = double(UR.x - LL.x) * double(UR.y - LL.y);
  ...
  for (auto neti : mydesign.Nets) {
    int HPWL_min_x = UR.x, HPWL_min_y = UR.y, HPWL_max_x = 0, HPWL_max_y = 0;
    int HPWL_extend_min_x = UR.x, HPWL_extend_min_y = UR.y,
        HPWL_extend_max_x = 0, HPWL_extend_max_y = 0;
    for (auto connectedj : neti.connected) {
      if (connectedj.type == placerDB::Block) {
        int iter2 = connectedj.iter2, iter = connectedj.iter;
        for (auto centerk : mydesign.Blocks[iter2][curr_sp.selected[iter2]]
                                .blockPins[iter]
                                .center) {
          // calculate contact center
          int pin_x = centerk.x;
          ...
          if (Blocks[iter2].H_flip)
            pin_x =
                mydesign.Blocks[iter2][curr_sp.selected[iter2]].width - pin_x;
          if (Blocks[iter2].V_flip)
            pin_y =
                mydesign.Blocks[iter2][curr_sp.selected[iter2]].height - pin_y;
          pin_x += Blocks[iter2].x;
          pin_y += Blocks[iter2].y;
          HPWL_min_x = std::min(HPWL_min_x, pin_x);
          HPWL_max_x = std::max(HPWL_max_x, pin_x);
        ...
        for (auto boundaryk : mydesign.Blocks[iter2][curr_sp.selected[iter2]]
                                  .blockPins[iter]
                                  .boundary) {
          int pin_llx = boundaryk.polygon[0].x,
              pin_urx = boundaryk.polygon[2].x;
          int pin_lly = boundaryk.polygon[0].y,
              pin_ury = boundaryk.polygon[2].y;
          if (Blocks[iter2].H_flip) {
            pin_llx = mydesign.Blocks[iter2][curr_sp.selected[iter2]].width -
                      boundaryk.polygon[2].x;
            pin_urx = mydesign.Blocks[iter2][curr_sp.selected[iter2]].width -
                      boundaryk.polygon[0].x;
          }
          if (Blocks[iter2].V_flip) {
            pin_lly = mydesign.Blocks[iter2][curr_sp.selected[iter2]].height -
                      boundaryk.polygon[2].y;
            pin_ury = mydesign.Blocks[iter2][curr_sp.selected[iter2]].height -
                      boundaryk.polygon[0].y;
          }
    ...
    HPWL += (HPWL_max_y - HPWL_min_y) + (HPWL_max_x - HPWL_min_x);
    HPWL_extend += (HPWL_extend_max_y - HPWL_extend_min_y) +
                   (HPWL_extend_max_x - HPWL_extend_min_x);
    ...
    if (is_terminal_net)
      HPWL_extend_terminal += (HPWL_extend_max_y - HPWL_extend_min_y) +
                              (HPWL_extend_max_x - HPWL_extend_min_x);
  }

  // HPWL norm
  if (!mydesign.Nets.empty())
    HPWL_norm = HPWL_extend / mydesign.GetMaxBlockHPWLSum() /
                double(mydesign.Nets.size());
  // calculate linear constraint
  linear_const = 0;
  std::vector<std::vector<double>> feature_value;
  for (auto neti : mydesign.Nets) {
    ...
    for (auto connectedj : neti.connected) {
      if (connectedj.type == placerDB::Block) {
        ...
        for (auto ci :
             mydesign
                 .Blocks[connectedj.iter2][curr_sp.selected[connectedj.iter2]]
                 .blockPins[connectedj.iter]
                 .center) {
          placerDB::point newp;
          newp.x = ci.x;
          ...
          if (Blocks[connectedj.iter2].H_flip)
            newp.x = mydesign
                         .Blocks[connectedj.iter2]
                                [curr_sp.selected[connectedj.iter2]]
                         .width -
                     newp.x;
          if (Blocks[connectedj.iter2].V_flip)
            newp.y = mydesign
    ...
    feature_value.push_back(temp_feature);
    double temp_sum = 0;
    for (int j = 0; j < neti.connected.size(); j++)
      temp_sum += neti.connected[j].alpha * temp_feature[j];
    temp_sum = std::max(temp_sum - neti.upperBound, double(0));
    linear_const += temp_sum;
  }
  ...
  if (!mydesign.Nets.empty())
    linear_const /=
        (mydesign.GetMaxBlockHPWLSum() * double(mydesign.Nets.size()));
...
double ILP_solver::GenerateValidSolution_select(design &mydesign,
                                                SeqPair &curr_sp,
                                                PnRDB::Drc_info &drcInfo) {
  auto logger = spdlog::default_logger()->clone(
      "placer.ILP_solver.GenerateValidSolution_select");
  ...
  vector<unsigned int> select_begin_id(mydesign.Blocks.size(),
                                       0); // begin id of select in var
  vector<unsigned int> pin_pos_begin_id(
      mydesign.Blocks.size()); // begin id of pin position in var
  vector<unsigned int> pin_aux_begin_id(
      mydesign.Blocks.size()); // begin id of pin auxiliary in var
  for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) {
  ...
  for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) {
    ...
    }
    {
      ...
      for (unsigned int j = 0; j < mydesign.Blocks[i].size(); j++)
        colno[j] = select_begin_id[i] + j;
      if (!add_constraintex(lp, mydesign.Blocks[i].size(), sparserow, colno, EQ,
                            1))
        logger->error("error");
    }
    {
      ...
      for (unsigned int j = 0; j < mydesign.Blocks[i].size(); j++)
        sparserow[j] = mydesign.Blocks[i][j].width;
      sparserow[mydesign.Blocks[i].size()] = -1;
      for (unsigned int j = 0; j < mydesign.Blocks[i].size(); j++)
        colno[j] = select_begin_id[i] + j;          // select id
      colno[mydesign.Blocks[i].size()] = i * 6 + 5; // width id
      if (!add_constraintex(lp, mydesign.Blocks[i].size() + 1, sparserow, colno,
                            EQ, 0))
        logger->error("error");
      // set height of a block
      for (unsigned int j = 0; j < mydesign.Blocks[i].size(); j++)
        sparserow[j] = mydesign.Blocks[i][j].height;
      sparserow[mydesign.Blocks[i].size()] = -1;
      for (unsigned int j = 0; j < mydesign.Blocks[i].size(); j++)
        colno[j] = select_begin_id[i] + j;          // select id
    ...
    for (unsigned int k = 0; k < mydesign.Blocks[i][0].blockPins.size(); k++) {
      ...
      // set x of a blockpin
      for (unsigned int j = 0; j < mydesign.Blocks[i].size(); j++)
        sparserow[j] = mydesign.Blocks[i][j].blockPins[k].center.front().x;
      sparserow[mydesign.Blocks[i].size()] = -1;
      for (unsigned int j = 0; j < mydesign.Blocks[i].size(); j++)
        colno[j] = select_begin_id[i] + j; // select id
      colno[mydesign.Blocks[i].size()] =
          pin_pos_begin_id[i] + 2 * k; // pin x id
      if (!add_constraintex(lp, mydesign.Blocks[i].size() + 1, sparserow, colno,
                            EQ, 0))
        logger->error("error");
      // set x of a blockpin
      for (unsigned int j = 0; j < mydesign.Blocks[i].size(); j++)
        sparserow[j] = mydesign.Blocks[i][j].blockPins[k].center.front().y;
      sparserow[mydesign.Blocks[i].size()] = -1;
      for (unsigned int j = 0; j < mydesign.Blocks[i].size(); j++)
        colno[j] = select_begin_id[i] + j; // select id
      colno[mydesign.Blocks[i].size()] =
          pin_pos_begin_id[i] + 2 * k + 1; // pin y id
      if (!add_constraintex(lp, mydesign.Blocks[i].size() + 1, sparserow, colno,
                            EQ, 0))
        logger->error("error");
    }
    for (unsigned int k = 0; k < mydesign.Blocks[i][0].blockPins.size(); k++) {
      // y1+y2=flipx*(width-pinx)+(1-flipx)*pinx
      {
        ...
        if (!add_constraintex(lp, 2, sparserow, colno, LE, 0))
          logger->error("error");
      }
      {
        // y1<=width-pinx
        double sparserow[3] = {1, -1, 1};
        int colno[3] = {int(pin_aux_begin_id[i] + 4 * k), int(i) * 6 + 5,
                        int(pin_pos_begin_id[i] + 2 * k)};
        if (!add_constraintex(lp, 3, sparserow, colno, LE, 0))
          logger->error("error");
      }
      {
        // y1>=width-pinx-Mwidth*(1-flipx)
        double sparserow[4] = {1, -1, 1, double(-Mwidth)};
        int colno[4] = {int(pin_aux_begin_id[i] + 4 * k), int(i) * 6 + 5,
                        int(pin_pos_begin_id[i] + 2 * k), int(i) * 6 + 3};
        if (!add_constraintex(lp, 4, sparserow, colno, GE, -Mwidth))
          logger->error("error");
      }
      {
        // y1>=0
      ...
      }
      {
        ...
        if (!add_constraintex(lp, 2, sparserow, colno, LE, Mwidth))
          logger->error("error");
      }
      {
        // y2<=pinx
        double sparserow[2] = {1, -1};
        int colno[2] = {int(pin_aux_begin_id[i] + 4 * k + 1),
                        int(pin_pos_begin_id[i] + 2 * k)};
        if (!add_constraintex(lp, 2, sparserow, colno, LE, 0))
          logger->error("error");
      }
      {
        ...
        int colno[3] = {int(pin_aux_begin_id[i] + 4 * k + 1),
                        int(pin_pos_begin_id[i] + 2 * k), int(i) * 6 + 3};
        if (!add_constraintex(lp, 3, sparserow, colno, GE, 0))
          logger->error("error");
      }
      {
        // y2>=0
      ...
      }
      {
        // y1<=height-piny
        double sparserow[3] = {1, -1, 1};
        int colno[3] = {int(pin_aux_begin_id[i] + 4 * k + 2), int(i) * 6 + 6,
                        int(pin_pos_begin_id[i] + 2 * k + 1)};
        if (!add_constraintex(lp, 3, sparserow, colno, LE, 0))
          logger->error("error");
      }
      {
        ...
        int colno[4] = {int(pin_aux_begin_id[i] + 4 * k + 2), int(i) * 6 + 6,
                        int(pin_pos_begin_id[i] + 2 * k + 1), int(i) * 6 + 4};
        if (!add_constraintex(lp, 4, sparserow, colno, GE, -Mheight))
          logger->error("error");
      }
      {
        // y1>=0
      ...
      }
      {
        ...
        if (!add_constraintex(lp, 2, sparserow, colno, LE, Mheight))
          logger->error("error");
      }
      {
        // y2<=piny
        double sparserow[2] = {1, -1};
        int colno[2] = {int(pin_aux_begin_id[i] + 4 * k + 3),
                        int(pin_pos_begin_id[i] + 2 * k + 1)};
        if (!add_constraintex(lp, 2, sparserow, colno, LE, 0))
          logger->error("error");
      }
      {
        ...
        int colno[3] = {int(pin_aux_begin_id[i] + 4 * k + 3),
                        int(pin_pos_begin_id[i] + 2 * k + 1), int(i) * 6 + 4};
        if (!add_constraintex(lp, 3, sparserow, colno, GE, 0))
          logger->error("error");
      }
      ...
      if (i_pos_index < j_pos_index) {
        if (i_neg_index < j_neg_index) {
          ...
          if (find(mydesign.Abut_Constraints.begin(),
                   ...
                   make_pair(make_pair(int(i), int(j)), placerDB::H)) !=
              mydesign.Abut_Constraints.end()) {
            if (!add_constraintex(lp, 3, sparserow, colno, EQ, 0))
              logger->error("error");
          } else if (!add_constraintex(lp, 3, sparserow, colno, LE,
                                       0 - bias_Hgraph))
            logger->error("error");
        } else {
          // i is above j
          ...
          if (find(mydesign.Abut_Constraints.begin(),
            ...
            if (!add_constraintex(lp, 3, sparserow, colno, EQ, 0))
              logger->error("error");
          } else if (!add_constraintex(lp, 3, sparserow, colno, GE,
                                       bias_Vgraph))
            logger->error("error");
        }
      } else {
    ...
    if (SPBlock.axis_dir == placerDB::H) {
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        // x center of blocks in each pair are the same
        {
          ...
          int colno[4] = {first_id * 6 + 1, second_id * 6 + 1, first_id * 6 + 5,
                          second_id * 6 + 5};
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        int i_first_id = SPBlock.sympair[i].first,
            i_second_id = SPBlock.sympair[i].second;
        // int i_first_y_center =
        // mydesign.Blocks[i_first_id][curr_sp.selected[i_first_id]].height / 4;
        // int i_second_y_center =
        // mydesign.Blocks[i_second_id][curr_sp.selected[i_second_id]].height /
        // 4;
        for (unsigned int j = i + 1; j < SPBlock.sympair.size(); j++) {
          ...
          // int j_first_y_center =
          // mydesign.Blocks[j_first_id][curr_sp.selected[j_first_id]].height /
          // 4; int j_second_y_center =
          // mydesign.Blocks[j_second_id][curr_sp.selected[j_second_id]].height
          // / 4;
          double sparserow[8] = {0.5,  0.5,  -0.5,  -0.5,
                                 0.25, 0.25, -0.25, -0.25};
          int colno[8] = {i_first_id * 6 + 2, i_second_id * 6 + 2,
                          j_first_id * 6 + 2, j_second_id * 6 + 2,
                          i_first_id * 6 + 6, i_second_id * 6 + 6,
                          j_first_id * 6 + 6, j_second_id * 6 + 6};
          // int bias = -i_first_y_center - i_second_y_center + j_first_y_center
          // + j_second_y_center;
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        for (unsigned int j = 0; j < SPBlock.selfsym.size(); j++) {
          ...
          int colno[6] = {i_first_id * 6 + 2,  i_second_id * 6 + 2,
                          j_id * 6 + 2,        i_first_id * 6 + 6,
                          i_second_id * 6 + 6, j_id * 6 + 6};
          // int bias = -i_first_y_center - i_second_y_center + j_y_center;
      ...
      for (int i = 0; i < SPBlock.selfsym.size(); i++) {
        int i_id = SPBlock.selfsym[i].first;
        // int i_y_center = mydesign.Blocks[i_id][curr_sp.selected[i_id]].height
        // / 2;
        for (unsigned int j = i + 1; j < SPBlock.selfsym.size(); j++) {
          // the y center of the two selfsyms are the same
          int j_id = SPBlock.selfsym[j].first;
          ...
          // mydesign.Blocks[j_id][curr_sp.selected[j_id]].height / 2;
          double sparserow[4] = {1, -1, 0.5, -0.5};
          int colno[4] = {i_id * 6 + 2, j_id * 6 + 2, i_id * 6 + 6,
                          j_id * 6 + 6};
          // int bias = -i_y_center + j_y_center;
          add_constraintex(lp, 4, sparserow, colno, EQ, 0);
        }
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        {
          double sparserow[4] = {1, -1, 0.5, -0.5};
          int colno[4] = {first_id * 6 + 2, second_id * 6 + 2, first_id * 6 + 6,
                          second_id * 6 + 6};
          // int first_y_center =
          // mydesign.Blocks[first_id][curr_sp.selected[first_id]].height / 2;
          // int second_y_center =
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        for (unsigned int j = i + 1; j < SPBlock.sympair.size(); j++) {
          ...
          // int j_first_x_center =
          // mydesign.Blocks[j_first_id][curr_sp.selected[j_first_id]].width /
          // 4; int j_second_x_center =
          // mydesign.Blocks[j_second_id][curr_sp.selected[j_second_id]].width /
          // 4;
          double sparserow[8] = {0.5,  0.5,  -0.5,  -0.5,
                                 0.25, 0.25, -0.25, -0.25};
          int colno[8] = {i_first_id * 6 + 1, i_second_id * 6 + 1,
                          j_first_id * 6 + 1, j_second_id * 6 + 1,
                          i_first_id * 6 + 5, i_second_id * 6 + 5,
                          j_first_id * 6 + 5, j_second_id * 6 + 5};
          // int bias = -i_first_x_center - i_second_x_center + j_first_x_center
          // + j_second_x_center;
          add_constraintex(lp, 8, sparserow, colno, EQ, 0);
        }
      ...
      for (int i = 0; i < SPBlock.sympair.size(); i++) {
        ...
        for (unsigned int j = 0; j < SPBlock.selfsym.size(); j++) {
          ...
          // mydesign.Blocks[j_id][curr_sp.selected[j_id]].width / 2;
          double sparserow[6] = {0.5, 0.5, -1, 0.25, 0.25, -0.5};
          int colno[6] = {i_first_id * 6 + 1,  i_second_id * 6 + 1,
                          j_id * 6 + 1,        i_first_id * 6 + 5,
                          i_second_id * 6 + 5, j_id * 6 + 5};
          // int bias = -i_first_x_center - i_second_x_center + j_x_center;
          add_constraintex(lp, 6, sparserow, colno, EQ, 0);
        }
      }
      ...
      for (int i = 0; i < SPBlock.selfsym.size(); i++) {
        int i_id = SPBlock.selfsym[i].first;
        // int i_x_center = mydesign.Blocks[i_id][curr_sp.selected[i_id]].width
        // / 2;
        for (unsigned int j = i + 1; j < SPBlock.selfsym.size(); j++) {
          // the x center of the two selfsyms are the same
          int j_id = SPBlock.selfsym[j].first;
          ...
          // mydesign.Blocks[j_id][curr_sp.selected[j_id]].width / 2;
          double sparserow[4] = {1, -1, 0.5, -0.5};
          int colno[4] = {i_id * 6 + 1, j_id * 6 + 1, i_id * 6 + 5,
                          j_id * 6 + 5};
          // int bias = -i_x_center + j_x_center;
          add_constraintex(lp, 4, sparserow, colno, EQ, 0);
        }
  ...
  for (auto group : mydesign.Same_Template_Constraints) {
    ...
    for (unsigned int i = 0; i < group.size() - 1; i++) {
      for (unsigned int j = 0; j < mydesign.Blocks[group_vec[i]].size(); j++) {
        double sparserow[2] = {1, -1};
        int colno[2] = {int(select_begin_id[group_vec[i]] + j),
                        int(select_begin_id[group_vec[i] + 1] + j)};
        add_constraintex(lp, 2, sparserow, colno, EQ, 0);
      }
    }
  }
  ...
  // set_add_rowmode(lp, FALSE);
  {
    ...
    for (unsigned int i = 0; i < mydesign.Nets.size(); i++) {
      ...
      mydesign.Blocks[Lblock_id][curr_sp.selected[Lblock_id]].width,
          Lblock_height =
      mydesign.Blocks[Lblock_id][curr_sp.selected[Lblock_id]].height; int
      Rblock_width =
      mydesign.Blocks[Rblock_id][curr_sp.selected[Rblock_id]].width,
          Rblock_height =
      mydesign.Blocks[Rblock_id][curr_sp.selected[Rblock_id]].height; int Lpin_x
      =
      ...
      mydesign.Blocks[Dblock_id][curr_sp.selected[Dblock_id]].width,
          Dblock_height =
      mydesign.Blocks[Dblock_id][curr_sp.selected[Dblock_id]].height; int
      Ublock_width =
      mydesign.Blocks[Ublock_id][curr_sp.selected[Ublock_id]].width,
          Ublock_height =
      mydesign.Blocks[Ublock_id][curr_sp.selected[Ublock_id]].height; int Dpin_x
      =
    ...
    // add estimated area
    for (unsigned int i = 0; i < mydesign.Blocks.size(); i++)
      row[curr_sp.negPair[i] * 6 + 2] += estimated_width / 2;
    // estimate height
    for (unsigned int i = URblock_pos_id; i < curr_sp.posPair.size(); i++) {
      if (curr_sp.posPair[i] < int(mydesign.Blocks.size())) {
    ...
    // add estimated area
    for (unsigned int i = 0; i < mydesign.Blocks.size(); i++)
      row[curr_sp.negPair[i] * 6 + 1] += estimated_height / 2;
...
double ILP_solver::CalculateCost(design &mydesign) {
  ...
  for (auto mbi : mydesign.Match_blocks) {
    match_cost += abs(Blocks[mbi.blockid1].x +
                      mydesign.Blocks[mbi.blockid1][0].width / 2 -
                      Blocks[mbi.blockid2].x -
                      mydesign.Blocks[mbi.blockid2][0].width / 2) +
                  abs(Blocks[mbi.blockid1].y +
                      mydesign.Blocks[mbi.blockid1][0].height / 2 -
                      Blocks[mbi.blockid2].y -
...
double ILP_solver::CalculateCost(design &mydesign, SeqPair &curr_sp) {
  auto logger =
      spdlog::default_logger()->clone("placer.ILP_solver.CalculateCost");
  ...
  for (auto mbi : mydesign.Match_blocks) {
    match_cost +=
        (abs(Blocks[mbi.blockid1].x +
             mydesign.Blocks[mbi.blockid1][curr_sp.selected[mbi.blockid1]]
                     .width /
                 2 -
             Blocks[mbi.blockid2].x -
             mydesign.Blocks[mbi.blockid2][curr_sp.selected[mbi.blockid2]]
                     .width /
                 2) +
         abs(Blocks[mbi.blockid1].y +
             mydesign.Blocks[mbi.blockid1][curr_sp.selected[mbi.blockid1]]
  ...
  if (!mydesign.Match_blocks.empty())
    match_cost /= (mydesign.Match_blocks.size());
  constraint_penalty = match_cost * const_graph.BETA +
...
void ILP_solver::WritePlacement(design &mydesign, SeqPair &curr_sp,
                                string outfile) {
  ofstream fout;
  fout.open(outfile.c_str());
  // cout << "Placer-Info: write placement" << endl;
  fout << "# TAMU blocks 1.0" << endl << endl;
  fout << "DIE {" << LL.x << ", " << LL.y << "} {" << UR.x << "," << UR.y << "}"
       << endl
       << endl;
  for (unsigned int i = 0; i < mydesign.Blocks.size(); ++i) {
    string ort;
    ...
    fout << mydesign.Blocks.at(i).back().name << "\t" << Blocks[i].x << "\t"
         << Blocks[i].y << "\t" << ort << endl;
  }
  fout << endl;
  for (auto ni : mydesign.Nets) {
    ...
    for (auto ci : ni.connected) {
      if (ci.type == placerDB::Terminal) {
        int tno = ci.iter;
        fout << mydesign.Terminals.at(tno).name << "\t"
             << mydesign.Terminals.at(tno).center.x << "\t"
             << mydesign.Terminals.at(tno).center.y << endl;
        break;
      }
    }
  }
...
void ILP_solver::PlotPlacementAnalytical(design &mydesign, string outfile,
                                         bool plot_pin, bool plot_terminal,
                                         bool plot_net) {
  // cout << "Placer-Info: create gnuplot file" << endl;
  placerDB::point p, bp;
  if (!mydesign.is_first_ILP) {
    ofstream f("Results/" + mydesign.name + "_gds/" + mydesign.name + ".csv",
               std::ios::app);
    if (f.is_open()) {
  ...
  fout << "#Use this file as a script for gnuplot\n#(See "
          "http://www.gnuplot.info/ for details)"
       << endl;
  fout << "\nset title \" " << mydesign.name
       << " #Blocks= " << mydesign.Blocks.size()
       << ", #Terminals= " << mydesign.Terminals.size()
       << ", #Nets= " << mydesign.Nets.size() << ",Area=" << area
       << ", HPWL= " << HPWL << " \"" << endl;
  fout << "\nset nokey" << endl;
  ...
  for (unsigned int i = 0; i < mydesign.Blocks.size(); ++i) {
    ...
    if (mydesign.Blocks[i][0].width > 0 && mydesign.Blocks[i][0].height > 0)
      fout << "\nset label \"" << mydesign.Blocks[i][0].name << "\" at " << tp.x
           << " , " << tp.y << " center " << endl;
    if (plot_pin) {
      for (unsigned int j = 0; j < mydesign.Blocks[i][0].blockPins.size();
           j++) {
        for (unsigned int k = 0;
             k < mydesign.Blocks[i][0].blockPins[j].center.size(); k++) {
          placerDB::point newp;
          newp.x = mydesign.Blocks[i][0].blockPins[j].center[k].x;
          newp.y = mydesign.Blocks[i][0].blockPins[j].center[k].y;
          if (Blocks[i].H_flip)
            newp.x = mydesign.Blocks[i][0].width - newp.x;
          if (Blocks[i].V_flip)
            newp.y = mydesign.Blocks[i][0].height - newp.y;
          newp.x += Blocks[i].x;
          newp.y += Blocks[i].y;
          fout << "\nset label \"" << mydesign.Blocks[i][0].blockPins[j].name
               << "\" at " << newp.x << " , " << newp.y << endl;
          fout << endl;
        }
      }
    }
  ...
  if (plot_terminal) {
    for (auto ni : mydesign.Nets) {
      ...
      for (auto ci : ni.connected) {
        if (ci.type == placerDB::Terminal) {
          int tno = ci.iter;
          fout << "\nset label \"" << mydesign.Terminals.at(tno).name
               << "\" at " << mydesign.Terminals.at(tno).center.x << " , "
               << mydesign.Terminals.at(tno).center.y
               << " center                " << endl;
          break;
        }
      }
  ...
  if (plot_pin)
    fout << ", \'-\' with lines linestyle 7";
  if (plot_terminal)
    fout << ", \'-\' with lines linestyle 1";
  if (plot_net)
    fout << ", \'-\' with lines linestyle 0";
  fout << endl << endl;
  for (unsigned int i = 0; i < mydesign.Blocks.size(); ++i) {
    vector<placerDB::point> newp = mydesign.Blocks[i][0].boundary.polygon;
    fout << "# block " << mydesign.Blocks[i][0].name << " select " << 0
         << " bsize " << newp.size() << endl;
    for (unsigned int it = 0; it < newp.size(); it++) {
      fout << "\t" << newp[it].x + Blocks[i].x << "\t"
           << newp[it].y + Blocks[i].y << endl;
    }
    fout << "\t" << newp[0].x + Blocks[i].x << "\t" << newp[0].y + Blocks[i].y
         << endl;
    fout << endl;
  }
  fout << "\nEOF" << endl;
  ...
  if (plot_pin) {
    for (unsigned int i = 0; i < mydesign.Blocks.size(); ++i) {
      for (unsigned int j = 0; j < mydesign.Blocks[i][0].blockPins.size();
           j++) {
        for (unsigned int k = 0;
             k < mydesign.Blocks[i][0].blockPins[j].boundary.size(); k++) {
          for (unsigned int it = 0;
               it <
               mydesign.Blocks[i][0].blockPins[j].boundary[k].polygon.size();
               it++) {
            placerDB::point newp;
            newp.x =
                mydesign.Blocks[i][0].blockPins[j].boundary[k].polygon[it].x;
            newp.y =
                mydesign.Blocks[i][0].blockPins[j].boundary[k].polygon[it].y;
            if (Blocks[i].H_flip)
              newp.x = mydesign.Blocks[i][0].width - newp.x;
            if (Blocks[i].V_flip)
              newp.y = mydesign.Blocks[i][0].height - newp.y;
    ...
    for (auto ni : mydesign.Nets) {
      ...
      for (auto ci : ni.connected) {
        if (ci.type == placerDB::Terminal) {
          ...
          int bias = 20;
          fout << endl;
          fout << "\t" << mydesign.Terminals.at(tno).center.x - bias << "\t"
               << mydesign.Terminals.at(tno).center.y - bias << endl;
          fout << "\t" << mydesign.Terminals.at(tno).center.x - bias << "\t"
               << mydesign.Terminals.at(tno).center.y + bias << endl;
          fout << "\t" << mydesign.Terminals.at(tno).center.x + bias << "\t"
               << mydesign.Terminals.at(tno).center.y + bias << endl;
  ...
  // plot nets
  if (plot_net) {
    for (vector<placerDB::net>::iterator ni = mydesign.Nets.begin();
         ni != mydesign.Nets.end(); ++ni) {
      placerDB::point tp;
      ...
      for (auto ci : ni->connected) {
        if (ci.type == placerDB::Block) {
          if (mydesign.Blocks[ci.iter2][0].blockPins.size() > 0) {
            if (mydesign.Blocks[ci.iter2][0].blockPins[ci.iter].center.size() >
                0) {
              tp.x =
                  mydesign.Blocks[ci.iter2][0].blockPins[ci.iter].center[0].x;
              tp.y =
                  mydesign.Blocks[ci.iter2][0].blockPins[ci.iter].center[0].y;
              if (Blocks[ci.iter2].H_flip)
                tp.x = mydesign.Blocks[ci.iter2][0].width - tp.x;
              if (Blocks[ci.iter2].V_flip)
                tp.y = mydesign.Blocks[ci.iter2][0].height - tp.y;
...
void ILP_solver::PlotPlacement(design &mydesign, SeqPair &curr_sp,
                               string outfile) {
  // cout << "Placer-Info: create gnuplot file" << endl;
  placerDB::point p, bp;
  ofstream fout;
  ...
  fout << "#Use this file as a script for gnuplot\n#(See "
          "http://www.gnuplot.info/ for details)"
       << endl;
  fout << "\nset title\" #Blocks= " << mydesign.Blocks.size()
       << ", #Terminals= " << mydesign.Terminals.size()
       << ", #Nets= " << mydesign.Nets.size() << ",Area=" << area
       << ", HPWL= " << HPWL_extend << " \"" << endl;
  fout << "\nset nokey" << endl;
  fout << "#   Uncomment these two lines starting with \"set\"" << endl;
  fout << "#   to save an EPS file for inclusion into a latex document" << endl;
  fout << "# set terminal postscript eps color solid 20" << endl;
  ...
  for (int i = 0; i < mydesign.Blocks.size(); ++i) {
    ...
    fout << "\nset label \"" << mydesign.Blocks[i][curr_sp.selected[i]].name
         << "\" at " << tp.x << " , " << tp.y << " center " << endl;
    for (unsigned int j = 0;
         j < mydesign.Blocks[i][curr_sp.selected[i]].blockPins.size(); j++) {
      for (unsigned int k = 0;
           k <
           mydesign.Blocks[i][curr_sp.selected[i]].blockPins[j].center.size();
           k++) {
        placerDB::point newp;
        newp.x =
            mydesign.Blocks[i][curr_sp.selected[i]].blockPins[j].center[k].x;
        newp.y =
            mydesign.Blocks[i][curr_sp.selected[i]].blockPins[j].center[k].y;
        if (Blocks[i].H_flip)
          newp.x = mydesign.Blocks[i][curr_sp.selected[i]].width - newp.x;
        if (Blocks[i].V_flip)
          newp.y = mydesign.Blocks[i][curr_sp.selected[i]].height - newp.y;
        newp.x += Blocks[i].x;
        newp.y += Blocks[i].y;
        fout << "\nset label \""
             << mydesign.Blocks[i][curr_sp.selected[i]].blockPins[j].name
             << "\" at " << newp.x << " , " << newp.y << endl;
        fout << endl;
      }
    }
  ...
  for (auto ni : mydesign.Nets) {
    ...
    for (auto ci : ni.connected) {
      if (ci.type == placerDB::Terminal) {
        int tno = ci.iter;
        fout << "\nset label \"" << mydesign.Terminals.at(tno).name << "\" at "
             << mydesign.Terminals.at(tno).center.x << " , "
             << mydesign.Terminals.at(tno).center.y << " center                "
             << endl;
        break;
      }
    }
  ...
  // plot blocks
  fout << "\nplot[:][:] \'-\' with lines linestyle 3, \'-\' with lines "
          "linestyle 7, \'-\' with lines linestyle 1, \'-\' with lines "
          "linestyle 0"
       << endl
       << endl;
  for (unsigned int i = 0; i < mydesign.Blocks.size(); ++i) {
    vector<placerDB::point> newp =
        mydesign.Blocks[i][curr_sp.selected[i]].boundary.polygon;
    fout << "# block " << mydesign.Blocks[i][curr_sp.selected[i]].name
         << " select " << curr_sp.selected[i] << " bsize " << newp.size()
         << endl;
    for (unsigned int it = 0; it < newp.size(); it++) {
      fout << "\t" << newp[it].x + Blocks[i].x << "\t"
           << newp[it].y + Blocks[i].y << endl;
  ...
  for (unsigned int i = 0; i < mydesign.Blocks.size(); ++i) {
    for (unsigned int j = 0;
      ...
      for (unsigned int k = 0;
           k <
           mydesign.Blocks[i][curr_sp.selected[i]].blockPins[j].boundary.size();
           k++) {
        for (unsigned int it = 0; it < mydesign.Blocks[i][curr_sp.selected[i]]
                                           .blockPins[j]
                                           .boundary[k]
                                           .polygon.size();
             it++) {
          placerDB::point newp;
          newp.x = mydesign.Blocks[i][curr_sp.selected[i]]
                       .blockPins[j]
                       .boundary[k]
                       .polygon[it]
                       .x;
          newp.y = mydesign.Blocks[i][curr_sp.selected[i]]
                       .blockPins[j]
                       .boundary[k]
                       .polygon[it]
                       .y;
  ...
  for (vector<placerDB::net>::iterator ni = mydesign.Nets.begin();
    ...
    for (auto ci : ni->connected) {
      if (ci.type == placerDB::Block) {
        if (mydesign.Blocks[ci.iter2][curr_sp.selected[ci.iter2]]
                .blockPins[ci.iter]
                .center.size() > 0) {
          tp.x = mydesign.Blocks[ci.iter2][curr_sp.selected[ci.iter2]]
                     .blockPins[ci.iter]
                     .center[0]
                     .x;
          tp.y = mydesign.Blocks[ci.iter2][curr_sp.selected[ci.iter2]]
                     .blockPins[ci.iter]
                     .center[0]
                     .y;
          if (Blocks[ci.iter2].H_flip)
            tp.x = mydesign.Blocks[ci.iter2][curr_sp.selected[ci.iter2]].width -
                   tp.x;
          if (Blocks[ci.iter2].V_flip)
            tp.y =
                mydesign.Blocks[ci.iter2][curr_sp.selected[ci.iter2]].height -
                tp.y;
          tp.x += Blocks[ci.iter2].x;
          tp.y += Blocks[ci.iter2].y;
          pins.push_back(tp);
...
std::vector<double> ILP_solver::Calculate_Center_Point_feature(
    std::vector<std::vector<placerDB::point>> &temp_contact) {
  ...
  for (unsigned int i = 0; i < temp_x.size(); i++) {
    feature.push_back(abs(center_x - (double)temp_x[i]) +
                      abs(center_y - (double)temp_y[i]));
  }
  return feature;
}
...
void ILP_solver::updateTerminalCenterAnalytical(design &mydesign) {
  auto logger =
      spdlog::default_logger()->clone("placer.ILP_solver.updateTerminalCenter");
  ...
  for (unsigned int i = 0; i < mydesign.Terminals.size(); i++) {
    if (solved_terminals.find(i) != solved_terminals.end())
      continue;
    ...
    if (netIdx < 0 || netIdx >= mydesign.Nets.size()) {
      logger->error(
          "Placer-Warning: terminal {0} is dangling; set it on origin", i);
    ...
    alpha *=
        mydesign.Nets.at(netIdx)
            .weight;   // add weight to reflect the modification for bigMacros
    if (sbIdx != -1) { // in symmetry group
      placerDB::Smark axis = mydesign.SBlocks[sbIdx].axis_dir;
      if (cp == (int)i) { // self-symmetric
        if (axis == placerDB::V) {
          int axis_X =
              Blocks[mydesign.SBlocks[sbIdx].selfsym[0].first].x +
              mydesign.Blocks[mydesign.SBlocks[sbIdx].selfsym[0].first][0]
                      .width /
                  2;
          ...
          for (auto ci : mydesign.Nets[netIdx].connected) {
            if (ci.type == placerDB::Block) {
              ...
              for (unsigned int k = 0; k < mydesign.Blocks[ci.iter2][0]
                                               .blockPins[ci.iter]
                                               .center.size();
                   k++) {
                p = mydesign.Blocks[ci.iter2][0].blockPins[ci.iter].center[k];
                if (Blocks[ci.iter2].H_flip)
                  p.x = mydesign.Blocks[ci.iter2][0].width - p.x;
                if (Blocks[ci.iter2].V_flip)
                  p.y = mydesign.Blocks[ci.iter2][0].height - p.y;
                p += bp;
                if (p.y < distTerm) {
                  distTerm = p.y;
                  tp.y = 0;
          ...
          int axis_Y =
              Blocks[mydesign.SBlocks[sbIdx].selfsym[0].first].y +
              mydesign.Blocks[mydesign.SBlocks[sbIdx].selfsym[0].first][0]
                      .height /
                  2;
          int distTerm = INT_MAX;
        ...
        if (solved_terminals.find(cp) != solved_terminals.end())
          continue;
        solved_terminals.insert(cp);
        int netIdx2 = mydesign.Terminals.at(cp).netIter;
        if (netIdx2 < 0 or netIdx2 >= mydesign.Nets.size()) {
          logger->error("Placer-Error: terminal {0} is not dangling, but its "
                        "counterpart {1} is dangling; set them on origin",
                        i, cp);
          mydesign.Terminals.at(i).center = {0, 0};
        ...
        if (axis == placerDB::V) {
          ...
          if (distTermL * alpha + distTermR2 * alpha2 <
              distTermR * alpha + distTermL2 * alpha2) {
          ...
          if (distTermL * alpha + distTermU2 * alpha2 <
              distTermU * alpha + distTermL2 * alpha2) {
            mydesign.Terminals.at(i).center = tpL1;
      ...
      if (tar != -1) { // specifiy PortLocation constraint
        ...
        for (auto ci : mydesign.Nets.at(netIdx).connected) {
          if (ci.type == placerDB::Block) {
            ...
            for (unsigned int k = 0;
                 k <
                 mydesign.Blocks[ci.iter2][0].blockPins[ci.iter].center.size();
                 k++) {
              p = mydesign.Blocks[ci.iter2][0].blockPins[ci.iter].center[k];
              if (Blocks[ci.iter2].H_flip)
                p.x = mydesign.Blocks[ci.iter2][0].width - p.x;
    ...
    if (sbIdx != -1) { // in symmetry group
      ...
      if (cp == (int)i) { // self-symmetric
        if (axis == placerDB::V) {
          int axis_X =
              Blocks[mydesign.SBlocks[sbIdx].selfsym[0].first].x +
              mydesign.Blocks[mydesign.SBlocks[sbIdx].selfsym[0].first]
                             [curr_sp.selected
                                  [mydesign.SBlocks[sbIdx].selfsym[0].first]]
                                 .width /
                  2;
          int distTerm = INT_MAX;
          placerDB::point tp(axis_X, 0);
          for (auto ci : mydesign.Nets[netIdx].connected) {
            if (ci.type == placerDB::Block) {
              bp = placerDB::point(Blocks[ci.iter2].x, Blocks[ci.iter2].y);
              for (unsigned int k = 0;
                   k < mydesign.Blocks[ci.iter2][curr_sp.selected[ci.iter2]]
                           .blockPins[ci.iter]
                           .center.size();
                   k++) {
                p = mydesign.Blocks[ci.iter2][curr_sp.selected[ci.iter2]]
                        .blockPins[ci.iter]
                        .center[k];
                if (Blocks[ci.iter2].H_flip)
                  p.x = mydesign.Blocks[ci.iter2][curr_sp.selected[ci.iter2]]
                            .width -
                        p.x;
                if (Blocks[ci.iter2].V_flip)
                  p.y = mydesign.Blocks[ci.iter2][curr_sp.selected[ci.iter2]]
                            .height -
                        p.y;
                p += bp;
                if (p.y < distTerm) {
...
void ILP_solver::UpdateHierNode(design &mydesign, SeqPair &curr_sp,
                                PnRDB::hierNode &node,
                                PnRDB::Drc_info &drcInfo) {
  node.width = UR.x;
  node.height = UR.y;
  ...
  node.HPWL_extend_wo_terminal =
      node.HPWL_extend -
      HPWL_extend_terminal; // HPWL without terminal nets' HPWL
  ...
  for (unsigned int i = 0; i < mydesign.Blocks.size(); ++i) {
    ...
    node.HPWL_extend += node.Blocks[i]
                            .instance[node.Blocks.at(i).selectedInstance]
                            .HPWL_extend_wo_terminal;
    node.HPWL_extend_wo_terminal +=
        node.Blocks[i]
            .instance[node.Blocks.at(i).selectedInstance]
            .HPWL_extend_wo_terminal;
    ...
    UpdateBlockinHierNode(mydesign, ort, node, i, curr_sp.GetBlockSelected(i),
                          drcInfo);
  }
...
void ILP_solver::UpdateHierNodeAnalytical(design &mydesign,
                                          PnRDB::hierNode &node,
                                          PnRDB::Drc_info &drcInfo) {
  node.width = UR.x;
  node.height = UR.y;
...
void ILP_solver::UpdateBlockinHierNode(design &mydesign, placerDB::Omark ort,
                                       PnRDB::hierNode &node, int i, int sel,
                                       PnRDB::Drc_info &drcInfo) {
  ...
  for (int j = 0; j < mydesign.GetBlockPinNum(i, sel); j++) {
    ...
    for (unsigned int k = 0; k < nd.blockPins.at(j).pinContacts.size(); k++) {
      nd.blockPins.at(j).pinContacts.at(k).placedBox =
          ConvertBoundaryData(boundary.at(k));
      nd.blockPins.at(j).pinContacts.at(k).placedCenter =
          ConvertPointData(center.at(k));
    }
    // update pin vias
    for (unsigned int k = 0;
         k < node.Blocks.at(i).instance.at(sel).blockPins.at(j).pinVias.size();
         k++) {
      auto &pv = nd.blockPins.at(j).pinVias.at(k);
      pv.placedpos = mydesign.GetPlacedBlockInterMetalAbsPoint(
          i, ort, pv.originpos, LL, sel);
      pv.UpperMetalRect.placedBox = mydesign.GetPlacedBlockInterMetalAbsBox(
          i, ort, pv.UpperMetalRect.originBox, LL, sel);
      pv.LowerMetalRect.placedBox = mydesign.GetPlacedBlockInterMetalAbsBox(
          i, ort, pv.LowerMetalRect.originBox, LL, sel);
      pv.ViaRect.placedBox = mydesign.GetPlacedBlockInterMetalAbsBox(
          i, ort, pv.ViaRect.originBox, LL, sel);
      pv.UpperMetalRect.placedCenter =
          mydesign.GetPlacedBlockInterMetalAbsPoint(
              i, ort, pv.UpperMetalRect.originCenter, LL, sel);
      pv.LowerMetalRect.placedCenter =
          mydesign.GetPlacedBlockInterMetalAbsPoint(
              i, ort, pv.LowerMetalRect.originCenter, LL, sel);
      pv.ViaRect.placedCenter = mydesign.GetPlacedBlockInterMetalAbsPoint(
          i, ort, pv.ViaRect.originCenter, LL, sel);
    }
  }
  // update internal metals
  for (unsigned int j = 0;
       j < node.Blocks.at(i).instance.at(sel).interMetals.size(); j++) {
    auto &im = nd.interMetals.at(j);
    im.placedBox =
        mydesign.GetPlacedBlockInterMetalAbsBox(i, ort, im.originBox, LL, sel);
    im.placedCenter = mydesign.GetPlacedBlockInterMetalAbsPoint(
        i, ort, im.originCenter, LL, sel);
  }
  // update internal vias
  for (unsigned int j = 0;
       j < node.Blocks.at(i).instance.at(sel).interVias.size(); j++) {
    auto &iv = nd.interVias.at(j);
    iv.placedpos = mydesign.GetPlacedBlockInterMetalAbsPoint(
        i, ort, iv.originpos, LL, sel);
    iv.UpperMetalRect.placedBox = mydesign.GetPlacedBlockInterMetalAbsBox(
        i, ort, iv.UpperMetalRect.originBox, LL, sel);
    iv.LowerMetalRect.placedBox = mydesign.GetPlacedBlockInterMetalAbsBox(
        i, ort, iv.LowerMetalRect.originBox, LL, sel);
    iv.ViaRect.placedBox = mydesign.GetPlacedBlockInterMetalAbsBox(
        i, ort, iv.ViaRect.originBox, LL, sel);
    iv.UpperMetalRect.placedCenter = mydesign.GetPlacedBlockInterMetalAbsPoint(
        i, ort, iv.UpperMetalRect.originCenter, LL, sel);
    iv.LowerMetalRect.placedCenter = mydesign.GetPlacedBlockInterMetalAbsPoint(
        i, ort, iv.LowerMetalRect.originCenter, LL, sel);
    iv.ViaRect.placedCenter = mydesign.GetPlacedBlockInterMetalAbsPoint(
        i, ort, iv.ViaRect.originCenter, LL, sel);
  }
}

void ILP_solver::UpdateTerminalinHierNode(design &mydesign,
                                          PnRDB::hierNode &node,
                                          PnRDB::Drc_info &drcInfo) {
  ...
  for (int i = 0; i < (int)mydesign.GetSizeofTerminals(); i++) {
    ...
    for (auto c : node.Nets[terminal_to_net[i]].connected) {
      if (c.type == PnRDB::Terminal)
        continue;
      for (auto con : node.Blocks[c.iter2]
                          .instance[node.Blocks[c.iter2].selectedInstance]
                          .blockPins[c.iter]
                          .pinContacts) {
        tC.push_back(con);
  ...
  for (int i = 0; i < (int)mydesign.GetSizeofTerminals(); i++) {
    ...
    // tC.back() has other fields that remain at their default values:
  originBox, placedBox, originCenter tC.back().originCenter = c;
...
void ILP_solver::UpdateSymmetryNetInfoAnalytical(design &mydesign,
                                                 PnRDB::hierNode &node, int i,
                                                 int SBidx,
                                                 placerDB::Smark axis_dir) {
  auto logger = spdlog::default_logger()->clone(
      "placer.ILP_solver.UpdateSymmetryNetInfo");
  ...
  if (axis_dir == placerDB::V) {
    if (mydesign.SBlocks[SBidx].selfsym.size() > 0) {
      ...
      axis_coor =
          Blocks[mydesign.SBlocks[SBidx].selfsym[0].first].x +
          mydesign.Blocks[mydesign.SBlocks[SBidx].selfsym[0].first][0].width /
              2;
    } else {
      // sym pair x axis coordinate
      axis_coor =
          Blocks[mydesign.SBlocks[SBidx].sympair[0].first].x / 2 +
          mydesign.Blocks[mydesign.SBlocks[SBidx].sympair[0].first][0].width /
              4 +
          Blocks[mydesign.SBlocks[SBidx].sympair[0].second].x / 2 +
          mydesign.Blocks[mydesign.SBlocks[SBidx].sympair[0].second][0].width /
              4;
    }
    ...
    if (mydesign.SBlocks[SBidx].selfsym.size() > 0) {
      axis_coor =
          Blocks[mydesign.SBlocks[SBidx].selfsym[0].first].y +
          mydesign.Blocks[mydesign.SBlocks[SBidx].selfsym[0].first][0].height /
              2;
    } else {
      axis_coor =
          Blocks[mydesign.SBlocks[SBidx].sympair[0].first].y / 2 +
          mydesign.Blocks[mydesign.SBlocks[SBidx].sympair[0].first][0].height /
              4 +
          Blocks[mydesign.SBlocks[SBidx].sympair[0].second].y / 2 +
          mydesign.Blocks[mydesign.SBlocks[SBidx].sympair[0].second][0].height /
              4;
    }
  } else {
  ...
  for (std::vector<PnRDB::net>::iterator it = node.Nets.begin();
       it != node.Nets.end(); ++it) {
...
void ILP_solver::UpdateSymmetryNetInfo(design &mydesign, PnRDB::hierNode &node,
                                       int i, int SBidx,
                                       placerDB::Smark axis_dir,
                                       SeqPair &curr_sp) {
  auto logger = spdlog::default_logger()->clone(
  ...
  if (axis_dir == placerDB::V) {
    if (mydesign.SBlocks[SBidx].selfsym.size() > 0) {
      ...
      axis_coor =
          ...
          mydesign.Blocks
                  [mydesign.SBlocks[SBidx].selfsym[0].first]
                  [curr_sp.selected[mydesign.SBlocks[SBidx].selfsym[0].first]]
                      .width /
              2;
    } else {
      // sym pair x axis coordinate
      axis_coor =
          Blocks[mydesign.SBlocks[SBidx].sympair[0].first].x / 2 +
          mydesign.Blocks
                  [mydesign.SBlocks[SBidx].sympair[0].first]
                  [curr_sp.selected[mydesign.SBlocks[SBidx].sympair[0].first]]
                      .width /
              4 +
          Blocks[mydesign.SBlocks[SBidx].sympair[0].second].x / 2 +
          mydesign.Blocks
                  [mydesign.SBlocks[SBidx].sympair[0].second]
                  [curr_sp.selected[mydesign.SBlocks[SBidx].sympair[0].second]]
                      .width /
              4;
    }
  } else if (axis_dir == placerDB::H) {
...
PnRDB::bbox ILP_solver::ConvertBoundaryData(vector<placerDB::point> Bdata) {
  PnRDB::bbox newBdata;
  PnRDB::point tmpp;
  int x = Bdata.at(0).x, X = Bdata.at(0).x, y = Bdata.at(0).y,
      Y = Bdata.at(0).y;
  for (vector<placerDB::point>::iterator it = Bdata.begin(); it != Bdata.end();
       ++it) {
    tmpp.x = it->x;
    tmpp.y = it->y;
    if ((it->x) < x) {
