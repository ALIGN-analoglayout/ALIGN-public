  ...
  for (int i = 0.0; i < node_num1; i++) {  // start is -current, end is current
    for (unsigned int it = 0; it < Power_Grid_devices.size(); ++it) {
      ...
      if (Power_Grid_devices[it].device_type == MDB::V) {  // start is +1, end is -1
        ...
        if (power > 0) {
          rhs[node_num1 - 1 - end] = power;  // this is strange, why node_num1 - end ; end is a nagetive number; one only one Q: need to combine some when the
                                             // voltage have some start node number
        }
      }
    }
  }
    ...
    if (i < node_num1) {
      ...
      for (unsigned int j = 0; j < Power_Grid_devices.size(); ++j) {
        ...
        if (Power_Grid_devices[j].device_type == 2 && Power_Grid_devices[j].start_point_index == i + 1) {
          ...
          int position =
              node_num1 - Power_Grid_devices[j].end_point_index - 0.5 * powerdev * flag - 1;  // vdd and gnd power devices have different index? or their end?
...
void MNASimulation::Print_Devices(std::vector<MDB::device> &temp_devices) {
  ...
  for (unsigned int i = 0; i < temp_devices.size(); i++) {
    logger->debug("devices type {0} point1 {1} point2 {2} {3} ", temp_devices[i].device_type, temp_devices[i].start_point_index,
                  temp_devices[i].end_point_index, temp_devices[i].value);
  }
};
...
void MNASimulation::ExtractPowerGridPoint(PnRDB::PowerGrid &temp_grid, std::set<MDB::metal_point, MDB::Compare_metal_point> &temp_set, int &highest_metal,
                                          int &lowest_metal, double power) {
  MDB::metal_point temp_point;
...
void MNASimulation::ExtractPowerGridWireR(PnRDB::PowerGrid &temp_grid, std::set<MDB::metal_point, MDB::Compare_metal_point> &temp_set,
                                          PnRDB::Drc_info &drc_info, std::vector<MDB::device> &Power_Grid_devices, double power) {
  for (unsigned int i = 0; i < temp_grid.metals.size(); ++i) {
    MDB::device temp_device;
    MDB::metal_point temp_point;
...
void MNASimulation::ExtractPowerGridViaR(PnRDB::PowerGrid &temp_grid, std::set<MDB::metal_point, MDB::Compare_metal_point> &temp_set, PnRDB::Drc_info &drc_info,
  ...
  for (unsigned int i = 0; i < temp_grid.vias.size(); ++i) {
    ...
    temp_point.x = x;
    temp_point.y = y;
    // std::cout << "layer1= "<< temp_point.metal_layer <<" index1= "<<temp_point.index <<" power1= "<< temp_point.power << " x1= "<< temp_point.x <<" y1= "<<
    // temp_point.y << std::endl;
    if (temp_set.find(temp_point) == temp_set.end()) {
      continue;
    }
    auto frist_point = temp_set.find(temp_point);
    int start_index = frist_point->index;
    ...
    temp_point.x = x;
    temp_point.y = y;
    // std::cout << "layer2= "<< temp_point.metal_layer << " index2= "<<temp_point.index <<" power2= "<< temp_point.power << " x2= "<< temp_point.x <<" y2= "<<
    // temp_point.y << std::endl;
    if (temp_set.find(temp_point) == temp_set.end()) {
      continue;
    }
...
void MNASimulation::FindPowerPoints(std::set<MDB::metal_point, MDB::Compare_metal_point> &point_set, double power, int metal_layer, int power_number,
...
void MNASimulation::Map_new(std::vector<std::vector<double>> &currentstore, std::set<MDB::metal_point, MDB::Compare_metal_point> &point_set,
                            std::vector<MDB::device> &Power_Grid_devices, int metal_layer) {
  // it is adding some current devices
  for (unsigned int i = 0; i < currentstore.size(); ++i) {
    ...
    // std::cout<<"power mesh multi-connection "<<initial_x<<" "<<initial_y<<" "<<vdd_maxx<<" "<<vdd_maxy<<" "<<multi_connection<<" "<<unit_r<<"
    // "<<temp_device.value<<std::endl;
    Power_Grid_devices.push_back(temp_device);
    ...
    // std::cout<<"power mesh multi-connection "<<initial_x<<" "<<initial_y<<" "<<gnd_maxx<<" "<<gnd_maxy<<" "<<multi_connection<<" "<<unit_r<<"
    // "<<temp_device.value<<std::endl;
    Power_Grid_devices.push_back(temp_device);
