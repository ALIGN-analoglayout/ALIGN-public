...
class spiceParser:
    def __init__(self, netlistPath, top_ckt_name):
    ...
    def parser(self):
        if not os.path.isfile(self.netlist):
            ...
            #    print("not a test bench circuit")
            #    top_name= input("please enter subcircuit_name:")
            #    print ("using top as :",top_name)
            ...
            if FLAT:
                ...
                #Design = self._hier_circuit("TB_IBS_norm")
    ...
    def _show_circuit_graph(self,filename, graph):
        print("writing circuit graph:",graph)
        for subgraph in nx.connected_component_subgraphs(graph):
            ...
            #print("Graph for input netlist is in circuit: circuit_graphs/",filename)
...
if __name__ == '__main__':
    ...
    netlist= sys.argv[1] 
    top_ckt_name= sys.argv[2] 
    print("checking input circuit files in dir: input_circuit")
    if len(netlist_files)==0:
        print("No spice files found in input_circuit directory. exiting")
        exit(0)
    elif netlist not in netlist_files: 
        print("No spice files found in input_circuit directory with name",netlist,". exiting")
        exit(0)

    #output_dir ='train_nodes/'
    sp = spiceParser(netlist_dir+netlist,top_ckt_name)
