  ...
  for(int i=0;i<(int)HGraph.size();i++) {
    ...
    for(vector<int>::iterator it=candidate.begin(); it!=candidate.end(); ++it) {
      ...
      if(i<caseNL.GetSizeofBlocks() and *it<caseNL.GetSizeofBlocks() ) {
        if(mode==0) {
          // isBackward is set to zero for all edges.
...
double ConstGraph::LinearConst(design& caseNL, SeqPair& caseSP){
  ...
  for(int i=0;i< caseNL.Nets.size(); i++){
     ...
     if(temp_sum<=caseNL.Nets[i].upperBound){ // TD: upperBound may have been assigned with LinearConst()
        temp_sum = 0;
...
double ConstGraph::ML_LinearConst(design& caseNL, SeqPair& caseSP){
  ...
  for(int i=0;i<caseNL.ML_Constraints.size();i++){ // Each ML_Constraints represents one MLL_linearConst
     double temp_sum = 0;
...
double ConstGraph::MLPredict(design& caseNL, SeqPair& caseSP){
  std::cout << "Inside MLPredict" << std::endl;
  int BPin;
  double sum = 0;
  double cost = 0;
  double norm_scale = 2000;
//  double resistance_per_unit = 30;
//  double capacitance_per_unit = 2e-16;
double resistance_per_unit = 1;
double capacitance_per_unit = 1;
  std::vector<std::vector<double> > feature_value;
  std::vector<double> parasitic_value;
  std::vector<std::vector<std::string> > feature_name;

  ExtractLength(caseNL, caseSP, feature_value, feature_name);

  for(int i=0;i<caseNL.ML_Predict.size();i++){ // Each ML_Constraints represents one MLL_linearConst
//    std::cout << "Inside MLPredict 1" << std::endl;
     double temp_sum = 0;
     for(int j=0;j<caseNL.ML_Predict[i].VectConstraints.size();j++){
//           std::cout << "Inside MLPredict 2" << std::endl;
           int index_i=0;
           int index_j=0;
           //std::cout<<"ML Linear "<<caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].first<<" "<<caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].second<<std::endl;
           for(int m=0;m<caseNL.Nets.size();m++){
               for(int n=0;n<caseNL.Nets[m].connected.size();n++){
//                 std::cout << "Inside MLPredict 3" << std::endl;

                  //std::cout<<"searching" <<m<<" "<<n<<" "<<caseNL.Nets[m].connected[n].iter << " "<< caseNL.Nets[m].connected[n].iter2 << " " << caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].first << " " << caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].second<<std::endl;
                  double temp_parasitic_value;
                  if(caseNL.Nets[m].connected[n].iter2 == caseNL.ML_Predict[i].VectConstraints[j].pin.first and caseNL.Nets[m].connected[n].iter == caseNL.ML_Predict[i].VectConstraints[j].pin.second){
//                    std::cout << "Inside MLPredict 4" << std::endl;
                     //std::cout<<" found " <<caseNL.Nets[m].connected[n].iter2 << " "<< caseNL.Nets[m].connected[n].iter << " " << caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].first << " " << caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].second<<std::endl;

                     index_i=m;
                     index_j=n;
                     if(caseNL.ML_Predict[i].VectConstraints[j].type.compare("r")==0){
                       temp_parasitic_value = feature_value[index_i][index_j]/norm_scale*resistance_per_unit;
                     } else if(caseNL.ML_Predict[i].VectConstraints[j].type.compare("c")==0) {
                       temp_parasitic_value = feature_value[index_i][index_j]/norm_scale*capacitance_per_unit;
                     }else {};
                     parasitic_value.push_back(temp_parasitic_value);
                     std::cout << "Feature name: " << feature_name[index_i][index_j] << "Parasitic value: " << temp_parasitic_value << std::endl;
                     break;
                  }
               }
              }
         //std::cout<< caseNL.Nets[index_i].connected[index_j].iter2 << " "<< caseNL.Nets[index_i].connected[index_j].iter << " " << caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].first << " " << caseNL.ML_Constraints[i].Multi_linearConst[j].pins[k].second<<std::endl;
//         std::cout<<"MLLinearConst Cost: alpha "<<caseNL.ML_Constraints[i].Multi_linearConst[j].alpha[k]<<" upperbound "<<caseNL.ML_Constraints[i].upperBound<<" block idex "<<caseNL.Nets[index_i].connected[index_j].iter2<<" pin idx "<<caseNL.Nets[index_i].connected[index_j].iter<<" index_i "<<index_i<<" index_j "<<index_j<<" dist "<<feature_value[index_i][index_j]<<" alpha*dist "<<caseNL.ML_Constraints[i].Multi_linearConst[j].alpha[k]*feature_value[index_i][index_j]<<" pin_name "<<feature_name[index_i][index_j]<<std::endl;
//         temp_sum += caseNL.ML_Predict[i].Multi_linearConst[j].alpha[k]*feature_value[index_i][index_j];
     }
//     if(temp_sum<=caseNL.ML_Predict[i].upperBound){
//        temp_sum = 0;
//     }else{
//        temp_sum = caseNL.ML_Predict[i].upperBound;
//     }
//     sum += temp_sum;

  cost = CallMLModelPython(caseNL.ML_Predict[i].spec_name, parasitic_value);
  std::cout << "cost: " << cost << std::endl;
  parasitic_value.clear();
  sum  = sum + cost;

    // if ((caseNL.ML_Predict[i].spec_name.compare("maxgain") == 0) && (gain_index > 10)){
    //   gain_SA[gain_index] = cost;
    //   gain_index += 1;
    // }
    // else if (gain_index == 10000){
    //   for (int h = 0; h < 10000; h++){
    //     std::cout << "Gain at iteration " << h << ": " << gain_SA[h] << std::endl;
    //   }
    //   int BPin;
    //   std::cout << "Breakpoint at ConstGraph.cpp at line 2265. Press any number: " << std::endl;
    //   std::cin >> BPin;
    // }
  }

  // sum = CallMLModelPython(parasitic_value);
//  std::cout << "sum: " << sum << std::endl;
//  std::cout << "Breakpoint at ConstGraph.cpp line 2252. Press any number: " << std::endl;
//  std::cin >> BPin;
  return sum;
}
...
void ConstGraph::ExtractLength(design& caseNL, SeqPair& caseSP, std::vector<std::vector<double> > &feature_value, std::vector<std::vector<std::string> > &feature_name){
  ...
  std::string pin_name;
  int BPin; //TD: added to insert breakpoints
...
//        std::cout << "Pos_pin size: " << pos_pin.size() << std::endl;
// pos_pin are always 1 for the telescopic_ota example
        for(unsigned int i=0;i<pos_pin.size();i++){
          p = pos_pin[i];
          std::cout<<"Pin Center (x, y)"<<p.x<<" "<<p.y<<std::endl;
          pos.push_back(p);
	}
//        std::cout << "Breakpoint at ConstGraph.cpp at line 2224. Press any number:" << std::endl;
//        std::cin >> BPin;
...
double ConstGraph::CalculateCost(design& caseNL, SeqPair& caseSP) {
  int BPin;
  double cost=0;
  double cost_penalty = 0;
  double cost_wire_length = 0;
  double cost_match = 0;
  double cost_ratio = 0;
  double cost_area = 0;
  double cost_deadarea = 0;
  double cost_linear_const = 0;
  double cost_mllinear_const = 0;
  double cost_mlpredict = 0;
  CalculateLongestPath(sourceNode, this->HGraph, false);
  CalculateLongestPath(sourceNode, this->VGraph, false);
  ...
  // cost += MLPredict(caseNL, caseSP)*PIII;
  cost_penalty = (CalculatePenalty(this->HGraph)+CalculatePenalty(this->VGraph))*GAMAR;
  cost_wire_length = CalculateWireLength(caseNL, caseSP)*LAMBDA;
  cost_match = CalculateMatchCost(caseNL, caseSP)*BETA*0;
  cost_ratio = CalculateRatio()*SIGMA*0;
  cost_area = CalculateArea();
  cost_deadarea = CalculateDeadArea(caseNL, caseSP)*PHI;
  cost_linear_const = LinearConst(caseNL, caseSP)*PI*0;
  cost_mllinear_const = ML_LinearConst(caseNL, caseSP)*PII*0;
  cost_mlpredict = MLPredict(caseNL, caseSP)*PIII;
  cost = cost_penalty + cost_wire_length + cost_match + cost_ratio + cost_area + cost_deadarea;
  cost = cost + cost_linear_const + cost_mllinear_const + cost_mlpredict;
  //std::cout<<"GAMAR:"<<GAMAR<<" BETA "<<BETA<<"LAMBDA "<<LAMBDA<<"SIGMA "<<SIGMA<<"PHI "<<PHI<<"PI "<<PI<<"PII "<<PII<<"PIII "<<PIII<<std::endl;
  // std::cout<<"Penalt: "<<  (CalculatePenalty(this->HGraph)+CalculatePenalty(this->VGraph))*GAMAR<<" vs "<< (CalculatePenalty(this->HGraph)+CalculatePenalty(this->VGraph)) << std::endl;
  // std::cout<<"WL: "<<CalculateWireLength(caseNL, caseSP)*LAMBDA<<" vs "<<CalculateWireLength(caseNL, caseSP)<<std::endl;
  // std::cout<<"Area: "<<CalculateArea()<<std::endl;
  // std::cout<<"MAtch: "<<CalculateMatchCost(caseNL, caseSP)*BETA<<" vs "<< CalculateMatchCost(caseNL, caseSP) <<std::endl;
  // std::cout<<"Ratio: "<<CalculateRatio()*SIGMA<<" vs "<<CalculateRatio() <<std::endl;
  // std::cout<<"DeadArea: "<<CalculateDeadArea(caseNL, caseSP)*PHI<<" vs "<<CalculateDeadArea(caseNL, caseSP)<<std::endl;
  // std::cout<<"ML_Predict: "<<MLPredict(caseNL, caseSP)*PIII<<" vs "<<MLPredict(caseNL, caseSP)<<std::endl;
  std::cout << "cost_penalty: " << cost_penalty << ", GAMAR: " << GAMAR << std::endl;
  std::cout << "cost_wire_length: " << cost_wire_length << ", LAMBDA: " << LAMBDA << std::endl;
  std::cout << "cost_match: " << cost_match << ", BETA: " << BETA << std::endl;
  std::cout << "cost_ratio: " << cost_ratio << ", SIGMA: " << SIGMA << std::endl;
  std::cout << "cost_area: " << cost_area << ", CONST: 1" << std::endl;
  std::cout << "cost_deadarea: " << cost_deadarea << ", PHI: " << PHI << std::endl;
  std::cout << "cost_linear_const: " << cost_linear_const << ", PI: " << PI << std::endl;
  std::cout << "cost_mllinear_const: " << cost_mllinear_const << ", PII: " << PII << std::endl;
  std::cout << "cost_mlpredict: " << cost_mlpredict << ", PIII: " << PIII << std::endl;
  std::cout << "cost: " << cost << std::endl;
//  std::cout << "Breakpoint at ConstGraph.cpp at line 2856. Press any number: "<<std::endl;
//  std::cin >> BPin;
  return cost;
}
...
void ConstGraph::Update_parameters(design& caseNL, SeqPair& caseSP) {
  ...
  if(MLPredict(caseNL, caseSP)>0){
  PII = cost/MLPredict(caseNL, caseSP) * 1.0;
  }
...
void ConstGraph::updateTerminalCenter(design& caseNL, SeqPair& caseSP) {
  ...
  for(int i=0;i<caseNL.GetSizeofTerminals();++i) {
    ...
    int netIdx=caseNL.Terminals.at(i).netIter;
    int sbIdx=caseNL.Terminals.at(i).SBidx;
    int cp=caseNL.Terminals.at(i).counterpart; // TD: ?
    if(netIdx<0 or netIdx>=caseNL.GetSizeofNets()) {
      std::cout<<"Placer-Warning: terminal "<<i<<" is dangling; set it on origin\n";
      caseNL.Terminals.at(i).center.x = 0;
      caseNL.Terminals.at(i).center.y = 0;
