...
def analysis_with_svm(WORK_DIR, param_list, PoI, mode):
...
def analysis_with_mlp(WORK_DIR, param_list, PoI, mode):
    df = pd.read_csv(WORK_DIR
                     + '/outputs/database_for_FT_' + str(mode) + '.csv')
    df_specs = pd.read_csv(WORK_DIR
                           + "/inputs/spec_limits.csv").set_index('bounds')
    ...
    filename = WORK_DIR + "/outputs/trained_model_mlp_" + PoI + ".sav"
    pickle.dump(clf, open(filename,'wb'))
