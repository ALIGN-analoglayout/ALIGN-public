...
def test_group_block_hsc(dir_name):
    ...
    gen_const = [const for const in gen_const if not isinstance(const, constraint.GroupBlocks)]
...
def test_symmnet_translation():
    ...
    symnet_const = [const for const in all_const if isinstance(const, constraint.SymmetricNets)][0]
    modified_symmnet = {
        "constraint": "SymmetricNets",
        "direction": "V",
        "net1": "A",
