...
def test_group_block_hsc(dir_name):
    ...
    compiler_output(
        test_path, updated_cktlib, "high_speed_comparator", result_path, pdk_dir
    )
    ...
    with open(gen_const_path, "r") as const_fp:
        gen_const = next(
            x
            for x in json.load(const_fp)["modules"]
            if x["name"] == "HIGH_SPEED_COMPARATOR"
        )["constraints"]
...
def test_scf():
    ...
    compiler_output(
        test_path, updated_cktlib, "SWITCHED_CAPACITOR_FILTER", result_path, pdk_dir
    )
    ...
    with open(gen_const_path, "r") as const_fp:
        gen_const = next(
            x
            for x in json.load(const_fp)["modules"]
            if x["name"] == "SWITCHED_CAPACITOR_FILTER"
        )["constraints"]
        gen_const.sort(key=lambda item: item.get("constraint"))
    with open(gold_const_path, "r") as const_fp:
        gold_const = json.load(const_fp)
